...
**Di layar DESKTOP (≥768px), tambahkan JUGA `.gc-card-menonjol`** kalau
kartu itu tampil di area `<main>` desktop (`.gc-main-desktop`, latar
putih) — lihat bagian "Kontras kartu vs latar putih desktop" di bawah
untuk penjelasan lengkap & aturan rollout-nya.
...
`.kpi` dengan `.label`, `.val`, `.delta.up`/`.delta.down` — dipakai buat ringkasan angka. **BEDA** dari `.gc-kpi-card` (Beranda desktop, lihat bagian Redesain Desktop di bawah) — itu varian gradien maroon khusus dashboard, bukan pengganti `.kpi` biasa. **Catatan (1 Sep 2026)**: tab "Selesai" di Bahan/Acc Sewing/Acc Webbing/Acc Finishing pakai gaya kartu-mini INLINE sendiri (`border:1px dashed var(--line); background:var(--ivory-dim)`), BUKAN class `.kpi` ini — konsisten dari sisi warna (sudah ivory-dim, sudah kontras dari `.gc-main-desktop` putih) tapi bukan pemakaian literal class `.kpi`, jangan bingung kalau grep `.kpi` tidak ketemu di file-file itu.
...
**REVISI (30 Agt 2026, sesi lanjutan)** — 3 hal, permintaan Guru lihat
screenshot live vs mockup:
1. **Teks brand** `.gc-side-brand` GANTI dari "Gechoo ♥" jadi "Zevanic
   ERP" (lalu **"Zevanic Core Optima"**, lihat revisi berikutnya) + ikon-kotak
   baru `.gc-side-brand-ico` (32px radius 10px latar
   `--burgundy`, glyph `--ivory`) — persis mockup. Blok ini SUDAH
   desktop-only dari awal (`.gc-sidebar hidden md:flex`), jadi ganti
   teks di sini TIDAK menyentuh branding mobile sama sekali (mobile
   punya header sendiri, tidak pernah menampilkan blok ini).
2. **Ikon ditambah ke 7 tombol kepala grup** (Pesanan/Master Absensi/
   Master Keuangan/Master Karyawan/Master Integrasi/Zevanic House/
   Persiapan Produksi) — dulu cuma teks+chevron, sekarang `.gc-nav-gico`
   (14px, text-faint biasa → aksen-ink saat grup terbuka). Ikonnya
   PERSIS data mockup (`mockup-desktop-adaptasi.html`, variabel `GRUPS`):
   fa-receipt/fa-clock/fa-sack-dollar/fa-users/fa-plug/fa-shirt/
   fa-diagram-project. Sub-item nested SUDAH lama punya ikon sendiri
   (tidak berubah).
3. **Padding & font-size dirapatkan** — base `.gc-nav-item` dari
   `padding:10px 12px; font-size:13px` jadi `padding:9px 10px;
   font-size:12.5px` (mendekati spec mockup 8px 10px/11.5px). Ini CUMA
   pengaruh ke item yang tidak override inline (Beranda/Profile + 7
   kepala grup) — sub-item nested sudah override inline sendiri, tidak
   ikut berubah.

**REVISI LANJUTAN (30 Agt 2026, ronde audit desain)** — sidebar &
topbar dikoreksi BALIK ke ivory (`.gc-sidebar`/`.gc-side-brand`/
`.gc-topbar-desktop` semua `var(--ivory)`, BUKAN putih seperti tertulis
di paragraf redesain di atas) — dicek ulang ke artefak mockup
`mockup-desktop-adaptasi.html`, ternyata `.sb`/topbar di sana memang
krem, bukan putih. Yang jadi putih SEBALIKNYA adalah area **KONTEN**
(`<main>`, lihat "Kontras kartu vs latar putih desktop" di bawah) —
supaya kartu krem di dalam konten menonjol dari latar konten yang
putih, BUKAN sidebar/topbar yang putih. Paragraf redesain warna sidebar
di atas (poin 1-3) soal STRUKTUR/teks/ikon/padding tetap berlaku apa
adanya, cuma soal WARNA LATAR sidebar/topbar-nya yang final ivory,
bukan putih.

State "grup terbuka" SEKARANG juga menambah class `.gc-grp-buka` ke
tombol kepala grup itu sendiri (dulu cuma chevron yang berubah, latar/
warna teks tombolnya tidak) — lihat `setGrupSidebarTerbuka()` di
`js/dashboard.js`.
...
## 🆕 Kontras kartu vs latar putih desktop (`.gc-card-menonjol`, BARU 31 Agt 2026)

> Konteks lengkap keputusan & aturan rollout ada di
> `PEDOMAN-GAYA-KERJA.md` bagian "WAJIB dicek tiap bikin/edit menu
> DESKTOP" — bagian ini fokus ke definisi teknis class-nya.

Sejak ronde audit desain Beranda (30 Agt 2026, lihat catatan "REVISI
LANJUTAN" di bagian Sidebar desktop di atas), area `<main>` DESKTOP
(class `.gc-main-desktop`, cuma aktif di `@media(min-width:768px)`)
punya latar **PUTIH** (`var(--surface)`) — TERPISAH dari latar
`<body class="gc-app">` yang tetap ivory (dipakai MOBILE, TIDAK
disentuh). Tujuannya: kartu ivory di dalamnya (`.gc-pipeline-card` di
Beranda, dst) kelihatan MENONJOL sebagai lapisan di atas konten, bukan
menyatu warna dengan latarnya.

**Masalahnya**: kartu di submenu LAIN (di luar Beranda) masih pakai
`.gc-card` polos — background-nya `var(--surface)`, **SAMA PUTIH**
dengan `.gc-main-desktop` di desktop. Efeknya, kartu-kartu itu
MENYATU lagi dengan latar konten di layar ≥768px — bug yang SAMA
seperti yang sudah dikoreksi di Beranda, cuma belum ikut dikoreksi di
submenu lain. Guru sempat menduga latar putih ini "belum
diimplementasikan" (31 Agt 2026) — dicek langsung ke kode, TERNYATA
sudah ada & benar di Beranda, cuma rollout-nya ke submenu lain yang
belum jalan.

**Solusi — class `.gc-card-menonjol`** (`css/gechoo-design.css`):
```css
@media(min-width:768px){
  .gc-card-menonjol{background:var(--ivory);}
}
```
Ditambahkan sebagai kelas KEDUA berdampingan dengan `.gc-card` (BUKAN
pengganti — `.gc-card` tetap dipakai buat border/radius/padding
dasarnya), jadi markup-nya `class="gc-card gc-card-menonjol"`. SENGAJA
di dalam media query desktop: di mobile (<768px) TIDAK ada perubahan
apapun — di sana `<main>` masih ivory dari `.gc-app`, jadi kartu putih
`.gc-card` sudah kontras dengan sendirinya tanpa butuh class tambahan
ini.

**Pilot & status rollout**: pertama diterapkan di "Perlu Disiapkan"
(Persiapan Produksi V2, `js/vue-persiapan-produksi-v2.js`, 31 Agt
2026) — kartu klaster, panel "Grouping baru", state kosong, placeholder
"Memuat...". DISETUJUI Guru ("mantap"), jadi pola RESMI ke depan.

**UPDATE ROLLOUT (1 Sep 2026)** — saat modul **Bahan** dibangun ulang
total (31 Agt 2026) lalu **Acc Sewing/Webbing/Finishing** (1 Sep 2026),
pola ini DIIKUTI di ke-4 file baru tersebut (`js/vue-persiapan-bahan.js`,
`vue-persiapan-sewing.js`, `vue-persiapan-webbing.js`, `vue-persiapan-
finishing.js`) — TAPI saat pengecekan silang (dipicu pertanyaan Guru
"apakah warna konten sudah disamakan"), ketahuan penerapannya BARU
SEBAGIAN: kartu daftar utama (kartu bahan/operator/sepack/tugas per
tab) SUDAH dapat `.gc-card-menonjol`, tapi **6 kartu per file TERLEWAT**
— 5 kartu placeholder "Memuat..." (1 per komponen tab) + 1 kartu
"Riwayat Saya" (tab Selesai, versi operator). **SUDAH DIPERBAIKI** (24
baris total di 4 file, ditambah `gc-card-menonjol` + versi cache-bust
dinaikkan `js/dashboard.js` **TIDAK** perlu naik karena tidak disentuh
— cukup naikkan versi query-string masing-masing file JS di
`index.html`: `vue-persiapan-bahan.js?v=2→3`, `vue-persiapan-sewing.js`/
`vue-persiapan-webbing.js`/`vue-persiapan-finishing.js` `?v=1→2`) — jadi
SEKARANG SEMUA kartu konten utama di ke-4 file itu konsisten pakai
`.gc-card-menonjol`. **Yang SENGAJA TIDAK diberi class ini**: kartu
modal/dialog (`max-width:360px`, dialog scan QR/konfirmasi dsb) — modal
mengambang di atas backdrop-nya sendiri, bukan bagian dari masalah
"menyatu dengan latar `<main>`" yang class ini pecahkan, jadi tetap
`.gc-card` polos di 4 file itu (3 modal per file — cek langsung kalau
ragu, jangan asumsikan "semua gc-card harus menonjol tanpa kecuali").

**Status rollout per 1 Sep 2026**: SUDAH konsisten di "Perlu Disiapkan",
Bahan, Acc Sewing, Acc Webbing, Acc Finishing (5 dari 6 sub-menu
Persiapan Produksi V2). **BELUM** (masih `.gc-card` polos): jalur
**Vendor** (SATU-SATUNYA yang masih generik lewat `JalurTahapManager`
lama, belum dibangun ulang) dan semua modul lain di luar Persiapan
Produksi V2 (Zevanic House, Pesanan, dst) — SENGAJA ditunda sampai
modul itu memang disentuh lagi untuk alasan lain, BUKAN retrofit massal
sekaligus.
...
| Class | Kegunaan |
|---|---|
| `.gc-topbar-desktop` | GANTI header 80px statis "ERP Portal" lama — latar ivory, isi breadcrumb (`.gc-crumb`, baris berikut) + pencarian global (`.gc-search-global`, baris berikut) + `#teks-nama-user` (JANGAN ganti id, dipakai `auth.js`) + lonceng notifikasi. Badge `#label-badge-role` (countdown shift lama, hardcode salah "01:00" utk semua orang) SUDAH DICOPOT dari topbar desktop (30 Agt 2026, sesi lanjutan) — `auth.js` diberi null-guard kalau elemen ini dicari lagi. Tinggi topbar **52px** (dikoreksi dari 80px salah, lihat catatan sidebar di atas) |
| `.gc-crumb` (+ `.gc-crumb-mid`) | Breadcrumb topbar desktop (BARU, 30 Agt 2026, sesi lanjutan) — GANTI h2 "ERP Portal" + badge shift lama. Statis "Umum › Beranda" untuk sekarang, belum ada routing breadcrumb dinamis per-layar |
| `.gc-search-global` + `.gc-palet-overlay`/`.gc-palet-card`/`.gc-palet-input`/`.gc-palet-results`/`.gc-palet-item`/`.gc-palet-label`/`.gc-palet-grup` | Pencarian global/palet Ctrl K (BARU, 30 Agt 2026, sesi lanjutan) — **REAL, bukan cuma visual**: klik tombol/Ctrl+K buka overlay, hasil dibaca LANGSUNG dari DOM sidebar (`data-menu-id`), klik hasil = trigger `.click()` tombol sidebar asli. Logic di `paletPencarianGlobal()`, `js/dashboard.js` — WAJIB baca komentar besar di atas fungsi itu sebelum ubah |
...
| `.gc-quote-desktop` | Kartu Quote kolom kanan — dipakai BARENG `.gc-kartu-gradien`, data SAMA seperti `QuoteCard` (`vue-components.js`) tapi warna beda (maroon, bukan pink-blue) — komponen terpisah, `QuoteCard` asli TIDAK diubah (mobile tetap pink-blue). **Kartu ini SENGAJA tidak render apapun kalau tidak ada dokumen `quotes` terjadwal (`tanggalTampil`) untuk HARI INI**, dihitung pakai timezone **Asia/Jakarta eksplisit** (bukan UTC/`toISOString()`, bug ditemukan & diperbaiki 30 Agt 2026 — lihat STATUS-PROYEK.md §5.9d/§5.9e) |
| `.gc-pipeline-card`/`.gc-pipeline-head`/`.gc-pipeline-tot`/`.gc-pipeline-desc`/`.gc-pipeline-steps`/`.gc-pipeline-step`/`.gc-pipeline-bar`/`.gc-pipeline-n`/`.gc-pipeline-lbl`/`.gc-pipeline-segera` | Kartu Pipeline Persiapan (data REAL, `order_spk`+`spk_track`) & Pipeline Produksi (placeholder "Segera Hadir", BELUM ada skema data — lihat komentar `js/vue-home-desktop.js`). `.gc-pipeline-card` latar `var(--ivory)` — contoh langsung pola "Kontras kartu vs latar putih desktop" di atas |
| `.gc-tindak-row`/`.gc-tindak-ico`/`.gc-tindak-txt`/`.gc-tindak-chip`/`.gc-tindak-subgrup`/`.gc-tindak-segera` | Daftar "Perlu Tindakan Anda" — DIPECAH 2 grup (30 Agt 2026, sesi lanjutan): grup "Persiapan" (real, dari Pipeline Persiapan) & grup "Produksi" (placeholder, redup lewat `.gc-tindak-segera`) |
...
| `.gc-pintasan-row` (+ `kbd`) | Kartu "Pintasan Papan Tik" kolom kanan (BARU, 30 Agt 2026, sesi lanjutan) — kartu pembungkus reuse `.gc-pipeline-card`/`.gc-pipeline-head`. **Isinya KONTEN STATIS/ILUSTRATIF** (badge Ctrl K/N/J/K/Esc di kartu ini ILUSTRATIF — TIDAK terikat listener, beda dari `.gc-search-global` yang di atas SUDAH real) — lihat catatan di bawah |
...
lintas-modul di skema data sungguhan (`PETA-DATABASE.md`). **Pengecualian**:
tombol pencarian global (`.gc-search-global`) di topbar BEDA — itu SUDAH
diikat fungsi sungguhan (bukan ilustratif), lihat baris di atas. Kalau
nanti mau Aktivitas Terbaru versi live beneran, itu kerjaan terpisah
(perlu koleksi log baru), belum diminta.
...
| `.gc-kosong` (+ `.lingkaran`) | Placeholder kartu kosong / empty state (search-x, ikon di lingkaran) | Dipakai luas: `vue-persiapan-produksi-v2.js`, `vue-persiapan-produksi.js`, `vue-order-spk.js`, `vue-account-profile.js`, `vue-menu-lengkap.js`, `vue-persiapan-bahan.js`, `vue-persiapan-sewing.js`, `vue-persiapan-webbing.js`, `vue-persiapan-finishing.js` |
