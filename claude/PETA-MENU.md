...
| **Keamanan** | 2 sub-tab: **Password** (ganti password) + **PIN** (dipakai "Absensi Melalui QR", DAN sejak 7 Sep 2026 malam — §5.14 — `pin_hash` yang sama DIREUSE untuk finalisasi Nota + Terapkan harga di Stok & Pembelian, DAN sejak §5.15 untuk gerbang PIN di Pesanan > Menunggu Proses/Transaksi Keuangan, lihat `PETA-DATABASE.md`) | ✅ Ya |
...
>
> **UPDATE LAGI (7 Sep 2026 malam, §5.15, `/design-terapkan-handoff`
> "Pesanan dan Transaksi beserta turunannya")**: TIDAK ADA yang berubah
> di grup menu ini dari rekonstruksi Pesanan — perubahan §5.15 murni di
> grup "🛒 Pesanan" di bawah. Menu "Order SPK" (sudah dipensiunkan sejak
> 30 Agt) SEKARANG hilang lebih total lagi — lihat baris "Order SPK" di
> bawah untuk detail.
>
> **UPDATE LAGI (7 Sep 2026 malam, §5.16, `/design-terapkan-handoff`
> "05 - Scan dan Cetak")**: **Riwayat PIN PINDAH KELUAR** dari Config ke
> menu top-level BARU "Scan & Cetak" (lihat bagian tersendiri di bawah,
> setelah Persiapan Produksi V2) — Config sekarang 9 tab, bukan 10.
> **Scan (Scan Opname + Scan Persiapan) JUGA PINDAH KELUAR** dari Zevanic
> House ke menu yang sama. Baris "Config"/"Scan" di tabel bawah sudah
> disesuaikan.

| Menu | File Utama | Sub-menu di dalamnya | Role | Status |
|---|---|---|---|---|
| **Config** | `vue-config.js` | **9 tab child (DIPINDAH KE 9, §5.16 — dulu 10, Riwayat PIN pindah keluar)**: **Jenis Bahan**, **Jenis Aksesoris** (lewat `MasterDataCategory`, koleksi `master_data`, kategori `jenis_bahan`/`jenis_aksesoris`), **Data Satuan**, **Data Warna**, **Data Ukuran**, **Jenis Produk**, **Data Komponen**, **Persiapan Untuk Tahap** (ke-6 ini lewat `MasterDataTabelManager`), **TLC & Prefix** (`AppConfigTlc`, koleksi `master_tlc` + kartu "Prefix Kode SPK"). ~~Riwayat PIN~~ **PINDAH (§5.16) ke Scan & Cetak > PIN** (`AppScanCetakRiwayatPin`, `js/vue-scan-cetak.js`) — lihat bagian "📷 Scan & Cetak" di bawah | Config Akses (`config_master_data`, default cuma Owner) | ⚠️ tab TLC & Prefix: kode ditulis, BELUM dikirim/di-push/diuji; 8 tab lain ✅ Aktif |
...
| **Master Pelanggan** | `js/vue-master-pelanggan.js` (BARU TOTAL, §5.12) | Single-view (`MasterPelangganManager`): list kartu per pelanggan (nama, badge tipe, saldo piutang, limit piutang) + 1 popup Tambah/Edit (nama*, telepon, alamat, email, tipe, limit_piutang, catatan). `saldo_piutang` readonly di form INI (TIDAK ada field edit manual) — **SEKARANG (7 Sep 2026, §5.15) mulai benar-benar berubah** lewat 2 titik resmi di modul Pesanan (checkout Kasir menambah, `catatPembayaranSusulan()` di Transaksi Keuangan mengurangi), bukan lagi selalu `0` seperti sebelum §5.15 — lihat `PETA-DATABASE.md`. Koleksi `master_pelanggan` | `master_pelanggan` (BARU) | ✅ **Rules dipublish + SUDAH DITES di browser (7 Sep 2026)** — Tambah/Edit/Hapus, dedupe nama, badge tipe semua jalan. SELESAI PENUH (fitur piutang yang MEMBACA/MENGUBAH `saldo_piutang` ADA di modul Pesanan §5.15, BELUM DITEST — lihat bagian "🛒 Pesanan" di bawah) |
| **Order SPK** | ~~`vue-order-spk.js`~~ | **DIPENSIUNKAN (30 Agt 2026)** — fungsi CRUD (daftar SPK, cetak label QR) PINDAH TOTAL ke **Pesanan > Menunggu Proses** (lihat bagian "🛒 Pesanan" di bawah). Tombol & div konten lama DIHAPUS dari `index.html`, `js/vue-order-spk.js` TIDAK LAGI DIMUAT (file dibiarkan ada di disk, tidak dihapus). **CATATAN (§5.15)**: bahkan bentuk "CRUD manual disalin ke Menunggu Proses" itu sendiri SEKARANG SUDAH TIDAK ADA LAGI (lihat di bawah — Menunggu Proses direbuild total jadi murni antrian QO, form CRUD manual dihapus juga dari sana) | — | ⚠️ DIPENSIUNKAN, lihat Pesanan |
| ~~**Scan**~~ | — | **PINDAH KELUAR (7 Sep 2026 malam, §5.16)** dari Zevanic House ke menu top-level baru **Scan & Cetak > Scan Stok** — lihat bagian "📷 Scan & Cetak" di bawah untuk detail Scan Opname/Scan Persiapan (logic TIDAK berubah, cuma lokasi menu) | — | lihat Scan & Cetak |
...
> `/design-terapkan-handoff` "Pesanan dan Transaksi")** — dari 5 sub-menu
> LAMA (Penjualan Kasir, Menunggu Proses [CRUD manual], Proses Persiapan,
> Proses Produksi, Proses Pengiriman) jadi **4 sub-menu BARU**: Penjualan
> Kasir (rombak alur checkout), Menunggu Proses (rombak total jadi murni
> antrian QO, CRUD manual DIHAPUS), Daftar Pesanan (BARU, gabungan 3 menu
> ringkasan lama), Transaksi Keuangan (BARU, piutang & kas). Grup sidebar
> `navgrp-pesanan`, tab `tab-pesanan` (BUKAN nested di Zevanic House).
> Semua menu-id akses ada di `DAFTAR_MENU` (`vue-config-akses.js`),
> kategori `"Pesanan"`. **Latar belakang & keputusan arsitektur lengkap
> (7 keputusan Guru D1-D7 via AskUserQuestion + 4 tambahan teknis flagged
> T1-T4, termasuk 1 temuan arsitektur yang mengoreksi jawaban awal Guru
> soal `status_grouping`)**: lihat komentar besar di atas `js/vue-
> pesanan.js`, dan `STATUS-PROYEK.md` §5.15. **Ini LONCAT dari urutan
> `RENCANA-REKONSTRUKSI-2026-09.md` §6** (piutang Pesanan tadinya
> dijadwalkan langkah 15, dikerjakan sekarang atas permintaan eksplisit
> Guru — bukan kesalahan urutan, tapi langkah 5-14 lain TETAP belum
> selesai).
>
> **⚠️ BELUM DIUJI di browser+Firebase sungguhan sama sekali** — modul
> ini menyentuh UANG (kasir, piutang, pembayaran). `firestore.rules`
> koleksi BARU `piutang_pembayaran` **BELUM dipublish — blocker keras**
> (tanpa ini fitur piutang `permission-denied` total). `transaksi_kasir`/
> `pengaturan_id_transaksi_kasir` rules-nya SUDAH ADA (klaim lama bahwa
> ini "belum dipublish" sudah basi, dikonfirmasi ke
> `claude/FIRESTORE-RULES-SNAPSHOT.md`).

| Menu | File Utama | Sub-menu di dalamnya | Role | Status |
|---|---|---|---|---|
| **Penjualan Kasir** | `vue-pesanan.js` (`PesananKasirManager`) | POS 2 langkah (keranjang → pembayaran): tab kategori produk (dari `jenis_produk`) + grid produk (harga dari `master_produk.harga_jual`) + panel keranjang (qty stepper, WAJIB pilih pelanggan dari `master_pelanggan`, metode Tunai/Transfer/QRIS, status bayar Lunas/DP/Tempo — Cicilan BUKAN opsi di sini, lihat Transaksi Keuangan). "Buat Order" → guard harga pending (`bahanTerblokirDiKeranjang()`, §5.14) → guard limit piutang BARU (`piutangTerblokir()`, §5.15, blokir TOTAL kalau `saldo_piutang+sisa > limit_piutang`) → 1 dokumen `transaksi_kasir` (field baru `pelanggan_id`/`status_bayar`/`dp_persen`/`total_dibayar`/`sisa_piutang`/`jatuh_tempo`) + N dokumen `order_spk` OTOMATIS (1 per item, field snapshot BARU `pelanggan_id`/`transaksi_kasir_id`/`no_transaksi`/`status_bayar`) + 1 dokumen `piutang_pembayaran` (kalau ada piutang) — mengalir ke pipeline Persiapan Produksi V2 tanpa hambatan. Struk cetak MVP `window.print()` browser (BUKAN integrasi printer thermal POS, disederhanakan & ditandai di kode, T4) | `pesanan_kasir` | ⚠️ Kode ditulis, `node --check` lolos, BELUM DIUJI SAMA SEKALI — modul UANG |
| **Menunggu Proses** | `vue-pesanan.js` (`PesananMenungguManager`) | **GANTI TOTAL (§5.15) — form CRUD manual (yang tadinya disalin dari `OrderSpkManager` lama) DIHAPUS**, sekarang murni antrian QO. Kelompokkan `order_spk` yang `status==='Aktif'` DAN belum diproses (`qo_diproses!==true`) per transaksi. Opsi QO per baris = 4 kelipatan pertama yang >= RO (`opsiQO()`, keputusan Guru D5). Akses digerbang GANDA: `cekIzinMenu` DAN `tierOwnerKeAtas()` (non-Owner-tier ditolak eksplisit, merujuk `PEDOMAN-SERAH-TERIMA.md` aturan #8). Tombol "Proses N orderan" SELALU minta `PopupPin` walau user login sudah Owner (keputusan Guru D4). Aksi HANYA update `qty_order`+`qo_diproses`+`qo_diproses_pada`+`qo_oleh` — **TIDAK PERNAH menyentuh `status_grouping`** (temuan arsitektur, field itu eksklusif milik Persiapan Produksi > "Perlu Disiapkan", lihat `STATUS-PROYEK.md` §5.15 poin 4) | `pesanan_menunggu` | ⚠️ Kode ditulis, `node --check` lolos, BELUM DIUJI SAMA SEKALI |
| **Daftar Pesanan** | `vue-pesanan.js` (`PesananDaftarManager`, BARU TOTAL) | **GANTI 3 menu ringkasan lama** (Proses Persiapan/Produksi/Pengiriman, `RingkasanSpkTrackManager`) **jadi 1 menu**. Baca semua `order_spk`+`transaksi_kasir` (filter `pelanggan_id` terisi)+`spk_track` sekaligus, susun 6-kotak ringkasan (total + per-kartu-pelanggan), popup rincian anak-SPK (filterable), popup timeline (baca `spk_track.riwayat_scan`). **Penyederhanaan yang WAJIB diketahui (T3)**: begitu SPK masuk grouping Persiapan Produksi, 1 grouping bisa berisi campuran SPK dari BEBERAPA pelanggan — atribusi per-pelanggan presisi CUMA untuk status "Perlu Disiapkan" (sebelum grouping); 5 kolom jalur sesudahnya cuma hitungan "tersentuh grouping ini" berlabel PERKIRAAN, bukan hitungan per-pelanggan eksak — trade-off disengaja, bukan bug | `pesanan_daftar` (BARU) | ⚠️ Kode ditulis, `node --check` lolos, BELUM DIUJI SAMA SEKALI |
| **Transaksi Keuangan** | `vue-pesanan.js` (`PesananTransaksiManager`, BARU TOTAL) | 3 tab internal: **Kas Besar** (agregasi per pelanggan: nilai/sudah dibayar/sisa/status jatuh tempo, tombol "Catat pembayaran" popup — KELIHATAN cuma untuk Owner-tier tapi TETAP WAJIB `PopupPin` saat diklik, keputusan Guru D4, panggil `catatPembayaranSusulan()` yang SATU-SATUNYA titik mengurangi `master_pelanggan.saldo_piutang`), **Rincian Transaksi** (list flat `piutang_pembayaran`, read-only, total per metode), **Rincian Piutang** (`transaksi_kasir` yang `sisa_piutang>0`, diurutkan jatuh tempo). Cicilan = status TURUNAN dari `limit_piutang` (keputusan Guru D6), BUKAN jadwal cicilan baru | `pesanan_transaksi` (BARU) | ⚠️ Kode ditulis, `node --check` lolos, BELUM DIUJI SAMA SEKALI — **BLOCKED total sampai rule `piutang_pembayaran` dipublish** |
...
>
> **CATATAN (7 Sep 2026 malam, §5.15)**: TIDAK ADA yang berubah di grup
> menu ini dari rekonstruksi Pesanan — perubahan §5.15 murni di grup
> "🛒 Pesanan" di atas (field baru `order_spk`, koleksi baru
> `piutang_pembayaran`). `status_grouping`/`qty_tergrouping`/
> `grouping_ids` dikonfirmasi ULANG tetap eksklusif milik "Perlu
> Disiapkan" di bawah.
...
## 📷 Scan & Cetak (grup top-level BARU, sejajar Zevanic House / Pesanan / Persiapan Produksi)

> **BARU (7 Sep 2026 malam, §5.16, `/design-terapkan-handoff` "05 - Scan
> dan Cetak", atas instruksi Guru "kerjakan terapkan dan seluruh
> turunannya")**. Grup sidebar sendiri `navgrp-scancetak`, 1 tab
> `tab-scan-cetak`, 4 sub-tab. **INI BARU TAHAP 1 (fondasi + menu)** —
> file `js/vue-scan-cetak.js` (BARU, genuinely diimpor, lihat
> `STATUS-PROYEK.md` §5.16 untuk detail lengkap) berisi `PopupPinGenerik`
> & `ScanGenerik` yang jadi FONDASI dipakai bersama oleh refactor
> Persiapan Produksi (Bahan/Acc Sewing/Webbing/Finishing) dan 5 modul
> Proses Produksi baru total (Cutting/Sewing/Finishing/Serie/Gudang
> Barang Jadi) — **KELIMANYA BELUM DIBANGUN**, menyusul di sesi
> berikutnya sesuai urutan: refactor Persiapan Produksi → Cutting →
> Sewing → Finishing → Serie → Gudang Barang Jadi → Cetak Label Produk.

| Menu | File Utama | Sub-menu di dalamnya | Role | Status |
|---|---|---|---|---|
| **Scan Stok** | `vue-scan-opname.js` + `vue-scan-persiapan.js` (DIPINDAH dari Zevanic House > Scan, logic TIDAK berubah) | **Scan Opname** + **Scan Persiapan** — lihat detail lengkap di `STATUS-PROYEK.md` §5.14 (belum berubah dari situ, cuma lokasi menu) | `scan_opname` / `scan_persiapan` (id sama, kategori pindah) | ✅ Aktif (logic tidak disentuh, cuma dipindah) |
| **Referensi Scan** | Statis di `index.html` (tidak ada file Vue) | Katalog baca-saja jenis-jenis scan per modul (Persiapan Produksi 6 generik, Cutting/Serie/Sewing/Finishing/Gudang Barang Jadi — ringkasan dari SERAH-TERIMA masing-masing). **BUKAN titik scan sungguhan** — scan dilakukan di layar pos masing-masing | `scan_cetak_referensi` (BARU) | ✅ Aktif (murni dokumentasi) |
| **Cetak** | Statis di `index.html` (tidak ada file Vue) | Indeks label/lembar cetak yang sudah ada (Label SPK Grouping/Kode Bagging/Lembar Kode Tugas/Label Bahan/Cetak Ulang — semua tombolnya di layar masing-masing, bukan di sini) + baris **Label Pcs / Cetak Label Produk** ditandai **Belum Dibangun** (menyusul setelah modul Sewing — label baru, beda dari label per-pcs Sewing lama, QR wajib) | `scan_cetak_cetak` (BARU) | ⚠️ Sebagian besar cuma indeks (aktif); Cetak Label Produk BELUM DIBANGUN |
| **PIN** | `js/vue-scan-cetak.js` (`AppScanCetakRiwayatPin`, DIPINDAH dari Zevanic House > Config, kode identik) | Riwayat PIN (baca `riwayat_pin`, read-only) — **PERTAMA KALI ada penulisnya** (`PopupPinGenerik` di file yang sama), TAPI cuma mencatat pemakaian PIN dari kode BARU (mulai §5.16) — `PopupPin` versi lama di modul lain (Pesanan/Stock & Pembelian/dst) BELUM ikut migrasi, jadi tabel ini masih akan tampak sepi untuk sementara | `scan_cetak_pin` (BARU, id lama `config_master_data` TIDAK dipakai lagi untuk screen ini) | ⚠️ Kode ditulis, `node --check` lolos, BELUM DIUJI BROWSER — akan tampil kosong sampai ada kode baru (Cutting dst) yang memanggil `PopupPinGenerik` |
...
| **Navigasi antar-tab** | `dashboard.js` | `pindahTab`/`pindahSubTab`, peta mount tiap layar (`petaMount`), DAN `petaTabIndukPerGrup` (map TERPISAH, keyed by sub-tab-group-prefix, dipakai breadcrumb/header-context — BUKAN mount). **BARU (1 Sep 2026)**: 15 entry lama jalur Sewing/Webbing/Finishing (`sub-pp-sewing-perludiproses` dkk → `pastikanMountPpSewingPerluDiproses` dkk) DIGANTI TOTAL ke id & nama fungsi baru (mis. `sub-pp-sewing-perludiproses` → `pastikanMountPpSewingPerluDisiapkan`), pola sama dengan 5 entry Bahan yang sudah diganti 31 Agt 2026. Entry jalur `pp_vendor` TIDAK ikut disentuh, tetap `-perludiproses`/`-sedangdiproses`. **BARU LAGI (5 Sep 2026, §5.12)**: `petaTabIndukPerGrup` ditambah `'sub-zh-suplayer': 'tab-zevanic-house'`; `petaMount` ditambah 3 entry Master Suplayer (`sub-zh-suplayer-entry`/`-alias-moq`/`-petakan` → `pastikanMountSuplayerEntry`/`SuplayerAliasMoq`/`SuplayerPetakan`), diganti 1 entry Config (`sub-zh-config-suplayer`→`sub-zh-config-tlc`), dihapus 1 entry (`sub-zh-stock-alias`). Array `tabs` (dipakai riwayat tombol kembali HP) termasuk `'tab-menu-lengkap'`/`'tab-atur-favorit'`/`'tab-pesanan'`. **BARU LAGI (7 Sep 2026 malam, §5.14)**: entry `sub-zh-stock-listorder` (List Order Belanja) DIHAPUS dari `petaMount`; entry mount Rak Penyimpanan DIPINDAH dari `sub-zevanic-house-databahan-rak` (lama) ke `sub-zh-stock-rak` (baru, di grup Stock & Pembelian). **BARU LAGI (7 Sep 2026 malam, §5.15)**: `petaMount` untuk grup Pesanan diganti total — 3 entry lama (`sub-pesanan-persiapan`/`-produksi`/`-pengiriman` → `pastikanMountPesananPersiapan`/`Produksi`/`Pengiriman`) DIHAPUS, ditambah `sub-pesanan-daftar` → `pastikanMountPesananDaftar` dan `sub-pesanan-transaksi` → `pastikanMountPesananTransaksi`; `sub-pesanan-kasir`/`sub-pesanan-menunggu` (id & nama fungsi mount) TETAP SAMA |
...
| `js/vue-master-pelanggan.js` | **BARU (5 Sep 2026)** — Master Pelanggan, lihat baris "Master Pelanggan" di tabel Zevanic House di atas. **SELESAI PENUH, SUDAH DITES browser (7 Sep 2026)** (bagian yang MENULIS `saldo_piutang` dari modul Pesanan §5.15 belum ikut ditest) |
...
| `js/vue-pesanan.js` | **DITULIS ULANG TOTAL (7 Sep 2026 malam, §5.15)** — 4 sub-menu Pesanan dan Transaksi (Kasir/Menunggu Proses/Daftar Pesanan/Transaksi Keuangan), lihat tabel "🛒 Pesanan" di atas untuk isi lengkapnya. Salinan KE-5 fungsi PIN (`hashPin`/`cariUserByPin`/`tierOwnerKeAtas`) dari `vue-stock-pembelian.js`, salinan `ambilDaftarPelanggan()` dari `vue-master-pelanggan.js` — pola "salin, jangan impor silang" |
