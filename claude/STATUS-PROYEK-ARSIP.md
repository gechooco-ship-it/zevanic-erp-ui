...
## §44 — Redesain Mobile "Gechoo Mobile Organic" — Fondasi (28 Agt 2026)

**Konteks**: Guru mengunggah folder mockup high-fidelity (`F:\ZEVANIC HOUSE\
FOUNDATION\Mockup\design_handoff_gechoo_mobile\`, README.md + github.md +
screenshot 12 layar) dan minta diimplementasikan segera. Sebelum mulai,
ditanyakan 4 pertanyaan pembuka (AskUserQuestion) — jawaban Guru:
1. Scope round ini: **Fondasi + 2-3 modul contoh** (bukan semua 35 modul).
2. Persiapan Produksi **ikut** direstyle pola generik yang sama.
3. Dark mode **dikerjakan sekarang**, bukan ditunda.
4. Drawer Profile lama **dihapus total**, ganti bersih (bukan disimpan
   sebagai fallback).

Di tengah pengerjaan, 2 keputusan tambahan ditanyakan & dijawab Guru:
5. Pengumuman (dulu carousel di Beranda) → **pindah jadi notifikasi
   lonceng** di header (bukan dihapus begitu saja).
6. "1 grup default" di Beranda (README bilang cuma 1 grup tampil,
   sisanya di layar Menu Lengkap) → **user pilih sendiri lewat gear** di
   layar Atur Favorit, BUKAN aturan otomatis sistem.

**⚠️ SCOPE ROUND INI (SELESAI)**: fondasi CSS/tema gelap, sistem
navigasi (ternyata SUDAH lengkap dari §39, tidak perlu dibangun ulang),
Beranda, Menu Lengkap (baru), Atur Favorit (baru), Bottom Sheet Profil
(baru, ganti drawer), sentuhan gaya Absensi (kamera) & Scan QR, dan foto
avatar + menu titik-tiga di Antrean Absensi & Antrean Dakar (permintaan
terpisah Guru di tengah sesi ini, bukan bagian mockup).

**⚠️ BELUM DIKERJAKAN round ini (menyusul)**: restyle penuh Persiapan
Produksi & Order SPK ke pola Kartu-baris/Form-bertahap baru (Task
tersendiri — README section "Daftar modul"/"Form bertahap" belum
diterapkan ke modul manapun), restyle visual Profil Lengkap & 4 sub-layar
(vue-account-profile.js) — SAAT INI baru Bottom Sheet-nya yang baru,
isi tab Account/Data Karyawan/Absensi/Reimburse/Pencapaian/Keamanan
MASIH tampilan lama (fungsional 100% sama, belum di-restyle visual), dan
sisa 32 modul lain (pola "Daftar modul"/"Form bertahap" generik) —
menunggu Guru review pola di 2-3 modul contoh dulu sebelum diterapkan
massal.

### Keputusan sepihak (PERLU REVIEW GURU)

1. **Beranda dikunci pas 1 layar, HANYA 1 grup kategori default** (bukan
   semua grup + scroll panjang seperti sebelumnya) — PERSIS ikut mockup.
   Sisa modul lain pindah ke layar baru "Menu Lengkap" (tombol "Lihat
   Semua Menu (N)"). Ini mengurangi APA YANG LANGSUNG TERLIHAT di
   Beranda dibanding versi sebelumnya (dulu semua grup langsung
   kelihatan) — sesuai instruksi eksplisit mockup "pakai apa adanya" &
   dikonfirmasi lewat pertanyaan #6 di atas.
2. **Grup default & jumlah kartu per grup di Beranda SEKARANG per-user**
   (field baru `users/{email}.beranda_grup`, `.beranda_grup_urutan`,
   `.beranda_batas_kartu`), diatur lewat gear di layar Atur Favorit.
   README mockup aslinya taruh pengaturan ini di dokumen GLOBAL
   `pengaturan_sistem/urutan_menu_home` (Owner-only) — SENGAJA DIBUAT
   PER-USER karena jawaban eksplisit Guru "user bisa memilih sendiri".
   Dokumen `urutan_menu_home` (existing, Owner) TIDAK disentuh sama
   sekali, tetap fungsi lama (urutan menu per kategori & urutan
   kategori untuk daftarMenuGroups()).
3. **Mode "Atur" inline (toggle bintang langsung di grid Beranda)
   DIHAPUS**, ganti tombol "Atur Favorit" yang navigasi ke layar baru.
   Data Firestore (`users/{email}.menu_favorit`) TIDAK berubah, cuma
   UI pemilihnya pindah — tidak perlu migrasi data.
4. **Kartu Statistik** (Hari kerja/Kehadiran/Peringkat) di Beranda
   SENGAJA ditampilkan "–" (bukan angka contoh dummy dari mockup: 14/
   98%/#2) — menghitungnya perlu query agregat baru (terutama
   "Peringkat" = lintas SEMUA karyawan, mahal & butuh definisi bisnis
   yang belum dikonfirmasi: peringkat berdasarkan apa, per periode apa,
   per gudang atau seluruh perusahaan). **Perlu keputusan Guru sebelum
   dikerjakan** — bukan sekadar tebak-tebak, sesuai pedoman kerja
   proyek ini.
5. **Banner motivasi** di Beranda (bagian bawah, gradien maroon) —
   kontennya STATIS/placeholder ("Kerja rapi, hasil maksimal... Cek
   panduan kerja & SOP terbaru") karena tidak ada sumber CMS untuk teks
   ini. Tidak ada tombol aksi terpasang (murni dekoratif dulu). Perlu
   konten asli dari Guru kalau mau dipakai sungguhan.
6. **Bottom Sheet Profil punya 6 tautan (bukan 5 seperti mockup)** —
   "Estimasi Gaji" ditambahkan supaya tidak hilang aksesnya dari mobile
   (drawer lama py 6 tautan, salah satunya "Estimasi Gaji" yang
   ditambahkan 28 Agt karena SATU-SATUNYA jalan mobile ke situ waktu
   itu). Menghapusnya demi persis 5 tautan mockup akan jadi REGRESI
   (fitur jadi tidak bisa dijangkau sama sekali dari HP).
7. **Statistik/foto avatar di Antrean Absensi**: avatar ringkasan kartu
   sekarang utamakan `foto_selfie_keluar` (Clock Out) sebelum
   `foto_selfie_masuk` (Clock In) — "foto terakhir" secara kronologis.
   Sebelumnya SELALU foto Clock In duluan walau Clock Out lebih baru.
8. **Antrean Dakar**: foto KTP-nya SUDAH bisa diklik dari sebelumnya
   (tidak ada perubahan fungsi) — cuma gaya bingkai foto disamakan
   dengan kartu lain. Tombol aksinya cuma 2 (Setujui/Assign Ulang +
   Tolak) — TIDAK diringkas ke menu titik-tiga (belum "banyak pilihan").

### Layar & komponen baru

- **`js/vue-menu-lengkap.js`** (BARU) — "Menu Lengkap": semua modul dari
  `daftarMenuGroups()`, bisa dicari (client-side, contains, case-
  insensitive — BUKAN prefix-match peka huruf besar/kecil seperti pola
  "Daftar modul", karena itu batasan Firestore prefix-query untuk data
  SERVER, sedangkan di sini murni filter array 35 modul yang sudah ada
  di memori). Tab `tab-menu-lengkap`.
- **`js/vue-atur-favorit.js`** (BARU) — "Atur Favorit": (1) pemilih
  kartu favorit maks 4 (cari + sakelar per modul, field
  `users/{email}.menu_favorit`, SAMA PERSIS logic lama, cuma UI-nya
  baru), (2) pemilih grup+jumlah kartu Beranda (lihat keputusan sepihak
  #2 di atas). Tab `tab-atur-favorit`.
- **`js/vue-sheet-profil.js`** (BARU) — Bottom Sheet Profil, GANTI TOTAL
  `js/vue-profile-drawer.js` (FILE DIHAPUS, sesuai jawaban Guru "hapus
  total, ganti bersih"). Naik dari bawah saat tombol Profil di nav bawah
  diketuk. Isi: kartu QR gradien, 3 aksi cepat (Clock in/out, Scan QR,
  Mode gelap — ketiganya fungsional penuh, bukan dekorasi), 6 tautan
  (lihat keputusan #6), Keluar.
- **`js/vue-home.js`** (DIROMBAK TOTAL) — lihat keputusan sepihak #1-5.
  Semua fungsi Firestore lama (shift, clock in/out, favorit, urutan
  menu) DIPERTAHANKAN — cuma tampilan & apa-yang-ditampilkan yang
  berubah.
- **`js/vue-header-mobile.js`** (mode `'home'` dirombak, mode `'lainnya'`
  TIDAK disentuh) — quote harian inline (ganti QuoteCard terpisah,
  QuoteCard sendiri MASIH ada di vue-components.js buat desktop), lonceng
  notifikasi Pengumuman + badge (baca koleksi sama seperti
  PengumumanCarousel, badge dari localStorage "terakhir dilihat", TIDAK
  ADA tulis Firestore tambahan), avatar inisial. Mode baru
  `'tersembunyi'` untuk layar yang sudah punya header sendiri (Menu
  Lengkap/Atur Favorit) — supaya tidak dobel header.
- **`js/vue-components.js`** (tambahan, TIDAK ADA yang dihapus/diubah) —
  4 komponen bersama baru: `KartuMenu` (kartu 1 menu, dipakai Beranda +
  Menu Lengkap), `AksesTerbatasDialog` (ganti alert() polos buat modul
  terkunci), `HeaderLayar` (tombol kembali+kicker+judul, pola dipakai di
  hampir semua layar baru ke depan), `KolomCari` (kolom cari pil
  v-model, dipakai Menu Lengkap & Atur Favorit).
- **`css/gechoo-design.css`** — token mode gelap + kelas `.gc-*` baru
  (kartu gradien, pil status, bottom sheet, dialog tengah, bar langkah,
  pil pilihan, placeholder kosong) — SEMUA sudah diverifikasi terpakai
  di file ini.
- **`js/dashboard.js`** (v17→v18) — array `tabs` di `window.pindahTab`
  tambah `'tab-menu-lengkap'` & `'tab-atur-favorit'`.
- **`js/app.js`** (v2→v3) — tombol Profil nav bawah sekarang buka Bottom
  Sheet (`window.bukaSheetProfil`, ganti `window.bukaProfileDrawer`);
  jaring pengaman `pindahLayar()` ikut diperbarui
  (`window.tutupSheetProfil`).
- **`js/vue-camera.js`, `js/vue-scan-qr.js`, `js/vue-absensi-qr.js`** —
  sentuhan gaya SAJA (bingkai kamera 3:4 + `--lens`, garis scan animasi,
  animasi gxPop di layar sukses) — TIDAK ADA logic kamera/Firestore yang
  disentuh.
- **`js/vue-antrean-absensi.js`** (v3→v4), **`js/vue-antrean-dakar.js`**
  (v2→v3) — lihat keputusan sepihak #7-8.
- **`js/vue-account-profile.js`** (v2→v3) — HANYA komentar diperbarui
  (drawer→sheet), TIDAK ADA perubahan fungsional/visual.
- **`index.html`** — mount + tab-content 2 layar baru, mount Bottom
  Sheet (ganti drawer), cache-bust semua file di atas.

### Verifikasi

`node --check` (mode ES-module, copy `.mjs`) lolos di **SEMUA** file
yang disentuh/dibuat round ini (16 file `.js`). Keseimbangan tag HTML
index.html: `<div>` 168/168, `<button>` 73/73, `<script>` 49/49,
`<nav>` 2/2, `<main>`/`<header>` 1/1. Brace CSS `gechoo-design.css`
224/224. Cross-reference manual (grep): semua `id="vue-xxx"` mount di
index.html punya `getElementById('vue-xxx')` yang cocok di file JS
barunya; tidak ada sisa referensi `window.bukaProfileDrawer`/
`tutupProfileDrawer` yang belum diperbarui.

**Belum diuji manual di browser sama sekali** — murni verifikasi
statis (syntax + tag balance + cross-reference), BUKAN jaminan tidak
ada bug visual/interaksi. Guru WAJIB coba manual di HP sungguhan
sebelum dipakai karyawan:
1. Buka Beranda → cek 1 layar penuh tanpa perlu scroll (di HP Guru,
   ukuran layar bisa beda dari asumsi desain).
2. Ketuk lonceng → cek dropdown Pengumuman muncul & badge hilang
   setelah diketuk.
3. Ketuk "Lihat Semua Menu" → Menu Lengkap → coba cari beberapa modul.
4. Ketuk "Atur Favorit" → coba pilih/lepas favorit & grup Beranda → cek
   balik ke Beranda, perubahan kepakai.
5. Ketuk Profil di nav bawah → cek Bottom Sheet naik dari bawah (BUKAN
   drawer dari kanan lagi) → coba semua 3 aksi cepat + 6 tautan +
   Keluar.
6. Coba Mode Gelap dari Bottom Sheet → cek SEMUA layar (termasuk yang
   BELUM disentuh round ini) tetap terbaca (kontras aman), bukan cuma
   layar baru.
7. Cek Antrean Absensi & Antrean Dakar tetap berfungsi normal (Accept/
   Reject/Setujui/Tolak) — kartu Absensi sekarang py menu titik-tiga
   buat Hapus, pastikan masih bisa dihapus dari situ.
8. (Regresi) Login/logout, Clock In/Out dari Home & dari Bottom Sheet,
   semua modul lama yang TIDAK disentuh round ini tetap jalan normal.

**Menyusul (round berikutnya, urutan disarankan)**:
- Restyle visual isi Profil Lengkap & 4 sub-layar (masih tampilan lama).
- Terapkan pola "Daftar modul"/"Form bertahap" ke sisa 32 modul —
  MENUNGGU Guru review & approve pola di 2 modul contoh (lihat §44.1)
  dulu.
- Keputusan Guru soal Kartu Statistik Beranda (lihat keputusan #4).

## §44.1 — Persiapan Produksi + Order SPK: modul contoh #1 & #2 (28 Agt 2026)

Lanjutan langsung §44 — menerapkan pola visual "Gechoo Mobile Organic"
(kartu-baris, kolom cari pil, empty state `.gc-kosong`) ke 2 modul
contoh yang disepakati Guru (jawaban "Ikutkan juga, pola generik yang
sama"), walau kedua modul ini dibuat SEBELUM mockup ada. **Restyle
VISUAL/TEMPLATE SAJA — nol perubahan logic**, termasuk logic
BOM-explosion, Approve-sekali-jalan, cek No. SPK dobel, sambungan SKU,
dan cetak label QR. Semua sudah dites `node --check` + cross-reference
token CSS, TAPI (sama seperti §44) belum dites manual di browser.

**File diubah**:
- **`js/vue-persiapan-produksi.js`** (v1→v2) — `PersiapanQueueManager`
  (kartu "Perlu Disiapkan"): kolom cari jadi pil, empty state jadi
  `.gc-kosong`, kartu antrean jadi meta-row berikon (qty+SKU) dengan
  area aksi dipisah border-top, tombol Approve jadi pil penuh.
  `PersiapanKomponenListManager` (4 tab Bahan/Sewing/Webbing/Finishing,
  1 komponen dipakai 4x lewat prop `tipe`): kolom cari pil, empty state
  `.gc-kosong`, kartu progress-bar+checklist dipertahankan strukturnya.
  Ditemukan & diperbaiki 1 bug saat restyle: salah pakai token CSS
  `var(--bg-dim)` (label deskriptif dari README mockup, BUKAN nama
  variabel asli) — sudah diganti `var(--ivory-dim)` (token asli di
  `gechoo-design.css`), dicek ulang tidak ada sisa di file manapun yang
  disentuh round ini.
- **`js/vue-order-spk.js`** (v7→v8) — form tambah/edit: ikon judul jadi
  `var(--aksen-ink)` (konsisten dgn modul pasangannya). Kartu daftar
  SPK (sudah dari sebelumnya berbentuk kartu, BUKAN tabel — lihat
  komentar "GANTI (28 Agt 2026)" di file itu sendiri, perubahan
  terpisah dari redesain ini): kolom cari jadi pil, empty state jadi
  `.gc-kosong`, `border-radius` kartu disamakan 20px (sebelumnya ikut
  default `var(--radius)` 16px — beda tipis dari modul pasangannya).
  Fitur cetak label QR (checkbox pilih-banyak, popup pratinjau,
  sambungan SKU ke Master Produk) TIDAK disentuh sama sekali.
- **`index.html`** — cache-bust kedua file di atas + komentar.

**Sengaja TIDAK diubah** (di luar scope "restyle visual"): struktur
paginasi cursor-based (`usePaginasiFirestore`, tombol Sebelumnya/
Berikutnya) di kedua modul TETAP model lama ("halaman", bukan "load
more 12 lagi" ala mockup) — sama seperti keputusan §44 soal komposabel
ini, dipakai banyak modul lain, ganti model butuh diskusi terpisah.

**Verifikasi**: `node --check` lolos kedua file. Tag HTML index.html
tetap seimbang (`<div>` 168/168, `<script>` 49/49, dst — tidak berubah
dari §44 karena cuma ganti versi query string). Cross-reference token
CSS ulang ke SEMUA file yang disentuh §44+§44.1 (13 file): 0 token
tidak terdefinisi.

**Uji manual tambahan buat Guru** (di luar 8 poin §44):
9. Buka Persiapan Produksi tab "Perlu Disiapkan" → cek kartu SPK
   tampil rapi, coba Approve 1 SPK yang sudah tersambung SKU → cek 4
   tab lain (Bahan/Sewing/Webbing/Finishing) muncul kartu sesuai isi
   BOM produk itu.
10. Buka Order SPK → tambah 1 SPK baru → cek otomatis masuk antrean
    "Perlu Disiapkan" di atas → coba cetak label (kalau ada izin print).

## §44.2 — Profil Lengkap: sentuhan gaya RINGAN saja, Task #20 ditutup (28 Agt 2026)

Guru bilang "oke, gas" setelah round §44.1 — lanjut ke sisa Task #20
("Bottom Sheet Profil SELESAI + restyle visual Profil Lengkap & sub-
layar BELUM"). **Keputusan sepihak**: TIDAK melakukan restyle penuh
("Daftar modul"/"Form bertahap") ke `js/vue-account-profile.js` round
ini — file ini 938 baris, isinya bukan daftar+form sederhana seperti 2
modul contoh (§44.1), tapi 6 sub-tab campuran (form self-edit data
diri, form pengajuan Izin/Cuti/Lembur yang nyambung ke alur kamera
vanilla, filter+riwayat+banding, PIN/password, dst) — restyle penuh di
sini PERSIS yang sudah ditandai "extremely high-risk" & didahulukan
ditunda di §44. Menerapkan pola dalam sebelum Guru sempat coba & approve
pola dari 2 modul contoh juga melanggar urutan proses yang sudah
disepakati sendiri (STATUS-PROYEK §44 "Menyusul").

**Yang DIKERJAKAN** (murni kosmetik, 0 perubahan logic): di tab
Absensi → daftar Riwayat Absensi (satu-satunya "daftar kartu"
sungguhan di file ini, mirip pola kartu-baris modul lain) — empty
state diganti pola `.gc-kosong`/`.lingkaran` (sebelumnya kotak dashed
custom), border-radius kartu riwayat disamakan 20px (konsisten dengan
Persiapan Produksi/Order SPK). Form Data Karyawan, form Izin/Cuti/
Lembur, filter laporan, tab Keamanan (Password/PIN), tab Pencapaian/
Estimasi Gaji — **SEMUA TETAP tampilan lama**, tidak disentuh.

**File**: `js/vue-account-profile.js` (v3→v4, cuma komentar di v3
sebelumnya, sekarang isi beneran berubah dikit). `index.html`
cache-bust + komentar.

**Verifikasi**: `node --check` lolos. Cross-reference token CSS ulang
ke SEMUA 13 file yang disentuh §44+§44.1+§44.2: 0 token tidak
terdefinisi. Tag HTML index.html tetap seimbang.

**Task #20 SEKARANG DITUTUP** dengan status ini (sentuhan ringan,
BUKAN restyle penuh) — kalau Guru mau Profil Lengkap ikut pola penuh
juga, itu masuk antrean bareng sisa ~32 modul lain, MENUNGGU review
pola dari 2 modul contoh dulu (poin "Menyusul" §44 belum berubah).

**Uji manual tambahan buat Guru**:
11. Buka Profil → tab Absensi → scroll ke Riwayat Absensi → kalau
    belum pernah absen di akun itu, cek tampilan "belum ada riwayat"
    (lingkaran ikon + teks) — kalau sudah ada riwayat, cek kartu-kartu
    tetap tampil normal (cuma sudut lebih bulat dari sebelumnya).

## §44.3 — Ivory lebih muda (celah dari paket handoff mockup, 29 Agt 2026)

Guru tanya: "kemarin saya minta ada perubahan komposisi warna" — dicek
ke SELURUH transkrip sesi ini (dari 27 Agt 2026) & semua dokumen
proyek, **TIDAK ADA catatan permintaan ini di sesi ini**. Guru lalu
mengirim potongan percakapan dari sesi LAIN (waktu menyiapkan paket
handoff mockup) yang mengungkap: Guru sempat minta "ivorynya bikin
lebih muda lagi biar clean" di sesi itu, dan disepakati 3 token
`gechoo-design.css` menyimpang dari repo untuk kebutuhan prototipe:

| Token | Repo (lama) | Prototipe/diminta |
|---|---|---|
| `--ivory` | `#FAF4E7` | `#FDFAF4` |
| `--ivory-dim` | `#F1E8D6` | `#F7F1E6` |
| `--line` | `#E9DDCE` | `#EFE7DA` |

**Root cause**: perubahan ini disepakati di paket handoff mockup, tapi
**TIDAK PERNAH ikut diterapkan** ke `gechoo-design.css` yang sungguhan
dipakai app — baik saat paket handoff dibuat, MAUPUN saat redesain
mobile "Gechoo Mobile Organic" diimplementasikan (§44, 28 Agt 2026).
Dicek langsung ke file sebelum menjawab (bukan tebak2) — token masih
nilai lama persis.

**Diperbaiki (29 Agt 2026)**: `css/gechoo-design.css`, blok `:root`
(bagian TERANG saja — blok `[data-theme="dark"]` TIDAK disentuh, nilai
gelapnya independen) — `--ivory`/`--ivory-dim`/`--line` diganti ke 3
nilai di atas, dikonfirmasi eksplisit ke Guru dulu (AskUserQuestion,
karena ini token DASAR dipakai di SELURUH app — bukan cuma layar
redesain mobile — efeknya global ke semua layar lama & baru, desktop &
mobile). Brace CSS diverifikasi tetap seimbang (224/224) setelah edit.
`PETA-DESAIN.md` tabel palet & catatan diperbarui sejalan.

**File yang berubah**: `css/gechoo-design.css` saja (1 file, 3 baris
nilai token). Tidak ada perubahan HTML/JS — efeknya otomatis menyebar
ke semua layar yang memakai `var(--ivory)`/`var(--ivory-dim)`/
`var(--line)` (praktis seluruh app).

**STATUS: BELUM DITES Guru di live** — perubahan sangat kecil secara
kode (cuma nilai hex token) tapi dampak visualnya menyeluruh, WAJIB
dicek sekilas di beberapa layar (termasuk yang lama, bukan cuma
redesain mobile) sebelum dianggap aman.

## §44.4 — Audit kepatuhan vs "instruksi" dari sesi handoff mockup (29 Agt 2026)

Guru menempel (paste) potongan sesi lain (sesi penyiapan paket handoff
mockup untuk developer eksternal) yang berisi 7 poin instruksi
build-order + "Aturan yang tidak boleh dilewat". Diminta cek —
dicocokkan LANGSUNG ke kode sungguhan (`grep`/`Read`, bukan tebak2) —
apakah implementasi §44/§44.1/§44.2 di sesi INI patuh. Hasil per poin:

**Poin 5 — "Aturan yang tidak boleh dilewat":**

1. ✅ **data-target + pindahSubTab(...,{catatRiwayat:true}) sampai
   child-tab** — PATUH. Dicek `index.html` — tombol menu Persiapan
   Produksi (baris 204) rantai 2x `pindahSubTab` turun sampai child-tab
   pertama; ke-5 tombol sub-tabnya sendiri (baris 607–611) semua punya
   `data-target` + `pindahSubTab(...,{catatRiwayat:true})`.
2. ℹ️ **vue-paginasi.js (cursor limit()+startAfter()), bukan potong
   array** — PATUH untuk yang backed Firestore: `vue-persiapan-
   produksi.js` pakai `usePaginasiFirestore` dari `vue-paginasi.js`.
   Menu Lengkap & Atur Favorit TIDAK pakai — tapi keduanya bukan daftar
   Firestore, isinya dari registry menu statis (`DAFTAR_MENU` di
   `vue-components.js`), jadi tidak ada yang perlu di-paginasi.
3. ℹ️ **getCountFromServer() untuk hitungan header** — dipakai di modul
   lama (`vue-hak-akses.js`). Untuk "Lihat Semua Menu ({{totalModul}})"
   di Beranda, sumbernya `semuaItemFlat.value.length` (client-side) —
   ini menghitung item di registry menu STATIS, bukan dokumen
   Firestore, jadi `getCountFromServer()` tidak relevan di sini (tidak
   ada koleksi untuk dihitung server-side).
4. ❌ **Pencarian prefix-match, peka huruf besar-kecil ("jangan cari di
   tengah kata")** — TIDAK PATUH, PENYIMPANGAN NYATA. `vue-menu-
   lengkap.js` baris 52–56 pakai
   `.toLowerCase().includes(kata)` — persis kebalikan dari yang
   diminta: case-INsensitive DAN cari di TENGAH kata (substring/
   contains), bukan prefix-match case-sensitive.
5. ✅ **cekIzinMenu(menuId,aksi) / wajibOwner hardcode owner** — PATUH.
   Menu Lengkap & Atur Favorit sama-sama pakai `daftarMenuGroups()`
   (`vue-components.js` baris 891-957) yang panggil
   `window.cekIzinMenu(m.id,'view')` per item DAN terapkan
   `wajibOwner` (Config Akses/Hak Akses/List Device Kiosk tetap
   terkunci non-owner) — logic sama persis dengan yang dipakai modul
   lama, bukan diduplikasi/disederhanakan.
6. ✅ **Mount-on-demand per sub-tab** — PATUH untuk cakupan sebenarnya
   (SUB-tab): semua sub-tab baru (Persiapan Produksi 5 child-tab, dll)
   terdaftar di `petaMount` (`dashboard.js` baris 366+) dengan fungsi
   `pastikanMountXxx()` on-demand. Menu Lengkap & Atur Favorit MOUNT
   EAGER (langsung saat modul JS load) — tapi keduanya itu TAB
   LEVEL ATAS (`tab-menu-lengkap`/`tab-atur-favorit`), bukan sub-tab,
   dan TIDAK ADA satupun tab level-atas di seluruh app (termasuk
   `vue-home.js` sendiri) yang mount-on-demand — jadi ini konsisten
   dengan arsitektur yang sudah ada dari dulu, bukan penyimpangan baru
   dari aturan yang secara eksplisit menyasar sub-tab.

**Poin 4 — urutan/detail pengerjaan:**

- ❌ **`vue-home.js` padding-bottom 102px wajib** (jejak nav + tombol
  Scan QR terangkat) — TIDAK ADA. Digrep `padding-bottom`/`102px` di
  `vue-home.js` → nol hasil. Container `<div id="vue-home">` di
  `index.html` baris 272 juga tidak pakai class `gc-safe-bottom`
  (yang ada nilainya beda, 90px, `gechoo-design.css` baris 318) atau
  padding manual apapun. **CELAH NYATA, belum diperbaiki — nunggu
  arahan Guru** (lihat di bawah).
- ✅ **petaMount registration Menu Lengkap/Atur Favorit** — tidak
  terdaftar di `petaMount`, tapi ini SESUAI ekspektasi (poin 6 di
  atas): keduanya tab level-atas, bukan sub-tab, jadi memang bukan
  cakupan `petaMount`.
- ℹ️ **Simpan pilihan ke `pengaturan_sistem/urutan_menu_home`** —
  SUDAH DIKETAHUI & DISEPAKATI berbeda: dipakai `users/{email}.
  beranda_grup`/`.beranda_grup_urutan`/`.beranda_batas_kartu`
  (per-user) sesuai jawaban eksplisit Guru saat itu ("user bisa
  memilih sendiri") — didokumentasikan di §44 sebagai "keputusan
  sepihak #2". Bukan penyimpangan yang lolos, tapi keputusan yang
  sudah dikonfirmasi — tidak diubah balik tanpa diminta.
- ℹ️ **Acuan pola "daftar + form bertahap" = Antrean Absensi** —
  SUDAH DIKETAHUI, dipakai Persiapan Produksi/Order SPK sebagai acuan
  alih-alih Antrean Absensi. Pola umumnya (daftar → form bertahap)
  tetap diikuti, cuma modul acuannya beda — tidak ada indikasi ini
  jadi masalah fungsional.
- ℹ️ **Bottom Sheet Profil "di vue-account-profile.js"** — SUDAH
  DIKETAHUI, dibuatkan file terpisah `vue-sheet-profil.js` alih-alih
  ditaruh di `vue-account-profile.js`. Fungsional sama, cuma lokasi
  file beda.

**Ringkasan — 2 celah NYATA yang belum diperbaiki (menunggu arahan
Guru, bukan diperbaiki sepihak, sesuai prinsip "jangan bikin tebak2"):**

1. `vue-home.js` — tidak ada padding-bottom 102px (atau sejenisnya) di
   container Beranda, berisiko konten ketutupan bottom nav + tombol
   Scan QR terangkat.
2. `vue-menu-lengkap.js` — pencarian pakai contains + case-insensitive,
   bukan prefix-match case-sensitive seperti yang diminta eksplisit.

Semua poin lain: patuh, tidak relevan (statis bukan Firestore), atau
sudah jadi keputusan yang dikonfirmasi eksplisit sebelumnya — tidak
diubah tanpa diminta.

## §44.5 — Pilot redesain kartu/tombol/header "Gechoo Mobile Organic" di Antrean Absensi (29 Agt 2026, DISETUJUI Guru)

**Konteks**: Guru minta lihat tampilan Antrean Absensi dibikin lebih
"kekinian dan simple" (tapi avatar tetap bisa zoom). Dikerjakan lewat
mockup HTML standalone (`antrean-absensi-clean.html`, dikirim via chat,
5 revisi: tambah ikon filter, foto Clock In/Out balik, tombol Accept
jadi 3 pilihan, contoh kartu Clock In+Out sama-sama pending, ikon SVG
inline karena FA gagal load di preview, kartu dipadatkan lagi,
perbandingan header sekarang-vs-usulan, sudut lebih bulat ala
"Organic") — DISETUJUI Guru ("oke gass saya setuju"), lalu diminta
"test dulu Antrean Absensi" dulu sebelum modul lain. Ringkasan pola
lengkap (moodboard) ditulis ke `PEDOMAN-GAYA-KERJA.md` supaya modul
berikutnya (Antrean Dakar/Lembur/Reimburse) tinggal contek, bukan
didesain ulang.

**File yang diubah**:
- `js/vue-antrean-absensi.js` — kartu `AntreanAbsensiCard` dirombak:
  blok Clock In/Clock Out (kotak besar+grid+dropdown+2 tombol) jadi 1
  baris "event" padat (foto 24px+label+jam+tag). Dropdown Seragam +
  Accept (2 langkah) GANTI 3 tombol sejajar **Sesuai/Tidak Sesuai/
  Reject** lewat fungsi baru `prosesDenganSeragam`/
  `prosesMasukDenganSeragam`/`prosesKeluarDenganSeragam` (cuma set
  `seragam*.value` lalu panggil `proses*` yang sudah ada — logic
  Firestore/validasi/cekIzinMenu TIDAK berubah). `AppAntreanAbsensi`:
  kolom cari hand-rolled GANTI komponen `KolomCari` (pil, dari
  `vue-components.js`, sudah dipakai Menu Lengkap/Atur Favorit);
  dropdown filter Owner (Jenis Pekerjaan/Gudang) yang tadinya selalu
  tampil sekarang di belakang 1 tombol ikon filter (toggle, titik merah
  kalau ada filter aktif) — logic filter-nya sendiri TIDAK berubah.
- `js/vue-header-mobile.js` — mode `'lainnya'` (dipakai SEMUA halaman
  selain Home — komponen ini GLOBAL) GANTI dari kartu pink+lingkaran
  dekoratif ke komponen `HeaderLayar` yang SUDAH ADA (dipakai Menu
  Lengkap/Atur Favorit — tombol kembali bulat ke Beranda + kicker nama
  tab + judul sub-tab). Karena komponennya dipakai bersama, perubahan
  ini otomatis berlaku ke SEMUA halaman sekaligus, bukan cuma Antrean
  Absensi — sudah dikonfirmasi & dicatat sebelum diterapkan (lihat
  AskUserQuestion "Terapkan semua ke Antrean Absensi dulu").
- `css/gechoo-design.css` — tambah class global baru `.approve-row` +
  `.appr-btn`(`.ok`/`.warn`/`.danger`) buat pola 3-tombol
  Sesuai/Tidak Sesuai/Reject, supaya modul lain bisa pakai ulang tanpa
  tulis CSS baru. `?v=1` → `?v=2` di `index.html`.
- `index.html` — cache-bust: `vue-antrean-absensi.js` `?v=4`→`?v=5`,
  `vue-header-mobile.js` `?v=4`→`?v=5`, `gechoo-design.css` `?v=1`→`?v=2`.

**Sengaja TIDAK diubah** (bukan bagian permintaan ini, dicatat biar
tidak dianggap lupa):
- Logic pencarian nama (`daftarPendingTersaring`) TETAP contains +
  case-insensitive (`.toLowerCase().includes(...)`) — BUKAN
  prefix-match case-sensitive. Ini penyimpangan yang SUDAH tercatat di
  §44.4 (audit kepatuhan) — mockup-nya sempat menulis hint teks "Cari
  awalan, peka huruf besar/kecil" tapi hint itu SENGAJA TIDAK
  diikutkan ke kode sungguhan supaya teks tidak berbohong soal
  perilaku aslinya. Kalau mau benar-benar prefix-match, itu keputusan
  terpisah yang perlu dikonfirmasi Guru dulu.
- Banner pink "Antrean validasi absensi" (di dalam `AppAntreanAbsensi`,
  BEDA dari header global `vue-header-mobile.js`) TIDAK disentuh —
  bukan bagian dari mockup yang direview Guru.
- Icon tetap Font Awesome (`fas fa-*`) sama seperti kode produksi lain
  — SVG inline di mockup standalone itu cuma akal-akalan karena preview
  mockup gagal load CDN FA, bukan keputusan buat kode sungguhan.

**Verifikasi**: `node --check` lolos utk `vue-antrean-absensi.js` &
`vue-header-mobile.js`; hitung `<div>`/`</div>`/`<template>`/
`</template>` seimbang di kedua file; brace CSS `gechoo-design.css`
230/230 (naik dari 224/224 sebelum §44.3, sesuai 6 baris aturan baru
yang ditambah).

**STATUS: header GLOBAL SUDAH DI-ACC Guru** ("Karena headernya global >
terapkan ke semua yah sudah cocok", 29 Agt 2026) — tidak ada aksi kode
lagi, sudah otomatis berlaku ke semua halaman karena `vue-header-
mobile.js` dipakai bersama. Guru juga bilang suka pilot Antrean
Absensi ini secara umum — daftar modul kandidat rollout berikutnya
ditulis di §44.6 di bawah.

## §44.6 — Revisi Bottom Sheet Profil: tombol Keluar pindah+redesain, toggle tema 3-arah Auto/Light/Dark (29 Agt 2026)

**Konteks**: Guru minta 2 revisi terpisah ke Bottom Sheet Profil
(`vue-sheet-profil.js`), masih dalam rangkaian redesain "Gechoo Mobile
Organic": (1) tombol Keluar (dulu lebar penuh di paling bawah sheet)
diganti oval kecil isi ikon X saja, DIPINDAH ke atas (dekat gagang,
sebelum kartu profil), dengan jarak yang sengaja dibedakan (margin-top
kecil dekat gagang, margin-bottom lebih besar ke kartu profil) supaya
"mengambang" terpisah dan ke-highlight; (2) toggle Mode Gelap yang
tadinya cuma terang/gelap (binary) DIPERLUAS jadi 3 pilihan Terang/
Gelap/**Otomatis** (ikut `prefers-color-scheme` sistem, re-apply
otomatis kalau sistemnya ganti tema saat "Otomatis" aktif), tetap bisa
di-pin manual ke Terang atau Gelap.

**File yang diubah**:
- `index.html` — script inline tema di `<head>` (dipasang sebelum
  `<body>` biar tidak "kedip") DIROMBAK dari binary jadi 3-arah.
  Dibedakan 2 fungsi publik SUPAYA kode lama yang sudah pakai
  `window.temaSaatIni()` TIDAK PERLU disentuh/tetap jalan sama:
  `window.temaSaatIni()` **TETAP** selalu balikin `'light'`/`'dark'`
  efektif saja (warna yang sedang tampil); `window.temaPreferensi()`
  **BARU**, balikin preferensi mentah user, BISA `'auto'`. Fungsi BARU
  lain: `window.aturTema(mode)` (set+simpan preferensi ke
  `localStorage['zevanic_tema']`, dispatch event
  `'zevanic-tema-berubah'`). `window.toggleTema()` sekarang siklus 3
  tahap (light → dark → auto → light...), dulu cuma 2. Listener
  `matchMedia('(prefers-color-scheme: dark)').addEventListener` BARU
  buat re-apply otomatis saat "auto" aktif & sistem ganti tema
  live (tanpa perlu reload/klik apapun). Digrep seluruh `js/`+
  `index.html` — TIDAK ADA file lain yang pakai
  `temaSaatIni`/`toggleTema`/`aturTema` selain `vue-sheet-profil.js`,
  jadi perubahan ini 100% backward-compatible, tidak ada tempat lain
  yang perlu ikut disesuaikan. Cache-bust: `vue-sheet-profil.js`
  (belum pernah ada `?v=` sebelumnya) → `?v=2`.
- `js/vue-sheet-profil.js`:
  - Toggle tema: `temaGelap` (boolean lama) GANTI `temaPref` (ref
    string, `'light'/'dark'/'auto'`) + 2 computed baru `ikonTema`
    (`fa-sun`/`fa-moon`/`fa-circle-half-stroke`) & `labelTema`
    ("Mode terang"/"Mode gelap"/"Otomatis"). `muatData()` &
    `klikModeGelap()` disesuaikan baca/tulis lewat
    `window.temaPreferensi()` (bukan `window.temaSaatIni()` lagi,
    supaya UI ini tahu kalau user lagi di "auto", bukan cuma tahu
    warna efektifnya).
  - Tombol Keluar: DIHAPUS dari bawah (dulu `.btn-outline.block` lebar
    penuh, ikon+teks "Keluar"). GANTI tombol oval kecil (34×26px,
    `border-radius:999px`, background+border `var(--danger-light)`,
    ikon `fa-xmark` doang, `var(--danger)`) diposisikan rata-kanan
    (`justify-content:flex-end`) tepat setelah area gagang sheet,
    SEBELUM kartu profil gradien — `margin-top:2px` (dekat gagang) vs
    `margin-bottom:16px` (jarak ke kartu profil) SENGAJA dibedakan
    biar tombolnya kebaca terpisah/ke-highlight, bukan nempel jadi
    1 grup sama kartu profil. Fungsi `keluar()` (logic-nya, panggil
    `tutup()` lalu `window.logout()`) **TIDAK diubah sama sekali** —
    cuma tombol & posisinya yang beda.

**Sengaja TIDAK diubah**: semua 6 link navigasi sub-tab (Data
Karyawan/Absensi/Reimburse/Estimasi Gaji/Pencapaian/Keamanan), tombol
Clock In/Out & Scan QR di grid 3-aksi-cepat, kartu profil gradien+QR —
semua persis sama, cuma slot Mode Gelap (icon+label) dan
posisi/bentuk tombol Keluar yang disentuh.

**Verifikasi**: `node --check` lolos untuk `vue-sheet-profil.js` &
script tema inline `index.html` (diekstrak & dicek terpisah). Hitung
`<div>`/`</div>` (12/12) & `<button>`/`</button>` (10/10) di template
`vue-sheet-profil.js` — seimbang. Digrep referensi
`temaSaatIni`/`toggleTema`/`aturTema`/`temaPreferensi` di seluruh
`js/`+`index.html` — dipastikan tidak ada pemanggil lama yang bakal
patah gara-gara `temaGelap` dihapus (memang tidak ada yang pernah
pakai `temaGelap` di luar file ini).

**STATUS: BELUM DITES Guru di live.**

---

## Jawaban: modul mana lagi yang cocok dapat treatment "Gechoo Mobile Organic" seperti Antrean Absensi? (29 Agt 2026)

Guru tanya "antrean ini sebagai bentuk percontohan dan saya suka, kira2
yang dapat update fitur mana saja?" — dicek LANGSUNG ke kode (bukan
tebak2), 3 modul lain yang bentuknya sama persis (kartu antrean +
tombol Accept/Reject/proses status) dengan Antrean Absensi:

1. **Antrean Lembur** (`js/vue-antrean-lembur.js`, 262 baris) — **PALING
   COCOK, direkomendasikan pertama**. Pola identik dengan Antrean
   Absensi SEBELUM redesain: 1 kartu = 1 pengajuan, tombol
   `proses('ACC')`/`proses('REJECT')` (2 tombol, bukan 3 — Lembur
   tidak punya konsep "Sesuai/Tidak Sesuai" terpisah dari Absensi,
   cukup Accept/Reject polos), header banner pink + dropdown filter
   Owner (Jenis Pekerjaan/Gudang) yang SELALU tampil (belum
   disembunyikan di belakang ikon filter), kolom cari (kalau ada)
   belum pakai `KolomCari`. TIDAK ada foto/avatar di kartunya (beda
   dari Absensi yang ada foto selfie) — jadi bagian "avatar zoom"
   dari moodboard tidak relevan di sini, tapi semua pola lain (kartu
   padat, tombol aksi rounded, filter di belakang ikon, search pil)
   langsung bisa dicontek. File paling kecil dari 3 kandidat = risiko
   paling rendah.
2. **Antrean Dakar** (`js/vue-antrean-dakar.js`, 498 baris) — cocok
   untuk BAGIAN ANTREAN-nya saja (kartu daftar pendaftaran karyawan
   baru + tombol approve, header pink + filter dropdown sama seperti
   di atas). Foto yang ada di sini foto KTP (persegi panjang, sudah
   disentuh dikit di §44 — border+shadow disamakan gaya kartu lain,
   TAPI TETAP kotak, bukan bulat, karena memang KTP bukan wajah) —
   BUKAN kandidat avatar bulat zoom seperti Absensi. File ini juga
   punya bagian FORM Entry Karyawan yang besar & terpisah dari kartu
   antrean — kalau dikerjakan, restyle SEBAIKNYA dibatasi ke bagian
   kartu antrean/header saja (pola sama seperti `vue-account-
   profile.js` di §44.2: sentuhan ringan di daftar, form TIDAK
   disentuh), supaya tidak melebar jadi restyle form besar yang belum
   direncanakan.
3. **Reimburse** (`js/vue-reimburse.js`, 1091 baris — ini yang Guru
   maksud "Antrean Reimburse" di screenshot sebelumnya) — kandidat
   TAPI **risiko paling tinggi dari 3-nya**, disarankan PALING
   TERAKHIR. Alasan: file paling besar (hampir 2× Dakar, 4× Lembur),
   DAN persetujuan 3-TAHAP berurutan (Admin Finance → PIC → Owner,
   field `tahap` tunggal, beda dari Absensi/Lembur yang cuma 1 tahap
   ACC/Reject) — lebih kompleks, potensi salah pencet lebih fatal
   (uang beneran, bukan cuma status kehadiran). Tombolnya juga 2
   (Accept/Reject polos, ada foto bukti pengeluaran bukan avatar
   wajah). Kalau dikerjakan, sebaiknya SETELAH Lembur & Dakar selesai
   dites Guru dulu, biar pola-nya benar-benar matang sebelum disentuh
   ke modul yang uangnya beneran jalan.

**Rekomendasi urutan pengerjaan (kalau Guru mau lanjut rollout)**:
Antrean Lembur dulu (paling mirip, paling kecil, paling aman) →
Antrean Dakar (bagian antrean-nya saja, form Entry Karyawan tidak
disentuh) → Reimburse (paling besar & paling sensitif, terakhir).
Semua 3 bisa langsung pakai kelas global `.approve-row`/`.appr-btn`
(CSS sudah ada dari §44.5) + komponen `KolomCari`/`HeaderLayar` yang
sudah ada — tidak perlu didesain ulang dari nol, tinggal contek pola
Antrean Absensi persis seperti yang ditulis di moodboard
`PEDOMAN-GAYA-KERJA.md`.

## §44.7 — Koreksi Bottom Sheet Profil (v3) + artefak rollout direvisi jadi v2, dari cek live Guru di HP (29 Agt 2026)

**Konteks**: Guru kirim 2 screenshot Antrean Absensi LIVE di HP (bukan
mockup) + daftar revisi. Ada 2 bagian: (A) koreksi ke Bottom Sheet
Profil yang baru dikirim di §44.6 — ternyata "pindah ke atas" itu
salah paham; (B) 3 temuan bug/leftover di Antrean Absensi live + detail
per-field buat 4 mockup di §44.6 (artefak "Gechoo Mobile Organic"),
SEMUA dicek dulu lewat AskUserQuestion sebelum ubah kode (sesuai
"jangan bikin tebak2") — bukan ditebak dari kalimat yang ambigu.

**(A) `js/vue-sheet-profil.js` v2→v3 (KODE, sudah dikirim)** —
dikonfirmasi lewat AskUserQuestion, jawaban Guru:
1. Tombol Keluar oval-X **DIKEMBALIKAN ke BAWAH** (setelah link
   Keamanan, rata TENGAH) — v2 sempat naruh di ATAS dekat gagang, itu
   salah paham dari kalimat "pindah di atas profil" (§44.6). Style
   TETAP oval kecil 34×26px, bukan lebar penuh seperti sebelum
   redesain.
2. Slot pertama grid 3-aksi-cepat (dulu "Clock in/out") **DIGANTI jadi
   tombol Keluar juga** — jadi sekarang ADA DUA jalan ke Keluar (grid
   atas + oval bawah), sesuai permintaan eksplisit Guru di kedua
   pertanyaan AskUserQuestion. `klikClockInOut()`, `sudahAbsen` ref,
   dan `cekStatusClockInSaya()` di `buka()` DIHAPUS (tidak dipakai lagi
   di manapun di file ini — digrep, bersih). Clock In/Out TETAP bisa
   diakses lewat kartu Favorit Saya di Beranda (WAJIB tampil di sana,
   §44) — bukan regresi, cuma jalan pintas KEDUA yang dilepas dari
   Profil.
3. Toggle Mode Gelap 3-arah (dari §44.6) — TIDAK diubah lagi di round
   ini.

Cache-bust `index.html`: `vue-sheet-profil.js?v=2` → `?v=3`.
Verifikasi: `node --check` lolos, `<div>`/`<button>` seimbang (12/12,
10/10). Digrep `sudahAbsen`/`klikClockInOut`/`cekStatusClockInSaya` —
nol sisa referensi. **Sudah dikirim ke folder Guru, BELUM DITES di
live.**

**(B) Artefak "Gechoo Mobile Organic" direvisi jadi v2 (MOCKUP, belum
kode)** — 3 perbaikan GLOBAL (berlaku ke semua modul antrean, bukan
cuma Absensi) ditemukan dari cek langsung ke `index.html` (bukan
tebak2):

1. Kartu deskripsi "Management & validasi absensi" (judul+teks 2
   baris di atas banner pink, digrep ketemu di `index.html` baris
   ~331) itu HEADER DESKTOP LAMA punya `#tab-admin-acc` — dipakai
   BERSAMA oleh Config Absensi/Penjadwalan/Antrean Absensi/Antrean
   Lembur/Riwayat All Absensi (5 sub-tab), SELALU tampil apa adanya
   (tidak ada `hidden md:` di elemen judul+deskripsinya, beda dari
   baris tombol sub-tab di bawahnya yang sudah `hidden md:flex`) —
   jadi dobel dengan `HeaderLayar` yang sudah menyampaikan judul yang
   sama. Guru minta dihapus/disembunyikan, desktop & mobile
   disamakan perlakuannya. **BELUM DIKODE** — nunggu Guru cek
   mockup-nya dulu, dan karena 1 blok ini dipakai 5 halaman sekaligus,
   perlu hati-hati baris tombol sub-tab desktop-nya (navigasi asli,
   BUKAN bagian yang mau dihapus) tidak ikut kehapus.
2. Banner pink "Antrean validasi absensi" (+ tombol Cek Data Sangat
   Lama/Refresh) pindah ke BAWAH kolom cari, dipadatkan jadi 1 baris.
3. Ikon filter (bulat, titik merah kalau aktif) ganti jadi **titik-tiga
   oval** "menu lainnya" — Filter Owner tetap di dalamnya, plus nampung
   fitur masa depan (Cek Data Lama, Refresh, kandidat Import CSV/
   Terapkan Massal).

**Detail per-modul (semua dicek ke kode dulu, bukan tebakan)**:
- **Antrean Absensi**: baris "email" di header kartu ganti **nama
  Gudang**, baris baru di bawahnya **nama Shift + jam shift**
  (07:00–15:00 dst) — dicek field `data.gudang`/`data.nama_shift` +
  `jamShift` (computed dari `master_shift`) SUDAH ADA di
  `vue-antrean-absensi.js`, cuma belum dipakai di posisi ini.
- **Antrean Lembur**: **avatar ditambahkan** — dicek `vue-camera.js`
  baris ~617-621, konfirmasi `foto_selfie` SELALU terisi juga untuk
  status `LEMBUR (CLOCK IN)` (lewat kamera yang sama seperti Hadir),
  cuma belum ditampilkan di kartu. Baris "Gudang: X" ganti format
  **"{tanggal pengajuan} – {nama gudang}"**. Jam Lembur diajukan
  didampingkan sama **Jam Shift** asli orangnya (Jam Shift rata kiri,
  Jam Lembur rata kanan) — field `nama_shift`/`gudang` dikonfirmasi
  ADA di dokumen Lembur (`vue-camera.js` `dataKirim` dasar).
- **Antrean Dakar**: banner pindah ke bawah cari (ikut pola global).
  **Form persetujuan diusulkan dipangkas dari 4 field (Status
  Kerja/Shift/Jabatan/Status Karyawan) jadi 2 field (Jadwal Shift +
  Jabatan, dropdown dibatasi Operator/Admin)** — Status Karyawan &
  Gudang pindah Owner-only lewat Config, Status Kerja dihapus dari
  form karena harusnya OTOMATIS aktif saat "Setujui" diklik. **Ini
  PERUBAHAN LOGIC, bukan cuma tampilan** — ditandai di artefak perlu
  ronde pengecekan kode TERPISAH sebelum dikerjakan (field mana persis
  yang pindah ke Config, gimana "auto-aktif" itu dipicu), tidak boleh
  langsung ditebak dari mockup saja.
- **Reimburse**: label "Diajukan" dilepas, cukup tampilkan tanggal
  polos. **Khusus kategori Isi Bensin**: diusulkan tambah field
  Odometer Sebelum/Sesudah + badge efisiensi BBM otomatis (km/L).
  Rumus dicari referensinya (bukan tebakan) — dikonfirmasi ke
  [Auto2000](https://auto2000.co.id/berita-dan-tips/cara-menghitung-bensin-mobil-per-kilometer):
  **Efisiensi (km/L) = Jarak Tempuh (km) ÷ Jumlah BBM Dibeli (liter)**,
  Jarak Tempuh = Odometer Sesudah − Odometer Sebelum, metode "isi
  penuh ke isi penuh".

**STATUS artefak**: dipublish ulang di URL Artifact yang SAMA
(v2), belum ada satu baris kode modul manapun yang diubah — MENUNGGU
ACC Guru per-item sebelum masuk fase koding (terutama item Dakar yang
sifatnya perubahan logic, bukan visual).

## §44.8 — Koreksi tombol (X) jadi v4 + ACC Guru: 4 modul antrean dikoding sekaligus sesuai artefak v2 (29 Agt 2026)

**Konteks**: Guru koreksi lagi tombol (X) di Bottom Sheet Profil
("bukan logout, tapi tutup bottom layer" + warna dark mode terlalu
mencolok) **DAN** dalam pesan yang sama kasih ACC blanket untuk
keempat mockup di §44.7(B): *"keempatnya kartu antrean update juga
sudah cocok sekaligus kan buatnya dan revisi tombol x"*. Kedua bagian
dikerjakan berurutan — (A) fix tombol X dulu (kecil, sudah dikirim),
lalu (B) 4 modul dikoding sesuai artefak (besar, bagian utama section
ini).

**(A) `js/vue-sheet-profil.js` v3→v4 + `css/gechoo-design.css` — tombol (X) diperbaiki (KODE, sudah dikirim)**:
1. `@click="keluar"` (logout) diganti `@click="tutup"` — tombol (X) di
   bawah CUMA menutup bottom sheet, BUKAN jalan logout kedua. Jalan
   logout SATU-SATUNYA sekarang cuma slot pertama grid 3-aksi-cepat
   (dari §44.7). `title`/`aria-label` ikut diganti "Tutup".
2. Warna dipisah jadi class baru `.gc-x-tutup` di `gechoo-design.css`
   (gantikan inline style `--danger-light`/`--danger` yang tidak bisa
   respon ke mode gelap): mode terang TETAP oval pucat + ikon danger
   (Guru bilang sudah cocok), mode gelap KHUSUS pakai
   `[data-theme="dark"] .gc-x-tutup{background:var(--burgundy);
   color:var(--pink);}` — soalnya token `--danger-light` SENGAJA TIDAK
   di-override antar tema (lihat catatan `:root` di CSS), jadi di
   permukaan gelap warnanya jadi sangat terang/mencolok kalau dipakai
   apa adanya.
3. Cache-bust: `gechoo-design.css?v=3`, `vue-sheet-profil.js?v=4`.
   Verifikasi: `node --check` lolos, tag `<div>`/`<button>` seimbang
   (12/12, 10/10), CSS brace 232/232.

**(B) 4 modul antrean dikoding sesuai artefak v2 (KODE, bagian utama round ini)** —
urutan pengerjaan sengaja Absensi → Lembur → Dakar → Reimburse (paling
sensitif dikerjakan TERAKHIR, sesuai catatan risiko di artefak §44.7).
3 perbaikan "desain global" (kartu deskripsi dihapus, banner pindah ke
bawah cari + dipadatkan, ikon filter jadi menu oval titik-tiga
`.gc-overflow-btn`/`.gc-overflow-panel`/`.gc-overflow-item` — class
BARU ditambahkan ke `gechoo-design.css`, pola dropdown-nya nyontek
lonceng notifikasi `vue-header-mobile.js`) diterapkan SAMA PERSIS di
keempat modul.

- **`js/vue-antrean-absensi.js` (?v=5→?v=6)**: header kartu 3 baris
  (Nama / Gudang / Shift + jam shift dari `jamShift` yang SUDAH ADA di
  `setup()` tapi belum dipakai — cuma ditambahkan ke `return`), baris
  "email" dilepas. Link "Peta" (format lama, ada koordinat) TETAP ada,
  dipindah jadi baris sendiri di bawah header (bukan dihapus). Cek Data
  Sangat Lama/Refresh pindah ke menu oval; `filterTerbuka` di-rename
  `menuTerbuka` (sekarang bukan cuma filter).
- **`js/vue-antrean-lembur.js` (?v=2→?v=3)**: modul ini BELUM PERNAH
  ikut redesain sebelumnya (masih pola `btn-acc`/`btn-rej` lebar penuh
  + search hand-rolled) — jadi round ini penerapan LENGKAP pertama
  kali: `KolomCari`, `.approve-row`/`.appr-btn` ("Setujui"/"Tolak", 2
  tombol — Lembur tidak punya konsep Sesuai/Tidak Sesuai), avatar dari
  `foto_selfie` (field SUDAH ADA, dikonfirmasi lewat `vue-camera.js`
  §44.7, baru DITAMPILKAN sekarang), baris "{tanggal} – {gudang}"
  (`formatTglSingkat(data.waktu_ts)`, GANTI `data.waktu` yang string
  mentah `toLocaleString`), baris Jam Shift (kiri, lookup BARU ke
  `master_shift` via `nama_shift` — pola disalin dari Absensi, Lembur
  belum pernah punya ini) vs Jam Lembur diajukan (kanan). **Instruksi
  Kerja TETAP ditampilkan** (baris kecil terpisah) — mockup v2 tidak
  menggambarnya demi ringkas, tapi sengaja TIDAK dihilangkan dari kode
  sungguhan karena itu info operasional buat admin yang approve, bukan
  cuma kosmetik yang aman dibuang.
- **`js/vue-antrean-dakar.js` (?v=3→?v=4) — PERUBAHAN LOGIC, bukan
  cuma tampilan, ditandai sejak awal di artefak**: form persetujuan
  dipangkas dari 4 field jadi 2 (Jadwal Shift + Jabatan). Detail
  keputusan yang SEBELUMNYA ditandai "perlu ronde cek terpisah" di
  §44.7, sekarang sudah dicek & diputuskan:
  - **Jabatan dibatasi Operator/Admin** — dicek `js/dashboard.js`
    baris 107, seed default Master Data Jabatan persis
    `["Operator","Admin","Warehouse"]`, jadi filter
    `j==='Operator'||j==='Admin'` match data asli, bukan tebakan. Kalau
    Master Data Jabatan Guru sudah diubah dari seed (ejaan beda),
    dropdown ini bisa kosong — ada `console.warn` jaring pengaman biar
    ketahuan pas testing, BUKAN gagal diam-diam.
  - **Status Kerja** dihapus dari form, di `setujui()` **hardcode
    `'Aktif'`** langsung (bukan bergantung urutan Master Data
    `status_kerja` seperti sebelumnya) — sesuai permintaan "harusnya
    aktifkan begitu diklik Setujui".
  - **Status Karyawan & Gudang penempatan** dilepas dari form, dikirim
    kosong (`''` dan `[]`) — WAJIB dilengkapi Owner lewat *Daftar
    Karyawan > Edit* (dicek: modal Edit di `vue-daftar-karyawan.js`
    SUDAH support field ini, jadi ada tempat mengisinya belakangan).
    **RISIKO OPERASIONAL NYATA yang dicek ke kode, bukan diabaikan**:
    `js/vue-login.js` baris 370 memblokir login SIAPAPUN (termasuk
    Owner) kalau `gudang_penempatan.length===0` — jadi SETIAP karyawan
    baru lewat jalur ini PASTI tidak bisa login sampai Owner
    melengkapi Gudang-nya. `confirm()` popup lama (muncul kalau admin
    lupa isi gudang) DIHAPUS karena sekarang PASTI selalu kosong di
    titik ini (popup tiap approve jadi mengganggu) — diganti catatan
    tetap (warning banner kuning) yang SELALU kelihatan di form,
    sebelum tombol Setujui, biar tidak lupa.
  - Tombol "Buat Data Uji" (testing, Owner-only) dipindah ke menu
    oval, TIDAK dihapus.
- **`js/vue-reimburse.js` (?v=2→?v=3) — dikerjakan PALING TERAKHIR
  sesuai catatan risiko (uang beneran, alur 3-tahap)**: label
  "Diajukan" dilepas, tanggal polos (`formatTgl`, lokal ke
  `ReimburseCard`, format sama seperti komponen riwayat lain di file
  ini). Foto bukti jadi ikon kecil 38×38 di header (dari thumbnail
  64×64 terpisah). **Khusus kategori Isi Bensin — field baru** (dicek
  ke `AjukanReimburseTab`, form pengajuan asli, bukan cuma tampilan
  antrean): field lama `km_saat_isi` (1 odometer tunggal) DIGANTI 3
  field baru — `odo_sebelum`, `odo_sesudah`, `liter_bensin` (field
  Liter ini TIDAK ADA SAMA SEKALI sebelumnya, ditambahkan karena rumus
  butuh angka liter, bukan cuma diasumsikan dari mockup). Efisiensi
  (km/L) dihitung `computed` di `ReimburseCard`
  (`(odo_sesudah-odo_sebelum)/liter_bensin`, 1 desimal) — TIDAK
  disimpan field terpisah, sesuai catatan di artefak. Rumus dikonfirmasi
  ke [Auto2000](https://auto2000.co.id/berita-dan-tips/cara-menghitung-bensin-mobil-per-kilometer)
  sejak §44.7 (bukan tebakan baru). Riwayat Pengajuan Saya (tab
  Ajukan Reimburse) & entri lama yang masih pakai `km_saat_isi`
  TETAP ditampilkan (fallback `v-else-if`) — data lama tidak hilang
  dari layar, cuma tidak dapat badge efisiensi (data odo/liter-nya
  memang tidak ada).

**Verifikasi semua file**: `node --check` lolos untuk kelima file JS
(`vue-antrean-absensi.js`, `vue-antrean-lembur.js`,
`vue-antrean-dakar.js`, `vue-reimburse.js`, `vue-sheet-profil.js`), tag
`<div>`/`<button>`/`<select>` seimbang per file (dihitung manual via
regex, bukan cuma "kelihatannya benar"), CSS brace 242/242 (naik dari
232, konsisten +10 selector baru: `.gc-overflow-*` × 8 + `.gc-badge-
count`), `<div>` seluruh `index.html` seimbang 167/167 sebelum &
sesudah edit header `#tab-admin-acc`. Digrep bersih — nol sisa
referensi ke field/nama lama yang sengaja dihapus
(`opsiStatusKerja`/`opsiStatusKaryawan`/`form.gudang` di Dakar,
`GudangCheckboxSelect` di Dakar, `km_saat_isi` sebagai field BARU di
Reimburse, `filterTerbuka` lama di semua file).

**`index.html`**: kartu deskripsi "Management & validasi absensi"
(§44.7 poin B.1) DIHAPUS (h2+p), baris navigasi sub-tab desktop di
bawahnya TETAP 100% sama — wrapper `.gc-card`-nya ditambah
`hidden md:block` supaya tidak ada kartu kosong nongol di mobile.
Cache-bust dinaikkan untuk SEMUA file yang berubah:
`gechoo-design.css?v=4`, `vue-antrean-dakar.js?v=4`,
`vue-antrean-absensi.js?v=6`, `vue-antrean-lembur.js?v=3`,
`vue-reimburse.js?v=3`.

**BELUM DITES di live** — sama seperti (A), semua di atas baru lolos
verifikasi statis (syntax + tag balance), belum pernah dibuka
sungguhan di HP Guru. Yang PALING perlu ditest manual sebelum dipakai
sungguhan:
1. Antrean Dakar — approve 1 karyawan uji, konfirmasi field
   `status_kerja`/`jabatan`/`status_karyawan`/`gudang_penempatan`
   tersimpan sesuai rencana, lalu coba login akun itu (harus DITOLAK
   sampai Gudang diisi lewat Daftar Karyawan > Edit) — buktikan gerbang
   `vue-login.js` baris 370 kejadian seperti dianalisis, bukan asumsi.
2. Reimburse Isi Bensin — ajukan 1 entri lewat form baru (odo
   sebelum/sesudah/liter), cek badge efisiensi muncul benar di kartu
   antrean Admin Finance.
3. Tombol (X) Bottom Sheet Profil di mode gelap sungguhan di HP
   (burgundy+pink) — belum ada screenshot pembanding dari Guru.

## §44.9 — Tombol "Unduh CSV" di Riwayat Reimburse/Isi Bensin/Servis, Master Keuangan (29 Agt 2026)

**Permintaan**: "bantu buatkan download csv menu daftar riwayat
reimburse pada master keuangan".

**Dicek dulu ke kode (bukan tebakan)**: sudah ada pola CSV export di
`js/vue-riwayat-absensi.js` (`exportCSV()`, data URI
`data:text/csv;charset=utf-8,` + elemen `<a download>`, TANPA library
apapun) dan `window.exportKeCSV` di `js/dashboard.js` — dipakai ulang
pola yang sama, bukan bikin cara baru. Komponen riwayat reimburse
(`RiwayatReimburseTable`, `js/vue-reimburse.js` baris ~1033) itu 1
komponen dipakai ULANG buat 3 menu (Riwayat Reimburse/Riwayat Isi
Bensin/Riwayat Servis) lewat prop `mode` — jadi tombol Unduh CSV
ditambahkan SEKALI di sini, otomatis muncul di ketiga menu.

**Implementasi**:
- Tombol "Unduh CSV" (ikon `fa-file-excel`, style `btn-outline`,
  disamping tombol Refresh yang sudah ada) mengekspor `daftarTersaring`
  (hasil PENCARIAN AKTIF, bukan cuma 1 halaman paginasi yang lagi
  tampil di layar) — sama seperti pola di `vue-riwayat-absensi.js`.
- Kolom CSV MENYESUAIKAN mode, SAMA PERSIS kolom yang sudah ditampilkan
  di kartu (`v-if="mode === ..."` di template): mode `semua` →
  Nama/Email/Gudang/Tanggal/**Kategori**/Jumlah/Status/Keterangan; mode
  `bensin` → tambah Kendaraan + **Odometer Sebelum/Sesudah + Liter
  Dibeli + Efisiensi (km/L)** (dihitung ulang di JS dengan rumus SAMA
  PERSIS `efisiensiBBM` di `ReimburseCard`/§44.8, bukan rumus baru);
  mode `servis` → tambah Kendaraan + Rincian Servis (gabungan nama
  barang×qty).
- Kolom **Jumlah diekspor sebagai ANGKA MENTAH** (mis. `100000`, bukan
  string `"Rp100.000"`) — keputusan sadar karena ini buat Master
  Keuangan, supaya bisa langsung dijumlah di Excel tanpa perlu bersihkan
  format teks dulu.
- Nama file: `<Judul_Mode>_Zevanic_<tanggal-hari-ini>.csv` (mis.
  `Riwayat_Isi_Bensin_Zevanic_2026-08-29.csv`).
- Setiap sel CSV dibungkus tanda kutip + tanda kutip internal
  di-escape (`"` → `""`) — SEDIKIT lebih ketat dari pola referensi di
  `vue-riwayat-absensi.js` (yang tidak escape kutip internal), demi
  mencegah data yang kebetulan mengandung tanda kutip merusak struktur
  file — bukan penyimpangan pola, cuma penguatan kecil.

**Cache-bust**: `index.html` — `vue-reimburse.js?v=3` → `?v=4`.
**Verifikasi**: `node --check` lolos, tag `<div>`/`<button>` seimbang
(98/98, 21/21), `<div>` seluruh `index.html` tetap 167/167.

**BELUM DITES manual** — cek bahwa file .csv yang di-download benar2
kebuka rapi di Excel/Google Sheets (terutama kolom Jumlah kebaca
sebagai angka, bukan teks) untuk ketiga mode.

## §44.10 — Diskusi arsitektur "Persiapan Produksi V2" (SPK Grouping + 5 jalur paralel) (29 Agt 2026)

**Konteks**: Guru minta diskusi ("kita diskusi perihal menu persiapan
produksi") dan kirim spesifikasi alur baru yang JAUH beda dari menu
"Persiapan Produksi" yang baru dibangun §43 (28 Agt 2026, SEHARI
sebelumnya): SPK Grouping (gabung beberapa Order ID yang SKU-nya sama
jadi 1 kode `SPK{YYMMDD}{urut3digit}`), lalu 5 jalur paralel (Vendor/
Bahan/Acc Sewing/Acc Webbing/Acc Finishing) yang masing-masing punya 5
tahap (Perlu Diproses → Sedang Diproses → Perlu Dikirim → Sedang
Dikirim → Selesai) dengan gerbang cetak-label-lalu-scan di tiap
transisi (Label SPK Grouping→Scan Operator, Scan Entry/Scan Masalah,
Label Bagging→Scan Pack, Label Tugas→Scan Kirim, lalu Scan SAMPAI di
sisi penerima yang baru mengubah status jadi Selesai).

**Riset dulu, bukan tebakan** (sesuai instruksi proyek): sebelum
membalas, dicek 3 lapis — Project docs, 1 agent riset, dan grep
langsung sendiri (field `bom_pola.tipe`/`jenis_vendor` di
`js/vue-master-produk.js`, baris 405/736/901/1170/1240/1697,
dikonfirmasi field AKTIF terpakai bukan data mati) — untuk memetakan
persis apa yang sudah ada di kode vs spek baru Guru, supaya diskusi
berbasis fakta.

**3 pertanyaan arsitektur kunci** (AskUserQuestion, sebelum desain data
model ditulis):
1. Alur baru GANTI TOTAL sistem "Persiapan Produksi" yang sekarang, atau
   jadi lapisan baru di depannya? → **Ganti total** (dipilih Guru).
2. Traceability qty per Order ID asli di dalam SPK Grouping gabungan? →
   **Simpan breakdown per Order ID** di dalam dokumen grouping (dipilih
   Guru).
3. 5 tahap di tiap jalur untuk 1 SPK Grouping yang sama jalannya gimana?
   → **Paralel independen** per jalur (dipilih Guru).

**Temuan penting yang mengubah kalkulasi risiko**: dicek
`STATUS-PROYEK.md` §43 — sistem "Persiapan Produksi" yang sekarang
BARU dibangun 28 Agt 2026 dan **BELUM PERNAH diuji manual sama sekali**
(masih terblokir publish Firestore rules + 2 composite index waktu
catatan terakhir ditulis). Artinya kemungkinan besar **belum ada data
live** di `persiapan_produksi`/`persiapan_komponen` — keputusan "ganti
total" jadi risiko rendah, tidak perlu migrasi data, tinggal
dikonfirmasi ke Guru lalu pensiunkan.

**Temuan hook yang bisa dipakai ulang** (bukan bangun dari nol):
- `PopupPratinjauCetakLabel` (`js/vue-components.js`) — komponen cetak
  label generik yang sudah dipakai 3 tempat, kontraknya pas buat 3 jenis
  label baru (SPK Grouping/Bagging/Tugas) tanpa modifikasi.
- Mekanisme scan kamera+`jsQR` di `js/vue-scan-persiapan.js` — pola
  scan-nya bisa dipakai ulang buat semua aksi SCAN baru (Operator/Entry/
  Masalah/Pack/Kirim/Sampai), tinggal beda target tulis.
- `master_tahap_persiapan` + field `tahap_proses` di BOM Aksesoris —
  sudah persis menghasilkan pengelompokan Sewing/Webbing/Finishing yang
  dibutuhkan 3 dari 5 jalur baru, tidak perlu field baru.
- `bom_pola.tipe==='vendor'` + `jenis_vendor` (Master Produk) — calon
  penentu jalur "Vendor" (potong pola yang dikerjakan pihak luar),
  masih perlu dikonfirmasi ke Guru.

**Hasil**: dokumen desain lengkap `RENCANA-PERSIAPAN-PRODUKSI-V2.md`
(struktur data usulan `spk_grouping`+`spk_track`, alur tiap tahap
dipetakan ke aksi scan yang diminta Guru, daftar hal yang masih perlu
dikonfirmasi sebelum coding, daftar yang dipensiunkan/dipertahankan,
saran pembagian 5 fase build) — dikirim ke Guru & disimpan di Project
(`claude/RENCANA-PERSIAPAN-PRODUKSI-V2.md`). **BELUM ADA KODE YANG
DITULIS** — masih tahap diskusi/perencanaan, menunggu Guru jawab sisa
pertanyaan (nomor urut grouping per-hari-global-vs-per-SKU, boleh 1
Order ikut >1 grouping atau tidak, isi pasti jalur Vendor, pembatasan
jabatan buat SCAN OPERATOR) sebelum mulai Fase 1.

## §44.11 — Putaran kedua diskusi "Persiapan Produksi V2": koreksi kunci grouping, QR Operator, jalur Vendor (29 Agt 2026, siang)

**Konteks**: 4 pertanyaan penutup §44.10 diajukan lewat AskUserQuestion,
Guru jawab dengan penjelasan bebas (bukan cuma pilih opsi) — jawabannya
MENGOREKSI beberapa dugaan awal di `RENCANA-PERSIAPAN-PRODUKSI-V2.md`.
Dicatat di sini + diverifikasi ulang ke kode sebelum masuk dokumen
rencana (bukan langsung ditulis mentah).

**Temuan/koreksi**:
1. **Kunci grouping BUKAN sekadar SKU sama** — kata Guru: SPK bisa
   digrouping kalau `nama_produk` sama DAN `panjang pola` + `isi pola`
   sama, WALAU asal bahan/warna beda — biar bisa "gelar kain bersamaan
   > pola kain > cutting bersamaan > dikirim". Dicek ulang ke
   `master_produk.bom_pola`: field `panjang` dan `isi_pola_pcs` MATCH
   PERSIS istilah Guru — bukan field baru, cuma logic grouping-nya yang
   perlu direvisi (nama_produk+panjang+isi_pola_pcs, bukan SKU).
2. **Split qty per Order terikat "KPK isi pola"** — pemahaman sementara:
   pemecahan qty ke grouping harus kelipatan `isi_pola_pcs` biar tidak
   ada sisa potong pola yang rugi bahan. **BELUM dijadikan logic kode**
   — diminta contoh angka konkret dulu ke Guru sebelum dipastikan
   (bagian paling berisiko salah tebak).
3. **Jalur Vendor = sublim/sablon/bordir dll (dekorasi luar)** — dugaan
   awal (`bom_pola.tipe==='vendor'` = potong pola vendor) **TERBUKTI
   KELIRU**. Field `tahap_proses` BOM Aksesoris sekarang cuma punya 3
   nilai (Sewing/Webbing/Finishing dari `master_tahap_persiapan`) —
   kemungkinan perlu tambahan nilai "Vendor" + field jenis vendor baru,
   BELUM final, masih nunggu konfirmasi Guru.
4. **SCAN OPERATOR pakai QR pribadi tiap akun yang SUDAH ADA** — dicek
   ke kode, INI SUDAH AKTIF: `js/vue-account-profile.js` (baris
   ~229-248) generate QR tiap akun dari `id_app`/email buat absensi
   fisik, captionnya BAHKAN sudah menyebut "verifikasi proses SPK"
   (baris 599) — kelihatan memang sudah disiapkan buat kebutuhan ini.
   Mekanisme scan (`js/vue-scan-qr.js`, jsQR) sama persis dengan Scan
   Persiapan. Tidak ada pembatasan jabatan. **Hook siap pakai, tidak
   perlu sistem QR karyawan baru.**

**Dokumen `RENCANA-PERSIAPAN-PRODUKSI-V2.md` direvisi** (§1.6 baru +
struktur data `spk_grouping`/`spk_track` disesuaikan + alur jalur Bahan
detail "gelar kain>pola kain>cutting" + §5 diganti 4 pertanyaan lanjutan
teknis: format nomor urut belum eksplisit dikonfirmasi, contoh angka
KPK isi pola, pendekatan tambahan tahap "Vendor" di BOM Aksesoris +
siapa yang scan di sisi vendor, dan apakah alur 5-tahap Vendor perlu
tahap tambahan khusus barang-keluar/barang-balik vendor). Dikirim ulang
ke Guru & disinkron ke Project. **MASIH BELUM ADA KODE YANG DITULIS.**

**Temuan tambahan sebelum mockup dibuat**: pertanyaan §5.B (aturan
"KPK isi pola") TERNYATA sudah punya jawaban di kode — field
`master_produk.kelipatan` (§42.2, 28 Agt 2026, fungsi `hitungKelipatan()`
di `js/vue-master-produk.js` baris 212-216) itu PERSIS rumus KPK/LCM
dari `bom_pola[].isi_pola_pcs` yang dimaksud Guru, sudah dipakai di Order
SPK sebagai "Rekomendasi Kelipatan Order". Jadi aturan split SPK
Grouping = qty yang diambil harus kelipatan bulat dari `produk.kelipatan`
— dikeluarkan dari daftar pertanyaan terbuka, dokumen direvisi.

## §44.12 — Mockup interaktif "Persiapan Produksi V2" (29 Agt 2026, sore)

**Permintaan**: "bisa dibantu dengan mockup".

Dibuat `mockup/persiapan-produksi-v2.html` — mockup HTML interaktif
(vanilla JS, TANPA Vue/build step, konsisten pola mockup review proyek
ini) menunjukkan 4 layar: (1) "Perlu Disiapkan" — kotak kandidat
grouping per kunci (nama produk + panjang pola + isi pola), checkbox
pilih Order, ringkasan kelipatan otomatis (contoh: 100pcs kelipatan 8 →
96pcs jalan, sisa 4pcs ditahan) + tombol "Buat SPK Grouping"; (2) Cetak
Label — 3 jenis label (SPK Grouping/Kode Bagging/Kode Tugas) preview
4x2 thermal; (3) Papan Jalur — pill 5 jalur (Bahan/Vendor/Acc Sewing/
Webbing/Finishing) × pill 5 status (Perlu Diproses..Selesai), kartu
contoh per status dengan tombol aksi sesuai (Cetak Label→Scan Operator
terkunci sampai label dicetak, Scan Entry/Scan Masalah, Scan Pack, Scan
Kirim, timeline riwayat scan di Selesai); (4) Scan Operator — mock
kamera scan QR pribadi karyawan (bukan label SPK), caption menjelaskan
tidak ada pembatasan jabatan.

Style pakai token CSS PERSIS dari `css/gechoo-design.css` (warna,
Poppins/Nunito Sans, pola kartu-baris/tag/tombol) — bukan desain
reka-reka, mode gelap/terang keduanya didukung (toggle 🌓 di pojok).
Ada catatan "MOCKUP DISKUSI — belum final" di header + daftar 4 poin
yang perlu dikomentari Guru di bawah frame HP.

Dipublish sebagai Artifact interaktif (bisa diupdate lagi lewat sesi
ini kalau ada revisi dari Guru) + dikirim juga file HTML-nya langsung.
Komputer Guru belum tersambung sesi ini jadi belum bisa ditaruh
langsung ke folder `F:\ZEVANIC HOUSE\FOUNDATION` — susulan begitu
desktop app dibuka lagi. **MASIH BELUM ADA KODE PRODUKSI YANG
DITULIS** — mockup ini murni buat diskusi visual sebelum Fase 1.

**Revisi (sore, permintaan Guru: "buatkan artefak karena ini bakal
jadi landasan... semirip mungkin dengan tampilan sekarang")** — mockup
pertama (kartu app-header generik) DIROMBAK TOTAL jadi pola "bingkai
HP realistis" yang sudah pernah dipakai &amp; disetujui Guru di
`mockup/gechoo-mobile-organic-rollout.html` (status bar 09:41+wifi+
baterai, topbar back+judul, ikon SVG inline bukan Font Awesome supaya
tidak gagal load, token `--app-*` diambil PERSIS dari
`css/gechoo-design.css`, toggle Terang/Gelap independen dari tema
browser). Struktur halaman JUGA diubah dari "1 phone + tab js pilih
layar" jadi 4 section terpisah (scroll ke bawah, ada navchip loncat)
masing-masing: 1 bingkai HP + kolom "Catatan desain" di sampingnya
(tag SUMBER/BARU/TANYA per catatan, pola sama seperti kolom
"changelog" di rollout mockup) — supaya jelas mana yang dari kode asli
vs konsep baru vs masih perlu jawaban Guru, TIDAK cuma gambar polos.
Dipublish ke **Artifact URL yang SAMA** (republish in-place, bukan link
baru) supaya jadi satu rujukan yang terus dimutakhirkan.

## §44.13 — Koreksi arsitektur menu + Fase 1 (SPK Grouping) dibangun (29 Agt 2026, malam)

**Koreksi Guru** (setelah 3 pertanyaan blocking dijawab — numbering
global per hari, koleksi lama ditinggalkan tanpa migrasi, & "menu sama
seperti sekarang" yang SEMPAT saya artikan keliru sebagai "tetap nested
di dalam Zevanic House"): *"ralat mending bikin grouo menu baru namanya
Persiapan Produksi supaya tersusun rapih"*, dipertegas lewat jawaban
bebas: *"persiapan produksi sejajar dengan zevanic house ... perlu
disiapkan, vendor, bahan, acc sewing, acc webing, acc finisihing >
sejajar dengan Stock & Pembelian/Keuangan/Master Produk (sub menu) ...
pipeline jadi child menu, sesuaikan masing2"*. Ditegaskan lewat
AskUserQuestion kedua: wiring lama **ditulis ulang bersih dari nol**
(bukan ditambal) — `js/vue-persiapan-produksi.js` & sub-tab lama di
`index.html` DITINGGALKAN (file JS tidak dihapus dari disk, tapi
`<script>`-nya dicopot dari index.html sehingga tidak lagi dimuat).

**Riset sebelum tulis kode** (wajib, jangan tebak-tebak): baca ulang
`index.html` (markup sidebar 5 grup top-level yang sudah ada + pola
`toggleNavGroup`/`pindahTab`/`pindahSubTab`), `js/dashboard.js`
(`petaGrupSidebarPerTab`, `window.pindahTab` array `tabs` + blok
landing-default per tab, `petaTabIndukPerGrup`, `petaMount`), dan —
KRITIS, nyaris jadi bug diam-diam kalau tidak ditelusuri — `js/auth.js`
`aturTampilanBerdasarkanRole()`: tombol header tiap grup top-level
(`menu-zevanic-house`, dst) mulai `hidden` di HTML dan HARUS
di-`classList.remove('hidden')` manual per role di fungsi itu; kalau
tombol grup baru tidak didaftarkan di sana, grup itu **permanen tidak
pernah muncul di sidebar siapapun** walau markup & routing-nya benar.
Ketemu juga `window.terapkanUrutanMenuDesktop()` (fitur urutan menu
custom Owner) yang punya peta kategori→id grup sendiri, perlu entry
baru juga supaya fitur itu tidak error diam-diam untuk kategori baru.

**Struktur BARU yang dibangun**:
- **Sidebar**: grup top-level ke-6, `navgrp-persiapanproduksi`
  ("Persiapan Produksi"), sejajar Master Absensi/Keuangan/Karyawan/
  Zevanic House/Integrasi. 6 sub-menu sejajar (Perlu Disiapkan/Vendor/
  Bahan/Acc Sewing/Acc Webbing/Acc Finishing) — persis pola
  Config/Data Bahan/Stock &amp; Pembelian dkk di Zevanic House.
- **5 jalur** (Vendor/Bahan/Acc Sewing/Acc Webbing/Acc Finishing)
  masing-masing punya **child-tab 5 tahap** (Perlu Diproses → Sedang
  Diproses → Perlu Dikirim → Sedang Dikirim → Selesai), riwayat browser
  aktif sampai level child-tab (`data-target` + `catatRiwayat:true`,
  sesuai `PEDOMAN-GAYA-KERJA.md`).
- **Fase 1 (SEKARANG)**: cuma "Perlu Disiapkan" yang fungsional penuh
  (lihat di bawah). 5 jalur × 5 tahap = 25 slot navigasi SUDAH aktif
  (bisa diklik, riwayat browser jalan) tapi isinya placeholder statis
  "segera hadir, Fase 2-5" — SENGAJA tidak dibangun komponen Vue-nya
  dulu (PRINSIP-HEMAT: jangan bangun infrastruktur yang belum
  dipakai) — TIDAK ada entry `petaMount` untuk 25 slot ini.

**File diubah/dibuat**:
- `index.html` — sidebar grup baru + `<div id="tab-persiapan-produksi">`
  (GANTI TOTAL blok lama nested di `tab-zevanic-house`), script tag
  `vue-persiapan-produksi.js` dicopot, ganti `vue-persiapan-produksi-v2.js`.
- `js/dashboard.js` (v18→v19) — `tabs[]`, `petaGrupSidebarPerTab`,
  landing-default `tab-persiapan-produksi`→"Perlu Disiapkan`,
  `petaTabIndukPerGrup`, `petaMount` (5 entry lama dihapus, 1 entry baru).
- `js/auth.js` — gerbang role `menu-persiapan-produksi` (pic/admin/
  owner/superuser, SAMA seperti Zevanic House) + entry
  `terapkanUrutanMenuDesktop()`.
- `js/vue-config-akses.js` — 5 menu-id lama `persiapan_produksi_*`
  ditandai `deprecated:true` (TIDAK dihapus — jaga config akses lama
  yang mungkin sudah terlanjur diatur), 6 menu-id baru `pp_*` kategori
  baru **"Persiapan Produksi"** ditambah ke `KATEGORI_URUTAN`.
- `js/vue-order-spk.js` — fungsi `buatAntreanPersiapanProduksi()` +
  pemanggilannya di `simpan()` DIHAPUS (dulu nulis ke koleksi
  `persiapan_produksi` yang sekarang tidak dibaca siapapun lagi —
  boros tulis tanpa guna, lawan `PRINSIP-HEMAT`). Import `setDoc` ikut
  dibersihkan (sudah tidak dipakai).
- `firestore.rules` — koleksi lama `persiapan_produksi`/
  `persiapan_komponen` rules DIBIARKAN (bukan dihapus, jaga-jaga
  dokumen lama), 2 match block baru: `spk_grouping` &
  `pengaturan_id_spk_grouping` (counter harian).
- `js/vue-header-mobile.js` — `LABEL_TAB` tambah `tab-persiapan-
  produksi`, `LABEL_SUBTAB` 5 entry lama `sub-zh-persiapanproduksi-*`
  diganti 6 label sub-menu + 25 label child-tab baru (supaya judul
  header mobile tetap benar di semua layar baru, celah yang sama
  seperti `tab-zevanic-house` yang pernah hilang dulu, §27).
- `js/vue-persiapan-produksi-v2.js` (BARU) — lihat detail Fase 1 di
  bawah.

**Fase 1 — "Perlu Disiapkan" (generator SPK Grouping)**:
- Baca `order_spk` where `status=='Aktif'` + semua `master_produk`
  (`ambilSemuaProduk()`, sudah ada), cocokkan kandidat grouping lewat
  `master_produk.nama` (nama DASAR, bukan `order_spk.nama_produk` yang
  ternyata string gabungan nama+warna+size — nyaris salah tebak, lihat
  §44.11) + "kunci pola" (sorted `bom_pola[].panjang`+`isi_pola_pcs`,
  field yang sama dipakai `hitungKelipatan()`/KPK).
- Kandidat ditampilkan sebagai kartu per klaster (anggota SPK + qty
  total + jalur otomatis terdeteksi dari BOM), tombol "Buat SPK
  Grouping" generate kode `SPK{yymmdd}{urutan 3 digit}` (counter
  transaksi harian, `pengaturan_id_spk_grouping/{yymmdd}`, pola sama
  `generateIdBerurutan()` di `vue-bahan-aksesoris.js`), tulis dokumen
  `spk_grouping` (breakdown per SPK asal, `jalur_aktif`, dst) +
  tandai tiap `order_spk` anggota `id_spk_grouping`/`status_grouping`
  supaya tidak muncul lagi di antrean.
- SPK tanpa SKU/BOM Pola (tidak bisa dicocokkan otomatis) tetap
  ditampilkan terpisah, bisa "Buat Grouping Sendiri" (isi 1 SPK).
- Jalur **Vendor** TIDAK terdeteksi otomatis (belum ada field BOM yang
  jelas buat itu, §5.C RENCANA doc masih terbuka) — checkbox manual
  opsional di form pembuatan grouping.
- Cetak label SPK Grouping pakai `PopupPratinjauCetakLabel` (reuse
  penuh, sama seperti Order SPK/Cetak Label Bahan).
- Split-qty per-KPK (kelipatan) & 5-tahap pipeline nyata (Bahan dulu)
  **BELUM** — itu scope Fase 2 ke atas.

**Validasi**: `node --check` semua file `.js` yang diubah/dibuat — OK.
Tag `<div>` index.html & template literal komponen baru dihitung
skrip (buka=tutup) — seimbang. Brace `firestore.rules` seimbang.
Cross-check manual (grep) `data-target`↔`id`, `petaMount`↔mount
div↔`window.pastikanMountPpDisiapkan`, `data-menu-id`↔`DAFTAR_MENU` —
semua cocok, tidak ada referensi lama yang tersisa (`sub-zh-
persiapanproduksi`, `menu-zevanic-persiapanproduksi-btn`, script tag
lama) selain komentar historis. **BELUM DIUJI di browser+Firebase
sungguhan** (sandbox ini tidak punya browser/Firebase live) — Guru
perlu coba manual: buka sidebar (grup baru harus muncul utk role
admin/pic/owner/superuser), klik tiap 6 sub-menu, coba generate 1 SPK
Grouping dari 2+ SPK aktif yang produk+pola-nya sama, cetak labelnya,
sebelum lanjut ke Fase 2.

**Belum diserahkan ke Guru**: rules baru (`spk_grouping`,
`pengaturan_id_spk_grouping`) belum di-publish ke Firebase Console —
harus dipublish manual sebelum Fase 1 bisa jalan nyata (tanpa itu,
tombol "Buat SPK Grouping" akan gagal permission-denied).

## §44.14 — Fase 2: pipeline 5-tahap jalur Bahan (`spk_track` + `JalurTahapManager`) (29 Agt 2026, malam)

**Instruksi Guru**: *"oke lanjut next step"* setelah ringkasan Fase 1
diserahkan. Ditanya lewat AskUserQuestion (mulai coding Fase 2 vs bantu
publish rules dulu) — dijawab bebas: *"sudah di publish aman"* (rules
Fase 1 sudah dipublish, aman) → dilanjutkan ke Fase 2 sesuai urutan
fase di `RENCANA-PERSIAPAN-PRODUKSI-V2.md` (Bahan duluan, jalur paling
sederhana/representatif buat jadi pola dasar 4 jalur lain).

**Model data baru — koleksi `spk_track`**: 1 dokumen = progres **1 SPK
Grouping di 1 jalur tertentu** (`grouping_id`, `kode_spk`, `nama_produk`,
`qty_total`, `jalur`, `status` — salah satu dari 5 tahap
`perlu_diproses`/`sedang_diproses`/`perlu_dikirim`/`sedang_dikirim`/
`selesai` — `operator_id`, `operator_nama`, `kode_bagging`,
`kode_tugas`, `riwayat_scan[]`, `catatan_masalah`). Dibuat otomatis
(1 doc per jalur aktif) tiap kali "Buat SPK Grouping"/"Buat Grouping
Sendiri" sukses di layar "Perlu Disiapkan" (Fase 1), lewat fungsi baru
`buatSpkTrackUntukGrouping()` — jadi grouping lama (dibuat sebelum
Fase 2 ini) TIDAK otomatis dapat `spk_track`, cuma yang baru dibuat
setelah kode ini aktif.

**Komponen reusable `JalurTahapManager`** (props `jalur`/`labelJalur`/
`tahap`/`labelTahap`) — 1 instance = 1 kartu tahap, dipasang 5× per
jalur (mount-on-demand, sama pola Config 8-tab), supaya Fase 3 (Acc
Sewing/Webbing/Finishing) tinggal pasang mount baru beda `jalur`, TIDAK
perlu tulis ulang komponen (sama semangat `PersiapanKomponenListManager`
di sistem lama yang direuse lewat prop `tipe`). Query pakai 2 filter
`==` (`jalur`+`status`) — TIDAK butuh index komposit manual (index
otomatis single-field Firestore sudah cukup untuk kombinasi
all-equality).

**Alur scan per tahap** (kamera+jsQR, pola SAMA persis
`vue-scan-persiapan.js`; lookup QR-pribadi-ke-karyawan pola SAMA
`vue-absensi-qr.js` — `id_app` diutamakan, fallback email sebagai id
dokumen `users`):
- **Perlu Diproses → Sedang Diproses**: "Scan Operator" (scan QR
  pribadi karyawan). Gerbang: label SPK Grouping WAJIB sudah dicetak
  dulu di "Perlu Disiapkan" (cek `spk_grouping.label_grouping_dicetak`)
  sebelum operator bisa discan/ditugaskan.
- **Sedang Diproses → Perlu Dikirim**: "Scan Entry" (scan label SPK
  Grouping yang sama). Ada juga "Scan Masalah" (non-blocking, cuma
  catat `catatan_masalah`+riwayat, status TIDAK berubah — belum
  terhubung ke `vue-persiapan-masalah.js`, itu keputusan sepihak
  terpisah, lihat di bawah).
- **Perlu Dikirim → Sedang Dikirim**: cetak "Label Bagging" dulu
  (`{kode_spk}-BAG`, `PopupPratinjauCetakLabel`), baru "Scan Pack".
- **Sedang Dikirim → Selesai**: cetak "Label Tugas" dulu
  (`{kode_spk}-TGS`), lalu "Scan Kirim" (TIDAK ubah status, cuma catat
  riwayat) dan "Scan Sampai" (baru ini yang ubah status ke `selesai`).
- **Selesai**: tampil riwayat scan lengkap (siapa+kapan tiap aksi).

**Keputusan sepihak (KEPUTUSAN GURU BELUM DITANYA, dicatat di komentar
kode `js/vue-persiapan-produksi-v2.js` biar gampang dikoreksi)**:
1. Yang discan tiap aksi (kecuali Scan Operator) adalah **label fisik
   batch** (SPK Grouping/Bagging/Tugas) — BUKAN item/roll individual
   seperti `vue-scan-persiapan.js` — karena jalur Bahan kerjanya per-
   batch gabungan (1 SPK Grouping = 1 hamparan kain), bukan per-item.
2. "Scan Kirim" TIDAK mengubah status (cuma riwayat) — cuma "Scan
   Sampai" yang mengubah ke `selesai`; keduanya dilakukan di layar yang
   SAMA (belum ada layar "penerima" terpisah untuk Fase 2).
3. "Scan Masalah" non-blocking, belum terintegrasi ke koleksi
   `persiapan_masalah` yang sudah ada — cuma dicatat lokal di
   `spk_track.catatan_masalah`+riwayat.

**File diubah/dibuat (Fase 2 saja, di atas Fase 1)**:
- `js/vue-persiapan-produksi-v2.js` — tambah `buatSpkTrackUntukGrouping()`,
  `cariKaryawanByQr()`, `muatJsQr()`, komponen `JalurTahapManager` penuh
  (setup+template), `buatAppJalurTahap()` (factory wrapper), 5 fungsi
  `window.pastikanMountPpBahanXxx` (PerluDiproses/SedangDiproses/
  PerluDikirim/SedangDikirim/Selesai). `PersiapanDisiapkanManager.
  buatGrouping()`/`buatGroupingSendiri()` (Fase 1) diedit supaya ikut
  memanggil `buatSpkTrackUntukGrouping()` setelah grouping tersimpan.
- `index.html` — 5 div placeholder statis `sub-pp-bahan-*` (dulu
  "segera hadir, Fase 2") diganti 5 div mount (`vue-pp-bahan-
  perludiproses` dst).
- `js/dashboard.js` — `petaMount` tambah 5 entry baru (`sub-pp-bahan-
  perludiproses` → `pastikanMountPpBahanPerluDiproses`, dst, 1:1 per
  tahap).
- `firestore.rules` — match block baru `spk_track` (pola SAMA:
  `login()` baca, `isAdminLevel()` tulis — konsisten semua koleksi
  Zevanic House lain, termasuk `persiapan_komponen` yang juga ditulis
  lewat aksi scan operator di lapangan; akun operator lapangan pakai
  role `pic`/`admin`, bukan role `operator` mentah).

**Validasi**: `node --check` ulang SEMUA file `.js` yang disentuh (5
file) — OK. Template literal `JalurTahapManager` dicek seimbang
(skrip stack-based per-tag, bukan cuma hitung `<`/`>`) — 0 tag
menggantung. Brace `firestore.rules` 92=92 — seimbang (naik dari
90=90 Fase 1, +1 match block). Cross-check manual (grep): 5 mount-div-
id di `index.html` ↔ 5 `document.getElementById(...)` di `window.
pastikanMountPpBahanXxx` ↔ 5 key `petaMount` — semua cocok persis,
tidak ada typo. `data-target` 5 tombol tahap ↔ 5 `id` div konten —
cocok. `petaTabIndukPerGrup` sudah punya entry `sub-pp-bahan-tahap`
dari Fase 1 (tidak perlu tambahan).

**BELUM DIUJI di browser+Firebase sungguhan** (sandbox ini tidak punya
browser/Firebase live) — murni validasi statis (syntax+cross-
reference), BUKAN bukti kode jalan benar saat dipakai nyata. Guru
perlu uji manual sebelum percaya penuh: generate 1 SPK Grouping baru
(bukan yang lama) dengan jalur Bahan aktif, buka "Persiapan Produksi >
Bahan > Perlu Diproses", coba "Scan Operator" pakai QR karyawan asli,
lanjut tiap tahap sampai "Selesai".

**Belum diserahkan ke Guru**: rules baru koleksi `spk_track` BELUM
di-publish ke Firebase Console — ini KOLEKSI BARU yang tidak termasuk
publish "sudah di publish aman" sebelumnya (itu cuma cover
`spk_grouping`+`pengaturan_id_spk_grouping` dari Fase 1). Tanpa publish
ulang, SEMUA aksi scan di Fase 2 (Bahan) akan gagal
permission-denied — WAJIB dipublish dulu sebelum Fase 2 bisa dicoba.

Fase 3 (3 jalur Acc — Sewing/Webbing/Finishing, pakai ulang
`JalurTahapManager` yang sama, cuma beda `jalur`) & Fase 4 (Vendor,
masih ada pertanyaan arsitektur terbuka §5.C/§5.D RENCANA doc) belum
dimulai.

## §44.15 — Bug boros: Antrean Absensi tarik seluruh koleksi `users` (ketemu Guru: "942 orang") (29 Agt 2026)

**Laporan Guru**: *"tapi loading di antrean absensi sampai tarik data
942 orang. apakah ditarik semua> paginasi atau lazy tidak diterapkan
dengan kartu baru desain baru??"* — dicek kode dulu (bukan tebak2,
sesuai `PEDOMAN-GAYA-KERJA.md`), BUKAN cuma dijawab dari ingatan.

**Temuan (dari baca kode langsung, `js/vue-antrean-absensi.js` fungsi
`muat()`)**: redesain kartu §44.5 ("kartu baru desain baru") TERBUKTI
**bukan** penyebabnya — komentar di §44.5 sudah menegaskan "logic
Firestore/validasi TIDAK berubah", dan dikonfirmasi di sini: kartu baru
cuma ubah tampilan, bukan cara ambil data. Query utama absensi
(`where("ada_pending","==",true)` / `where("status_acc","==",
"PENDING")`, sejak 18 Agt 2026) SUDAH tertarget, bukan fetch-semua.

**Penyebab sesungguhnya ketemu di baris LAIN** yang sudah ada sejak 19
Agt 2026, TIDAK disentuh redesain: baris `if (adaYangBelumPunyaJP) {
getDocs(collection(db,"users")) }` — begitu ADA SATU SAJA dokumen
absensi pending yang belum punya field `jenis_pekerjaan` (dokumen dari
SEBELUM `vue-camera.js` mulai menitip field ini, 19 Agt 2026), kode ini
narik SELURUH koleksi `users` tanpa filter — bukan cuma karyawan aktif
(~90-100 orang), tapi SEMUA dokumen di koleksi itu (kemungkinan
termasuk akun resign/ditolak yang belum dibersihkan — cocok kalau
`users` sekarang total 942 dokumen, jauh di atas jumlah karyawan aktif
sebenarnya — **Guru tolong konfirmasi jumlah dokumen `users` di
Firebase Console buat pastikan ini benar-benar match**, saya tidak
punya akses Firestore live dari sandbox ini).

**Perbaikan**: baris itu diganti — bukan fetch semua `users`, tapi
kumpulkan HANYA email dari dokumen pending yang benar-benar butuh
(belum punya `jenis_pekerjaan`), dedupe, lalu `where("email","in",
[...])` dipotong 30 email per query (batas Firestore utk klausa `in`).
Pola SAMA PERSIS seperti migrasi shift di `js/vue-riwayat-absensi.js`
(sudah lama dipakai di sana, tinggal dicontek). Begitu semua dokumen
pending lama ini habis di-ACC/Reject, query `users` ini otomatis
berhenti total (array `emailPerluJP` kosong) — sama seperti niat asli
komentar 19 Agt 2026, cuma implementasinya yang tadinya masih bolong.

**File diubah**: `js/vue-antrean-absensi.js` (fungsi `muat()`) —
`node --check` OK.

**Belum diverifikasi**: sandbox ini tidak bisa buka browser/Firebase
live, jadi belum bisa buktikan langsung angka baca turun dari 942 ke
sekecil apa. Guru perlu coba manual: buka Antrean Absensi, lihat tab
Network di DevTools atau Firebase Console > Usage, bandingkan jumlah
baca `users` sebelum/sesudah file ini di-update. Kalau ternyata masih
ada 942 di tempat lain (bukan dari baris ini), kabari lagi — jangan
diasumsikan ini satu-satunya sumbernya sampai terbukti dari angka
sungguhan.

**Catatan tambahan (BUKAN bagian perbaikan ini, cuma dicatat biar
tidak lupa)**: kartu `AntreanAbsensiCard` (per kartu, bukan per
halaman) masih query `master_shift` (1x) + `absensi` (1x, cek lembur
approved) per kartu yang dirender — pola N+1 network call, TAPI
masing-masing query tertarget (`where`, biasanya 0-1 dokumen hasil),
bukan fetch-semua — jadi bukan penyebab lonjakan besar seperti bug di
atas, cuma kurang ideal dari sisi jumlah request kalau antrean pending
sedang banyak. Belum diprioritaskan untuk diperbaiki sekarang.

## §44.16 — Koreksi teori §44.15 + audit "hampir semua menu sama" (29 Agt 2026)

**Laporan Guru**: setelah §44.15, buka Antrean Absensi memang sudah
cepat, TAPI banner "Antrean validasi absensi" masih menunjukkan angka
besar (945), dan diminta cek apakah pola yang sama ada di menu lain
("hampir semua menu sama, jadi loading berat").

**KOREKSI teori §44.15** (jujur diakui, bukan dibiarkan salah tercatat):
dugaan sebelumnya "942 = jumlah dokumen koleksi `users`" **KEMUNGKINAN
BESAR KELIRU**. Angka 945 di banner itu `daftarPendingTersaring.length`
— HITUNGAN ASLI dokumen `absensi` yang statusnya masih PENDING (belum
di-Accept/Reject), BUKAN dari query `users`. Dicek ulang: query utama
(`where("ada_pending","==",true)` / `where("status_acc","==",
"PENDING")`) sudah tertarget sejak 18 Agt 2026, dan fungsi Accept/
Reject (`proses`/`prosesMasuk`/`prosesKeluar`) TERBUKTI benar
membalik `status_acc`/`ada_pending` begitu diproses. Root cause angka
besarnya: **`js/vue-camera.js` menulis SETIAP absensi baru (Clock In
DAN Clock Out) dengan `status_acc:"PENDING"`/`ada_pending:true` by
default** — jadi kalau antrean ini belum rutin diproses habis oleh
Admin/PIC, jumlahnya WAJAR menumpuk cepat (~90-100 karyawan × 2 event/
hari). 945 kemungkinan besar backlog SUNGGUHAN, bukan bug baca
berulang. **Perbaikan §44.15 (query users bertarget) tetap valid &
tetap dipertahankan** (itu penyebab kenapa BUKAAN halamannya jadi
cepat), cuma bukan penjelasan yang tepat untuk ANGKA di banner itu
sendiri — dua hal berbeda. Kalau Guru rasa 945 ini TIDAK masuk akal
(kejauhan dari volume kerja nyata), tolong sampel beberapa item
terlama di antrean itu buat dicek manual — saya tidak punya akses
Firestore live buat verifikasi sendiri.

**Audit "hampir semua menu sama"** — grep semua `getDocs(collection(
db, ...))` TANPA `where()`/`limit()` di seluruh `js/*.js`, dicek satu-
satu apakah otomatis jalan tiap buka halaman atau di belakang tombol
manual:

1. **`js/vue-antrean-lembur.js` (Antrean Lembur) — BUG SAMA PERSIS
   dengan §44.15, DIPERBAIKI sekarang.** Baris 205 juga
   `getDocs(collection(db,"users"))` full-fetch, gerbang kondisi IDENTIK
   (`adaYangBelumPunyaJP`) — kemungkinan besar file ini dulu ditulis
   nyontek `vue-antrean-absensi.js` sebelum yang itu diperbaiki. Diganti
   pola sama: `where("email","in",[...])` dipotong 30. `node --check` OK.
2. **`js/vue-penjadwalan.js` (Penjadwalan)** — full-fetch `users` OTOMATIS
   tiap `muat()` (dipanggil `onMounted`), TANPA gerbang kondisi sama
   sekali (BUKAN cuma fallback kayak kasus di atas — SELALU jalan).
   **SUDAH DIKETAHUI sejak 17 Agt 2026** (`PRINSIP-HEMAT.md`/
   `PETA-HEMAT.md`, daftar "belum" paginasi) — bukan temuan baru,
   BELUM diperbaiki. Kalau `users` memang koleksinya besar, ini
   kandidat kuat berikutnya buat dibenahi.
3. **`js/vue-device-kiosk.js` (Device Kiosk)** — full-fetch `users`
   OTOMATIS tiap `muat()`, TANPA `where()`. **KEPUTUSAN SADAR** (bukan
   kelalaian) — komentar kode 23 Agt 2026 secara eksplisit bilang ini
   sengaja menghindari kerumitan Security Rules (`isKiosk()` pakai
   `get()`) dengan menyamai pola Master Kendaraan yang terbukti aman,
   trade-off "baca lebih besar" vs "rules lebih simpel" SUDAH pernah
   ditimbang. Tidak diubah sekarang — perlu keputusan ulang dari Guru
   kalau mau dirombak (bukan sekadar bug untuk ditambal).
4. **`js/vue-reimburse.js` — 2 tempat**:
   - `AntreanReimburseManager.muat()` (baris ±615): full-fetch
     `reimburse` TAPI CUMA untuk role Owner (Admin Finance/PIC tetap
     pakai `where("tahap",...)` tertarget) — **KEPUTUSAN SADAR**,
     komentar kode bilang sengaja "diagnostik lihat semua" buat Owner.
   - `RiwayatReimburseManager`/mode Bensin/Servis `muat()` (baris ±1073):
     full-fetch `reimburse` OTOMATIS tiap `muat()`, **SEMUA role**
     (bukan cuma Owner), **TANPA batas tanggal/paginasi sama sekali** —
     koleksi ini akan terus tumbuh (setiap reimburse/bensin/servis yang
     pernah diajukan, dari awal aplikasi jalan), jadi ini kandidat kuat
     lain yang belum dibenahi. Beda dari `vue-riwayat-absensi.js` yang
     SUDAH pakai filter rentang tanggal server-side — pola itu belum
     dicontek ke sini.
5. **`js/vue-whatsapp-gateway.js` (WhatsApp Gateway > Monitoring
   Respon)** — full-fetch **SELURUH** `wa_log` OTOMATIS tiap dibuka,
   baru dipotong ke 50 teratas DI CLIENT SETELAH semua terbaca
   (`snap` penuh → sort → `.slice(0,50)`) — paling boros dari semua
   yang ditemukan relatif terhadap yang ditampilkan (baca ribuan
   demi tampilkan 50). **Tidak bisa langsung diganti
   `orderBy()+limit(50)`** — dicek `js/auth.js` (penulis log ini):
   `wa_log` cuma simpan `waktu` sebagai STRING lokal Indonesia
   (`toLocaleString('id-ID')`), TIDAK ada Timestamp asli sama sekali.
   Perlu perbaikan skema dulu (tambah `waktu_ts: serverTimestamp()` pas
   nulis, pola SAMA PERSIS preseden `absensi.waktu_ts` 18 Agt 2026 +
   alat migrasi 1x-jalan buat dokumen lama) sebelum bisa
   `orderBy(waktu_ts,"desc")+limit(50)` — bukan tambal 1 baris, perlu
   keputusan & kerja terpisah.
6. **Sudah AMAN (dicek, bukan diasumsikan)**: `js/vue-riwayat-absensi.js`
   (2 full-fetch `absensi` yang ada CUMA di balik tombol manual "Cek
   Data Belum Migrasi", bukan otomatis — query utama layarnya sendiri
   sudah filter rentang tanggal server-side) dan
   `js/vue-antrean-absensi.js` (tombol "Cek Data Sangat Lama" juga
   manual). Semua full-fetch collection MASTER DATA kecil (
   `master_gudang`, `master_shift`, `master_kendaraan`,
   `master_bahan_aksesoris`, `master_suplayer`, `akses_config`,
   `alias_pembelian`, `master_produk`, `master_rak_penyimpanan`,
   `pendaftaran_pending`) dianggap WAJAR (tabel master biasanya cuma
   puluhan baris) — TIDAK diaudit ulang satu-satu isinya, cuma
   dipastikan bukan koleksi yang terus tumbuh besar seperti
   `absensi`/`users`/`reimburse`/`wa_log`.

**Ringkasan prioritas (urutan risiko, BUKAN keputusan — Guru yang
pilih)**: (a) `wa_log` Monitoring Respon — paling boros relatif
terhadap manfaat (baca ribuan tampilkan 50), tapi perlu migrasi
skema dulu; (b) `reimburse` Riwayat (Bensin/Servis/Reimburse) — tidak
ada batas sama sekali, terus tumbuh, semua role kena; (c) Penjadwalan
`users` — sudah lama diketahui, belum genting di skala sekarang; (d)
Device Kiosk `users` — trade-off sadar, biarkan kecuali Guru mau
rombak Security Rules-nya juga.

**File diubah**: `js/vue-antrean-lembur.js` (fungsi `muat()`, poin #1
di atas) — `node --check` OK. Belum ada file lain yang diubah di
putaran ini (poin #2-5 masih laporan, MENUNGGU arahan Guru soal mana
yang mau dikerjakan & urutannya).

## §44.17 — Perbaiki poin #2-5 sekaligus, skala asli 500 karyawan (86rb read/hari) (29 Agt 2026)

**Konteks penting BARU dari Guru**: skala sebenarnya **500 karyawan
aktif** (bukan ~90-100 seperti asumsi lama di `PRINSIP-HEMAT.md`/
`PETA-HEMAT.md` sejak 17 Agt — dokumen itu PERLU direvisi asumsi
skalanya, dicatat sebagai utang, belum sempat diedit di putaran ini),
dan **hari ini saja tercatat 86.000 read Firestore**. Instruksi Guru:
*"semua sekaligus perbaiki dan hemat data, percepat loading boleh
dimuat sebagian, muat lagi, muat lagi secara bertahap queue tapi
irit"* — greenlight kerjakan poin #2-5 dari §44.16 sekaligus, dengan
pola "muat sebagian dulu, tombol Muat Lagi" di mana relevan.

**Poin #2 — Penjadwalan (`js/vue-penjadwalan.js`)**: **TIDAK diubah
jadi paginasi cursor** — DIPERIKSA DULU (bukan tebak2): layar ini
SECARA FUNGSI butuh SELURUH dataset karyawan aktif sekaligus, bukan
cuma 1 halaman — kartu ringkasan per gudang, "Pilih Semua" + Update
Massal, Export/Import Excel SEMUANYA beroperasi di atas
`hasilFilter` (SELURUH hasil filter, lintas banyak dimensi: nama,
status terjadwal, jenis pekerjaan, gudang, shift, hari libur
sekaligus). Memotong ini jadi paginasi cursor akan MERUSAK 3 fitur
itu (kartu ringkasan salah hitung, "Pilih Semua" cuma pilih 1 halaman,
Export Excel cuma expor 1 halaman) — bukan sekadar tebalan 1 baris,
perlu redesain UX (mis. `getCountFromServer()` per kartu, bulk-update
server-side terpisah dari tampilan tabel) kalau mau paginasi
sungguhan di sini. **Yang DIPERBAIKI (aman, tanpa kurangi fitur
apapun)**: tambah `where("status_kerja","==","Aktif")` ke query
`users` — dulu tarik SEMUA akun (termasuk resign/nonaktif yang
TOH dibuang lagi di JS satu baris di bawahnya), sekarang filter itu
dipindah ke server, jadi akun tidak aktif tidak ikut terbaca sama
sekali. `node --check` OK.

**Poin #3 — Riwayat Reimburse/Bensin/Servis (`js/vue-reimburse.js`,
`RiwayatReimburseTable`)**: DIPERBAIKI PENUH — pola SAMA PERSIS
"Riwayat All Absensi" (§19 Agt 2026): filter rentang tanggal jadi
QUERY SUNGGUHAN (`where("diajukan_pada",">=",X).where("diajukan_pada",
"<=",Y)`, field itu SUDAH Timestamp asli sejak awal — TIDAK perlu
alat migrasi seperti `absensi.waktu_ts`), preset Hari Ini/Kemarin/7
Hari/30 Hari/90 Hari/Custom, DEFAULT **30 Hari Terakhir** (bukan Hari
Ini seperti Absensi — reimburse jauh lebih jarang diajukan daripada
Clock In/Out harian, biar layar tidak sering kelihatan kosong). Cari
nama/plat & Unduh CSV tetap jalan PERSIS sama, cuma beroperasi di
dalam rentang tanggal yang aktif sekarang (bukan lagi seluruh
sejarah). Caption rentang ikut ditulis ke CSV biar jelas cakupannya.
`node --check` OK, template literal dicek seimbang (skrip stack-based
per-tag). `AntreanReimburseManager` (mode Owner "lihat semua"
diagnostik) **TIDAK disentuh** — itu keputusan sadar terpisah
(komentar kode sendiri bilang sengaja), dan cuma dipakai Owner
(1-2 akun), bukan kontributor besar ke 86rb read/hari dari 500 orang.

**Poin #5 — WhatsApp Gateway Monitoring Respon
(`js/vue-whatsapp-gateway.js` + `js/auth.js`)**: DIPERBAIKI PENUH —
ini yang PALING boros dari semua temuan (baca RIBUAN demi tampilkan
50). Langkah: (1) `js/auth.js`, fungsi `kirimPesanWhatsapp` — tiap
`wa_log` BARU sekarang JUGA menulis `waktu_ts: serverTimestamp()`
(field `waktu` teks lama TETAP ada, tidak dihapus, pola SAMA PERSIS
preseden `absensi.waktu_ts` 18 Agt 2026); (2) `muatMonitoring()`
diganti jadi `query(...,orderBy("waktu_ts","desc"),limit(50))`
SUNGGUHAN (bukan fetch-semua-lalu-slice), dengan `muatLagiLog()` baru
("Muat Lagi" — cursor `startAfter`, MENAMBAH ke daftar yang sudah
ada, bukan ganti halaman, sesuai instruksi "muat lagi secara
bertahap"); (3) **keterbatasan jujur**: log dari SEBELUM perbaikan
ini tidak punya `waktu_ts`, otomatis TIDAK ikut di query
`orderBy(waktu_ts)` (perilaku baku Firestore: dokumen yang field
urutnya kosong dikecualikan) — jadi log lama akan "hilang" dari
tampilan utama. Ditambahkan tombol terpisah **"Lihat Log Sebelum
Pembaruan"** (fetch-semua manual, SEKALI diklik, pola SAMA PERSIS
"Cek Data Sangat Lama") supaya log lama tetap bisa dilihat kalau
perlu, tanpa membebani pemuatan normal. `node --check` OK, template
dicek seimbang.

**Poin #4 — Device Kiosk (`js/vue-device-kiosk.js`)**: **SENGAJA
TIDAK disentuh** di putaran ini (bukan lupa) — full-fetch `users` di
sini adalah KEPUTUSAN SADAR bertanggal 23 Agt 2026 (dites, query
`where("jenis_akun","==","kiosk")` TERBUKTI bermasalah gara-gara
aturan `isKiosk()` di `firestore.rules` pakai `get()`, direvert
sengaja demi keamanan yang terbukti jalan). Risiko mengubahnya lagi
tanpa bisa dites live (sandbox ini tidak ada browser/Firebase
sungguhan) lebih besar daripada manfaatnya — dampak baca-nya sendiri
kecil (layar KHUSUS OWNER, sudah mount-on-demand, dibuka jarang,
bukan oleh 500 karyawan). Kalau Guru mau ini tetap dibenahi, perlu
sesi terpisah yang bisa dites live di Firebase Console, bukan tebakan
dari sandbox ini.

**Ringkasan hasil putaran §44.15-44.17**: 4 dari 5 temuan boros
sudah diperbaiki (Antrean Absensi, Antrean Lembur, Riwayat Reimburse,
WhatsApp Log), 1 diperbaiki sebagian aman tanpa redesain
(Penjadwalan — filter status_kerja server-side), 1 sengaja
dipertahankan dengan alasan jelas (Device Kiosk).

**BELUM DIUJI di browser+Firebase sungguhan** untuk SEMUA perbaikan
poin #2/3/5 di atas (sandbox ini tidak punya browser/Firebase live) —
Guru WAJIB coba manual sebelum percaya penuh, terutama: (a) query
baru mungkin minta INDEX FIRESTORE BARU (link otomatis muncul di
Console browser kalau kena, bukan bug) untuk `reimburse`
(`diajukan_pada` range) dan `wa_log` (`waktu_ts` orderBy) — belum
tentu perlu, tapi WAJIB dicek; (b) `users` composite index untuk
`status_kerja=="Aktif"` di Penjadwalan kemungkinan besar TIDAK perlu
index baru (single equality field, index otomatis Firestore) tapi
tetap wajib dicoba; (c) pastikan hasil Riwayat Reimburse & Monitoring
WA benar-benar tampil sesuai yang diharapkan, bukan cuma tidak error.

**Utang tercatat (BELUM dikerjakan)**: `PRINSIP-HEMAT.md`/
`PETA-HEMAT.md` masih tertulis asumsi "~90-100 karyawan" di beberapa
tempat — perlu direvisi ke skala asli (500 karyawan) supaya keputusan
desain berikutnya (kapan mulai paginasi cursor sungguhan, dst) tidak
salah kalibrasi lagi.

## §44.18 — Bug N+1 per-kartu (jam shift & cek Lembur) di Antrean Absensi/Lembur, jawab pertanyaan Guru "1 data dipakai 2 menu, bisa dihemat?"

**Konteks**: setelah §44.17 dikirim, Guru bertanya: *"apa bisa hemat misal 1 data dipakai 2 menu, misal antrean kehadirna dan riwayat al absensi. bisa hematkan"*. Sebelum menjawab, dibaca ulang kode `vue-antrean-absensi.js` dan `vue-riwayat-absensi.js` SUNGGUHAN (bukan dari ingatan) — sesuai aturan "jangan bikin tebak2".

**Temuan soal pertanyaan aslinya (lintas-menu, Antrean Absensi vs Riwayat All Absensi)**: kedua menu memang bisa membaca dokumen `absensi` yang SAMA di hari yang sama (Antrean cari yang `PENDING`, Riwayat cari rentang tanggal hari ini) — TAPI ini 2 bentuk query yang beda (field beda: `ada_pending`/`status_acc` vs `waktu_ts` rentang). Firestore menagih PER QUERY berdasarkan dokumen yang dibaca query itu, bukan berdasarkan "pernah dibaca di menu lain sebelumnya" — jadi TIDAK ADA cara aman untuk benar-benar menyatukan baca dua query yang bentuknya beda tanpa risiko data basi (approve di Antrean tidak langsung kelihatan di Riwayat kalau dipaksa pakai cache). Kesimpulan jujur: berbagi cache lintas-query seperti ini TIDAK direkomendasikan untuk data approval yang sensitif — potensi hemat kecil, risiko bug besar.

**Tapi ketemu bug yang JAUH lebih besar saat baca ulang** (soal semangat yang sama — "1 data dipakai berkali-kali, boros"): di **DALAM 1 menu yang sama** (Antrean Absensi & Antrean Lembur), TIAP KARTU pending memanggil `getDocs()` **sendiri-sendiri** begitu di-mount:
- `muatJamShift()` — query `master_shift` cari jam berdasarkan `nama_shift` kartu itu.
- `cekLemburHariItu()` (cuma di Antrean Absensi) — query `absensi` cari Lembur ter-ACC buat email kartu itu.

Ini pola N+1 klasik: kalau ada 200 kartu pending dan semuanya shift "Pagi", itu 200 query Firestore yang **identik** buat 1 baris jawaban yang sama. Firestore menagih minimal 1 read per query walau hasilnya kosong — jadi di skala 500 karyawan, kalau antrean menumpuk (misal 100-300 kartu belum diproses), ini bisa jadi 200-600 read TAMBAHAN cuma dari 2 fungsi ini, di LUAR baca daftar utamanya.

**Perbaikan**: dipindah dari kartu (child component) ke induk (`AppAntreanAbsensi`/`AppAntreanLembur`, fungsi `muat()`):
- Kumpulkan `nama_shift` unik dari seluruh daftar pending → 1 (atau beberapa, dipotong 30) query `where("nama_shift","in",[...])` ke `master_shift` buat SEMUA kartu sekaligus.
- (Antrean Absensi saja) Kumpulkan email unik dari daftar pending → 1/beberapa query `where("email","in",[...]).where("status","==","LEMBUR (CLOCK IN)").where("status_acc","==","ACC")` ke `absensi` buat SEMUA kartu sekaligus, hasilnya jadi peta `email -> [tanggal lembur ter-ACC]`.
- Kedua peta hasil dikirim ke tiap kartu lewat prop (`shiftInfo`, `lemburTanggal`) — kartu (child) SEKARANG murni `computed()` dari prop, TIDAK ada `getDocs()` sama sekali di dalam kartu.

**File diubah**: `js/vue-antrean-absensi.js`, `js/vue-antrean-lembur.js`. `node --check` lolos kedua-duanya; script pengecek keseimbangan tag HTML template (custom, lihat catatan §44.17) juga lolos (0 error) untuk kedua template di kedua file.

**BELUM DIUJI di browser+Firebase sungguhan.** Perlu diperhatikan:
- Query `where("nama_shift","in",[...])` dan `where("email","in",[...]).where(...).where(...)` — kombinasi `in` + 2 kesetaraan lain PERNAH jalan sebelumnya dalam bentuk `where("email","==",...)` tunggal (per-kartu), jadi kemungkinan besar tidak butuh index composite baru (semua klausa kesetaraan/`in`, bukan rentang), tapi INI BUKAN KEPASTIAN — kalau Firestore minta index baru, akan muncul link di Console/error pertama kali dipakai live.
- Perilaku FUNGSIONAL kartu (jam shift tampil, badge "Lembur" muncul di Clock Out) TIDAK dimaksudkan berubah sama sekali — cuma SUMBER datanya yang pindah dari query-per-kartu ke peta-dari-induk. Perlu dicek visual sekali di HP/browser sungguhan untuk pastikan tidak ada regresi tampilan.

**Belum disentuh / masih ada peluang serupa**: pola ini (query per-baris di dalam kartu/komponen anak) BELUM disisir di semua menu lain — kalau Guru mau, bisa diaudit lagi khusus untuk pola N+1 ini di menu lain (di luar 5 layar yang sudah dibenahi §44.15-44.18).

## §44.19 — Fase 3: 3 jalur Acc (Sewing/Webbing/Finishing) (29 Agt 2026, malam)

**Instruksi Guru**: *"klw sudah hemat lanjut ke menu pesiapan fase lainnya"*
— setelah §44.18 (perbaikan N+1) selesai dikirim, lanjut sesuai urutan
fase di `RENCANA-PERSIAPAN-PRODUKSI-V2.md` §7. Fase berikutnya yang siap
dikerjakan (tidak terblokir pertanyaan terbuka) adalah **Fase 3 — 3 jalur
Acc (Sewing/Webbing/Finishing)**; Fase 4 (Vendor) masih terblokir 2
pertanyaan arsitektur ke Guru (§5.C/§5.D RENCANA doc — field BOM utk
"Vendor" & siapa yang scan), jadi TIDAK dikerjakan sesi ini.

**Sesuai perkiraan di rencana §7 poin 3 ("state machine-nya sama persis
dengan Fase 2, tinggal parametrisasi jalur") — TERBUKTI BENAR**: Fase 3
**TIDAK menulis komponen baru sama sekali**. Komponen `JalurTahapManager`
+ factory `buatAppJalurTahap(jalur, labelJalur, tahap, labelTahap)`
(Fase 2, `js/vue-persiapan-produksi-v2.js`) dipakai APA ADANYA, cuma
parameter `jalur`/`labelJalur` diganti (`sewing`/"Acc Sewing",
`webbing`/"Acc Webbing", `finishing`/"Acc Finishing") — collection
`spk_track` yang sama, rules Firestore yang sama (sudah generic, tidak
peduli nilai `jalur`), mekanisme scan (kamera+jsQR) yang sama persis.

**Yang ditambahkan (murni wiring, bukan logic baru)**:
- **15 mount function baru** (3 jalur × 5 tahap) di
  `js/vue-persiapan-produksi-v2.js`, ditulis eksplisit satu-satu (bukan
  loop — konsisten gaya kode proyek ini, gampang di-grep 1:1) —
  `window.pastikanMountPpSewingPerluDiproses` dst.
- **`index.html`**: 15 div placeholder statis "Logic tahap ini belum
  dibangun" (Sewing/Webbing/Finishing × 5 tahap) diganti 15 div mount
  (`vue-pp-sewing-perludiproses` dst) — pola SAMA PERSIS dengan yang
  sudah dipakai jalur Bahan. Placeholder Vendor (5 sisanya) **SENGAJA
  DIBIARKAN** — Fase 4 belum dikerjakan.
- **`js/dashboard.js`**: `petaMount` tambah 15 entry baru, 1:1 ke 15
  mount function di atas. `petaTabIndukPerGrup` TIDAK perlu diubah —
  entry `sub-pp-sewing-tahap`/`sub-pp-webbing-tahap`/
  `sub-pp-finishing-tahap` sudah ada sejak Fase 1.
- **`js/vue-config-akses.js`**: TIDAK diubah — menu-id `pp_sewing`/
  `pp_webbing`/`pp_finishing` sudah dibuat di Fase 1, `JalurTahapManager`
  otomatis pakai `menuId = 'pp_' + jalur` jadi langsung cocok tanpa
  perubahan.
- **`firestore.rules`**: TIDAK diubah — rules `spk_track` generic
  (tidak bercabang per nilai `jalur`), sudah dipublish sejak Fase 2.
  **Tidak ada rules/index baru yang perlu dipublish untuk Fase 3 ini.**

**Validasi**: `node --check` OK untuk `js/vue-persiapan-produksi-v2.js`
dan `js/dashboard.js`. Cross-check manual (grep) — 3 rangkaian dicocokkan
1:1: (a) 15 `id="vue-pp-..."` di `index.html`, (b) 15
`getElementById('vue-pp-...')` di `js/vue-persiapan-produksi-v2.js`, (c)
15 key `petaMount` + 15 value fungsinya di `js/dashboard.js` — SEMUA
cocok persis, tidak ada typo/orphan. Keseimbangan tag `<div>` dicek per
blok jalur (Python, hitung buka=tutup) — Sewing/Webbing/Finishing
masing-masing 12=12, seimbang. Dikonfirmasi juga placeholder "Logic
tahap ini belum dibangun" yang tersisa di `index.html` PERSIS 5 (semua
milik Vendor, bukan sisa Sewing/Webbing/Finishing yang lupa diganti).

**BELUM DIUJI di browser+Firebase sungguhan** (sandbox ini tidak punya
browser/Firebase live) — murni validasi statis. Guru perlu uji manual:
buat 1 SPK Grouping baru dengan produk yang BOM Aksesoris-nya
`tahap_proses` Sewing/Webbing/Finishing terisi (supaya `jalur_aktif`
otomatis mencakup jalur itu), buka menu Persiapan Produksi > Acc Sewing
(atau Webbing/Finishing) > Perlu Diproses, coba jalankan sampai Selesai
persis seperti alur Bahan yang sudah divalidasi konsepnya di §44.14.
Karena TIDAK ada rules/collection baru, risiko kegagalan permission-
denied jauh lebih kecil dari Fase 1/2 (yang sempat butuh publish rules
manual) — tapi tetap wajib dicoba manual sebelum dipercaya penuh.

**Sisa pekerjaan Persiapan Produksi V2**: Fase 4 (Vendor) menunggu
jawaban Guru §5.C/§5.D RENCANA doc. Fase 5 (audit referensi lama) belum
dimulai.

## §44.20 — Fase 4: jalur Vendor (29 Agt 2026, malam)

**Konteks**: setelah Fase 3 (§44.19) dikirim & dikonfirmasi Guru
*"oke sudah diuji aman"*, Guru langsung menjawab pertanyaan terbuka
§5.D RENCANA-PERSIAPAN-PRODUKSI-V2.md: *"vendor yg scan driver yg biasa
belanja, karena dia jg kurir yg kirim dan sampai barang."*

**Yang terjawab**: siapa yang SCAN di jalur Vendor — **driver INTERNAL**
(karyawan yang sudah biasa jalan belanja/kulakan bahan, akun+QR pribadi
yang SAMA seperti karyawan lain, BUKAN akun/portal vendor eksternal
terpisah), karena driver yang sama itu juga jadi kurir yang antar barang
ke vendor DAN jemput baliknya. Ini menjawab §5.D ("apakah alur 5-tahap
tetap sama walau barang fisik keluar lokasi") — **jawabannya YA, tetap
sama**: 5 tahap generic yang sudah ada (Scan Operator → assign driver;
Scan Entry → driver konfirmasi vendor selesai kerja; Scan Pack/Kirim/
Sampai → driver antar balik ke internal) sudah cukup, TIDAK perlu
tahap/label tambahan khusus "barang keluar"/"barang balik".

**Yang MASIH terbuka** (bagian lain §5.C, belum ditanyakan/dijawab):
field baru di BOM Aksesoris Master Produk buat deteksi OTOMATIS jalur
Vendor + field jenis vendor (Sublim/Sablon/Bordir dll, dropdown atau
teks bebas?). **Keputusan: TIDAK menunggu ini** — jalur Vendor sudah
punya jalur AKTIVASI MANUAL yang dibangun sejak Fase 1 (checkbox "+
Jalur Vendor (manual)" di form "Perlu Disiapkan", state `vendorManual`)
yang SUDAH generic terhadap `jalur_aktif` (`buatSpkTrackUntukGrouping()`
tidak peduli nama jalur, `PETA_JALUR.vendor` sudah didefinisikan sejak
awal file ini ditulis) — jadi jalur Vendor BISA langsung diwiring end-to-
end pakai mekanisme yang sudah ada, TANPA perlu skema BOM baru. Deteksi
otomatis dari BOM didokumentasikan sebagai **kenyamanan tambahan buat
nanti**, bukan syarat wajib — supaya tidak menahan Fase 4 cuma karena
1 pertanyaan yang belum krusial buat jalan.

**Implementasi**: PERSIS pola Fase 3 (§44.19) — **TIDAK ADA komponen
baru**. `JalurTahapManager` + `buatAppJalurTahap()` dipakai apa adanya
dengan `jalur='vendor'`, `labelJalur='Vendor'`.
- `js/vue-persiapan-produksi-v2.js` — 5 mount function baru
  (`window.pastikanMountPpVendorPerluDiproses` dst).
- `index.html` — 5 div placeholder "Logic tahap ini belum dibangun"
  (Vendor × 5 tahap, sisa TERAKHIR dari 25 slot placeholder Fase 1)
  diganti 5 div mount (`vue-pp-vendor-perludiproses` dst). **Placeholder
  statis di seluruh menu Persiapan Produksi SEKARANG HABIS (0 sisa)** —
  cek `grep -c "Logic tahap ini belum dibangun" index.html` = 0.
- `js/dashboard.js` — `petaMount` tambah 5 entry baru, 1:1 ke 5 mount
  function di atas. `petaTabIndukPerGrup` & `js/vue-config-akses.js`
  (menu-id `pp_vendor`) TIDAK diubah — sudah ada sejak Fase 1.
- `firestore.rules` — TIDAK diubah, rules `spk_track` generic (sudah
  dipublish sejak Fase 2), tidak ada publish tambahan.

**Validasi**: `node --check` OK (`vue-persiapan-produksi-v2.js`,
`dashboard.js`). Cross-check manual (grep) — 5 `id="vue-pp-vendor-..."`
↔ 5 `getElementById(...)` ↔ 5 key+value `petaMount` — cocok semua persis.
Keseimbangan `<div>` blok Vendor: 12 buka = 12 tutup. Placeholder
"Logic tahap ini belum dibangun" tersisa **0** di seluruh `index.html`
(dulu 5, semuanya milik Vendor — sekarang habis karena Fase 4 ini).

**BELUM DIUJI di browser+Firebase sungguhan.** Guru perlu coba manual:
tandai 1 SPK Grouping baru dengan checkbox "+ Jalur Vendor (manual)"
saat dibuat, buka menu Persiapan Produksi > Vendor > Perlu Diproses,
scan QR pribadi driver yang dimaksud di "Scan Operator", jalankan sampai
Selesai — pastikan driver yang sama bisa dipakai buat Scan Kirim & Scan
Sampai tanpa hambatan (sesuai kata Guru dia yang pegang kedua peran itu).

**Status Persiapan Produksi V2 sekarang**: Fase 1-4 SELESAI (kode, belum
diuji browser+Firebase). Sisa: (a) deteksi otomatis BOM utk jalur Vendor
— enhancement opsional, belum dijadwalkan; (b) Fase 5 (audit referensi
lama) — belum dimulai; (c) "Scan Masalah" belum terhubung ke
`js/vue-persiapan-masalah.js` (keputusan sepihak sejak Fase 2, masih
berlaku di semua jalur termasuk Vendor).

## §44.21 — Fase 5: Audit referensi lama (30 Agt 2026)

Guru konfirmasi "mulai sekarang" = mulai Fase 5 (lewat AskUserQuestion).
Metodologi: `grep -r` seluruh repo cari sisa `persiapan_produksi`/
`persiapan_komponen`/`vue-persiapan-produksi.js`/nama komponen & id
lama, lalu tiap hit DIBACA LANGSUNG (bukan ditebak) & diklasifikasi:
komentar historis aman / shim kompatibilitas sengaja / kode aktif
berbahaya yang kelewat.

**1. Kode aktif YANG DITEMUKAN masih pakai koleksi lama (perlu keputusan
Guru)**: `js/vue-scan-persiapan.js` fungsi `tandaiPersiapanDariScan()`
(baris 144-184), dipanggil dari `simpanPemakaian()` (baris 461) — YAITU
**setiap kali** tombol "Catat Pemakaian" di menu Scan Persiapan dipakai
(fitur LIVE, sudah pernah ditest Guru, sering dipakai). Fungsi ini query
koleksi `persiapan_komponen` (sistem LAMA, sudah ditinggalkan sejak
sebelum Fase 1 V2 dibangun) — karena dokumen di koleksi itu tidak pernah
dibuat lagi, query-nya SELALU kosong, fungsi ini jadi **no-op senyap**
di setiap Catat Pemakaian. Dibungkus try/catch jadi TIDAK menggagalkan
pencatatan pemakaian utama (aman, kartu stok tetap normal) — tapi buang
1 baca Firestore percuma tiap scan, dan komentarnya (baris 144-162)
masih bilang ini "integrasi Persiapan Produksi" padahal sudah mati.
**Belum diubah** — nyentuh kode live yang sudah ditest, jadi ditanyakan
dulu ke Guru: hapus fungsi+pemanggilannya, atau dibiarkan tapi komentar
diperbaiki jadi "sudah tidak aktif, sengaja dibiarkan aman"?

**2. Komentar nyasar ke file lama (kosmetik, resiko rendah)**:
`js/vue-master-produk.js` baris 923 & `js/vue-config.js` baris 109-113
masih sebut `js/vue-persiapan-produksi.js` sebagai pemakai field
`tahap_proses`/koleksi `master_tahap_persiapan` — **DICEK LANGSUNG**:
field `tahap_proses` MEMANG masih dipakai (kini oleh
`js/vue-persiapan-produksi-v2.js` baris 109, bukan bug logic), cuma nama
file di komentar yang ketinggalan zaman. Aman, tapi menyesatkan buat
yang baca kode nanti.

**3. Dikonfirmasi BERSIH** (baca langsung, bukan cuma grep-hit):
`js/vue-config-akses.js` (5 menu-id lama sudah ditandai `deprecated:
true` dgn benar), `js/vue-order-spk.js`, `js/vue-header-mobile.js`,
`js/dashboard.js`, `index.html` — semua cuma nyisa komentar historis,
tidak ada pemanggilan aktif ke sistem lama. `firestore.rules` — block
`match /persiapan_produksi/{docId}` & `match /persiapan_komponen/{docId}`
(baris 239-243) masih ada, sesuai keputusan lama "dibiarkan ada" buat
dokumen lama, bukan bug. 2 file `.txt` referensi
(`firestore-index-tambahan-persiapan-produksi.txt`,
`firestore-rules-tambahan-persiapan-produksi.txt`) dikonfirmasi TIDAK
dirujuk kode manapun — arsip historis aman.

**4. File mati (tidak bahaya, tapi bisa dirapikan)**: `js/vue-persiapan-
produksi.js` (25KB, file LAMA) masih ada di disk tapi **TIDAK dimuat**
`<script>` manapun (dikonfirmasi grep — cuma disebut di komentar sbg
"DITINGGALKAN"). Aman dibiarkan atau dihapus, sepenuhnya kosmetik.

**5. TEMUAN TERBESAR — dokumentasi peta/menu/database TIDAK sinkron,
ada 3 salinan berbeda-beda umurnya** (bagian "rapikan dokumentasi" dari
Fase 5): ditemukan file dokumentasi jenis sama ada di 3 lokasi beda
dengan isi beda:
- Root (`PETA-MENU.md`, `PETA-DESAIN.md`, `PETA-DATABASE.md`,
  `PEDOMAN-GAYA-KERJA.md`) — SEMUA bertanggal 28 Agt 17:02, dari
  **SEBELUM** menu Persiapan Produksi (versi manapun) pernah dibangun
  (0 hit kata `persiapan_produksi`/`persiapan_komponen`).
- `docs-local/` (`PETA-MENU.md`, `PETA-DESAIN.md`, `PEDOMAN-GAYA-
  KERJA.md`) — lebih baru (29 Agt dini hari ~03:00-04:36), tapi masih
  gambarkan sistem Persiapan Produksi versi LAMA (`PersiapanQueueManager`
  /`PersiapanKomponenListManager`, koleksi `persiapan_produksi`/
  `persiapan_komponen`) seolah masih aktif — TIDAK menyebut V2/
  `spk_track`/`JalurTahapManager` sama sekali.
- `PETA-DATABASE-live.md` & `STATUS-PROYEK-live.md` di root — juga
  menggambarkan state sistem lama, juga belum diupdate ke V2.
- `claude/RENCANA-PERSIAPAN-PRODUKSI-V2.md` (yang selama ini disinkron
  ke Project) VS duplikat di root `RENCANA-PERSIAPAN-PRODUKSI-V2.md`
  (29 Agt 12:45, ketinggalan Fase 3 & 4) — duplikat root TIDAK dirujuk
  kode manapun, aman dihapus/diabaikan.
- `PETA-INFRASTRUKTUR.md` ada di root, TAPI daftar dokumen Project
  menyebutnya sebagai `claude/PETA-INFRASTRUKTUR.md` — lokasi tidak
  cocok, belum dicek isi/kecocokannya.
- `claude/FIRESTORE-RULES-SNAPSHOT.md` (terdaftar sbg dokumen Project)
  **TIDAK DITEMUKAN sama sekali** di working copy lokal manapun.
- `PRINSIP-HEMAT.md` (terdaftar sbg dokumen Project, path root) juga
  **TIDAK DITEMUKAN** di working copy lokal.

Kesimpulan poin 5: dokumentasi peta/menu/database sudah lama tidak
disinkronkan dengan kode sungguhan — bukan cuma soal sisa referensi
Persiapan Produksi lama, tapi drift dokumentasi yang lebih luas dari
prinsip proyek ("Update semua knowledge reading setiap waktu di
Claude/"). **Belum ditindaklanjuti** — perlu arahan Guru dulu soal mana
yang jadi sumber kebenaran ke depan (root vs `docs-local/` vs isi asli
di Project claude.ai) sebelum saya konsolidasikan/timpa salah satunya,
supaya tidak salah pilih & malah menghapus versi yang justru dianggap
benar oleh Guru.

**Belum dilakukan**: perbaikan/hapus apapun (poin 1, 2, 4 di atas) —
menunggu arahan Guru dulu, konsisten pola "jangan keputusan sepihak"
buat hal yang nyentuh kode live atau dokumentasi yang statusnya
ambigu.

## §44.22 — Lanjutan Fase 5: eksekusi persetujuan Guru + TEMUAN LEBIH SERIUS dari dugaan awal (30 Agt 2026)

Guru jawab (AskUserQuestion) 2 keputusan §44.21: (a) hapus
`tandaiPersiapanDariScan()` — **SUDAH dieksekusi**: fungsi + 1
pemanggilannya di `simpanPemakaian()` dihapus dari
`js/vue-scan-persiapan.js`, import `doc`/`updateDoc` yang jadi nganggur
ikut dibuang, `node --check` OK. Komentar nyasar di
`js/vue-master-produk.js`/`js/vue-config.js` (poin 2 §44.21) juga
dikoreksi sekalian (nama file lama → `vue-persiapan-produksi-v2.js`).
(b) "rapikan dokumentasi, root jadi sumber kebenaran" — **SEBAGIAN
dieksekusi, TAPI ketemu masalah lebih besar dari yang saya kira waktu
tanya**:

Saya pakai 2 agent buat baca kode langsung & tambahkan bagian "Zevanic
House" (menu manufaktur/gudang yang sama sekali belum tercatat) ke
`PETA-MENU.md` (+2 section baru: Zevanic House, Persiapan Produksi V2),
`PETA-DATABASE.md` (13→36 koleksi), dan `PETA-DESAIN.md` (248→448
baris CSS, +50 class baru). **SUDAH disinkron ke Project.**

**TAPI** — pas iseng `diff` root vs `docs-local/` SETELAH update di
atas selesai, ketemu: `docs-local/PETA-MENU.md` & `docs-local/PETA-
DESAIN.md` (dan `PETA-DATABASE-live.md` di root) ternyata BUKAN cuma
"versi lebih tua yang boleh diabaikan" seperti saya duga di §44.21 —
mereka punya **RATUSAN BARIS KONTEN UNIK** yang TIDAK ADA di root sama
sekali, dan bukan soal Persiapan Produksi/Zevanic House, tapi soal
**redesain mobile besar (§27→§27.2, lalu §44 "Gechoo Mobile Organic")**:
Beranda dirombak total (Kartu Statistik, Favorit Saya, grup kategori
per-user), Profile ganti dari drawer ke Bottom Sheet (`vue-sheet-
profil.js`, `vue-profile-drawer.js` DIHAPUS), Menu Lengkap & Atur
Favorit (2 layar baru), notifikasi lonceng di header (ganti Pengumuman
Carousel), Mode Gelap — **SEMUA ini kode SUDAH ADA & sudah dikirim ke
Guru**, tapi TIDAK PERNAH sampai tercatat di `PETA-MENU.md`/`PETA-
DATABASE.md`/`PETA-DESAIN.md` ROOT (yang disinkron ke Project) —
**HANYA** ada di `docs-local/`/`*-live.md`. Angka baris beda jauh: `diff
docs-local/PETA-MENU.md PETA-MENU.md` (root, SETELAH update Zevanic
House saya) masih nunjukkan **116 baris cuma ada di docs-local**;
`PETA-DESAIN.md` **145 baris**; `PETA-DATABASE-live.md` vs root **459
baris**.

**Kesimpulan**: dokumentasi bukan cuma "ketinggalan 1 arah" (root
ketinggalan Zevanic House SAJA) seperti saya duga & tanyakan ke Guru
sebelumnya — ini 2 CABANG dokumentasi yang SAMA-SAMA punya konten unik
penting yang TIDAK PUNYA satu sama lain (root ketinggalan redesain
mobile §27-§44; `docs-local` ketinggalan Zevanic House DAN Persiapan
Produksi V2 total). Auto-merge/timpa salah satu SEKARANG berisiko
BENAR-BENAR MENGHILANGKAN dokumentasi kerja nyata yang sudah dikirim ke
Guru — ini persis situasi "jangan bikin tebak2" yang project ini minta
dihindari, jadi **BERHENTI DULU** di titik ini, TIDAK saya lanjutkan
otomatis. `PEDOMAN-GAYA-KERJA.md` root vs `docs-local` juga sama
pola-nya (85 baris cuma di root — termasuk tips efisiensi reasoning-
effort/tool-use — VS ~130 baris cuma di `docs-local` — termasuk ATURAN
PENTING "kapan nulis ke dokumen Claude/" & **struktur folder pengiriman
file yang BEDA dari yang saya pakai sepanjang sesi ini**: `F:\ZEVANIC
HOUSE\FOUNDATION` buat dokumen peta LANGSUNG lewat `device_commit_files`
[BUKAN zip], vs sub-folder `Data Yang Disiapkan` khusus file kode). Ini
KEMUNGKINAN BESAR relevan ke cara kerja sesi-sesi Claude berikutnya,
bukan cuma soal isi dokumentasi — perlu diklarifikasi Guru duluan
sebelum dipakai/dipercaya.

**Belum dilakukan**: penggabungan/pemilihan pemenang PETA-MENU.md,
PETA-DATABASE.md, PETA-DESAIN.md, PEDOMAN-GAYA-KERJA.md antara root vs
`docs-local`/`*-live.md`. Root SUDAH lebih baik dari sebelumnya (Zevanic
House & Persiapan Produksi V2 sekarang tercatat), tapi BELUM lengkap
(masih hilang seluruh cerita redesain mobile §27-§44). Menunggu arahan
Guru: mana yang jadi acuan gabungan ke depan, dan apakah folder
`F:\ZEVANIC HOUSE\FOUNDATION` (bukan zip) itu memang cara pengiriman
yang seharusnya dipakai sesi ini.

## §44.23 — Fase 5 SELESAI: penggabungan dokumentasi + Fase 5 ditutup (30 Agt 2026)

Guru jawab §44.22: (a) gabungkan `docs-local`/`*-live.md` ke root
sekarang, (b) protokol folder `F:\ZEVANIC HOUSE\FOUNDATION` (langsung,
bukan zip) buat dokumen peta + `\Data Yang Disiapkan` buat file kode
**MASIH BERLAKU**, dipakai lagi ke depan begitu device bridge nyambung.

**Penggabungan dieksekusi** (2 agent baca-langsung-kode, bukan
copy-paste dari docs-local mentah-mentah — tiap klaim dicek ulang ke
`js/*.js`/`index.html`/`css/gechoo-design.css` versi TERBARU, karena
`docs-local`/`*-live.md` sendiri ternyata juga sedikit basi di beberapa
detail, misal Bottom Sheet Profile sempat direvisi lagi SETELAH
`docs-local` ditulis):
- `PETA-MENU.md`: +cerita redesain mobile lengkap (Beranda/Kartu
  Statistik/Favorit Saya, Bottom Sheet Profile ganti Drawer, Menu
  Lengkap, Atur Favorit, notifikasi lonceng, Mode Tema 3-arah, panel
  Urutan Menu Config Akses) digabung ke section yang sudah ada — bukan
  duplikat section baru. 6.5KB → 19.3KB.
- `PETA-DATABASE.md`: field baru `users/{email}` (`menu_favorit`,
  `beranda_grup`, `beranda_grup_urutan`, `beranda_batas_kartu`) +
  koleksi `pengaturan_sistem/urutan_menu_home` ditambahkan. 37 koleksi
  tercatat sekarang.
- `PETA-DESAIN.md`: +pola Bottom Sheet/Dialog (contoh markup nyata),
  konvensi "kartu-baris", catatan `.gc-drawer-*` DEPRECATED (dicek
  grep — 0 pemakaian aktif, digantikan `.gc-sheet`), placeholder Kartu
  Statistik. 237→311 baris.
- `PEDOMAN-GAYA-KERJA.md`: digabung 2 ARAH (bukan cuma root menang) —
  root kebagian aturan "kapan nulis dokumen 1x per fitur selesai",
  checklist riwayat browser, DAN protokol folder pengiriman file
  (§44.22) dari `docs-local`; sebaliknya bagian tips efisiensi
  reasoning-effort/tool-use & "kebiasaan bikin hasil dipercaya" yang
  cuma ada di root TETAP ada, tidak hilang. 276→507 baris.

**File lama ditandai (BUKAN dihapus)**: `docs-local/PETA-MENU.md`,
`docs-local/PETA-DESAIN.md`, `docs-local/PEDOMAN-GAYA-KERJA.md`,
`PETA-DATABASE-live.md`, `STATUS-PROYEK-live.md`, dan duplikat root
`RENCANA-PERSIAPAN-PRODUKSI-V2.md` — semua dikasih banner ⚠️ di baris
atas "SUDAH DIGABUNG/USANG, baca versi root", bukan dihapus fisik
(jaga-jaga masih ada yang perlu ditelusuri histori-nya).

**Fase 5 (audit referensi lama) sekarang RESMI SELESAI**, mencakup lebih
luas dari scope awal — bukan cuma sisa referensi Persiapan Produksi
lama (temuan asli §44.21 poin 1-4, sudah dieksekusi), tapi juga
penemuan & perbaikan fork dokumentasi peta/database/desain/gaya-kerja
yang ternyata sudah lama diam-diam bercabang 2 arah (poin 5). Semua
sudah disinkron ke Project claude.ai.

**Yang masih belum sempurna, dicatat biar jujur**: `PETA-INFRASTRUKTUR.md`
(lokasi root, TAPI Project daftar-nya sebagai `claude/PETA-
INFRASTRUKTUR.md`) & `claude/FIRESTORE-RULES-SNAPSHOT.md`/`PRINSIP-
HEMAT.md` (2 dokumen ini terdaftar di Project TAPI tidak ditemukan sama
sekali di working copy lokal) — BELUM diperiksa/diperbaiki, di luar
scope penggabungan sesi ini. "🔌 Integrasi" section `PETA-MENU.md` juga
kelupaan menu **Device Kiosk** (era lebih lama, 22 Agt) — ditemukan
1 agent tapi sengaja tidak disentuh (di luar scope redesain mobile).
Kandidat kerjaan lanjutan kalau Guru mau dirapikan lagi.

## §44.24 — Jalur pengiriman file KODE dipindah TOTAL ke Google Drive (30 Agt 2026)

Setelah §44.23 (Fase 5 audit ditutup), Guru bertanya lewat chat apakah
knowledge/dokumen bisa cukup disinkron ke Project claude.ai saja tanpa
perlu juga ditulis ke folder lokal `F:\ZEVANIC HOUSE\FOUNDATION`
(alasan konkret: device bridge ke komputer Guru sering gagal
tersambung sepanjang sesi ini, `device_list_dir` 2x dicoba gagal
"tidak terhubung ke bridge"). Dikonfirmasi via `AskUserQuestion`:
**dokumen peta CUKUP `project_write` ke Project, tidak perlu lagi ke
folder lokal** — ditulis ke `PEDOMAN-GAYA-KERJA.md` bagian "Cara kirim
hasil kerja" (lihat riwayat §44.23 di atas, direvisi lagi di sini).

Guru lanjut tanya **"klw disimpan di gdrive bisa?"** — awalnya
ditanyakan balik lewat `AskUserQuestion` apa maksudnya (dokumen atau
kode?), Guru klarifikasi: **"project tetap yg utama untuk [dokumen],
yg saya tanyakan lebih ke file kode"**. Ditanya lagi 2 keputusan lewat
`AskUserQuestion`: (1) Google Drive GANTIIN TOTAL jalur kode (bukan
cuma fallback tambahan) — Guru pilih **"Gantiin total — kode selalu ke
Drive"**; (2) pakai folder Drive baru atau folder existing — Guru
pilih **"Buatkan folder baru"**.

**Dieksekusi:**
- Folder Google Drive baru dibuat: **"Zevanic ERP - Kode"**
  (`parentId` = `10ZZYxaNFTyEM8YDj5D17SIRDTCcEnyKc`), dimiliki akun
  `gechoo.co@gmail.com` (akun Guru sendiri — connector Drive yang
  tersambung ke sesi ini memang akun itu, dicek lewat
  `get_file_metadata` sebelum mulai upload, jadi tidak perlu langkah
  share/undang terpisah).
- 3 file kode yang berubah sepanjang Fase 5 (lihat §44.21) diupload ke
  folder itu sebagai TES SEKALIGUS ROLLOUT jalur baru:
  `vue-config.js` (9958 byte, upload lewat `base64Content` — muat 1x
  panggilan Read base64), `vue-scan-persiapan.js` (32601 byte, upload
  lewat `textContent` dari Read teks asli — base64 gagal/terpotong di
  file ini), `vue-master-produk.js` (119150 byte/1911 baris, file
  terbesar — perlu 3x panggilan `Read` berurutan buat isi lengkap, lalu
  `textContent`). Ketiganya diverifikasi lewat `fileSize` hasil API
  dibanding `wc -c` — 2 file pertama PERSIS sama, `vue-master-produk.js`
  beda 1 byte (newline terakhir file tidak ikut ter-copy — kosmetik,
  BUKAN korupsi isi, sudah dicek `od -c` isi akhir file cocok).
- **Pelajaran teknis penting** (dicatat detail di `PEDOMAN-GAYA-KERJA.md`
  bagian "Cara kirim hasil kerja" supaya tidak diulang lagi sesi
  depan): base64 TANPA line-wrap gagal dipaginasi tool `Read` (satu
  baris raksasa, terpotong permanen di ~25000 token pertama, sisanya
  hilang tanpa cara lanjut baca) — solusinya untuk file teks/kode
  SELALU pakai Read isi ASLI (bukan base64) lalu kirim lewat parameter
  `textContent` (bukan `base64Content`), dengan `contentMimeType:
  text/plain`/`application/javascript` + `disableConversionToGoogleType:
  true` (WAJIB, supaya tidak dikonversi jadi Google Docs). File yang
  lebih besar dari ~700-800 baris perlu beberapa panggilan `Read`
  berurutan (`offset`/`limit`) sebelum digabung jadi 1 `textContent`.
- `PEDOMAN-GAYA-KERJA.md` bagian "Cara kirim hasil kerja (file yang
  berubah) ke Guru" ditulis ULANG bagian file KODE-nya (isi lama:
  `Data Yang Disiapkan`/zip — SEKARANG isinya protokol Google Drive di
  atas, termasuk aturan "flat" versi Drive: `title` = nama file asli,
  jangan bikin sub-folder tiruan struktur repo di dalam folder Drive
  ini) — disinkron ke Project.

**Status jalur pengiriman file per 30 Agt 2026 (ringkasan)**:
dokumen peta/pengetahuan → Project claude.ai (`project_write`) SAJA;
file kode → Google Drive folder "Zevanic ERP - Kode" SAJA — device
bridge (`F:\ZEVANIC HOUSE\FOUNDATION` & `\Data Yang Disiapkan`) TIDAK
DIPAKAI LAGI untuk kedua jenis file ini per keputusan eksplisit Guru,
kecuali Guru minta balik lagi di kemudian hari (harus ditanya ulang,
bukan diasumsikan).

## §44.25 — Checklist testing manual Persiapan Produksi V2 dibuatkan (30 Agt 2026)

Guru minta lanjut fokus "Testing manual Persiapan Produksi V2" (lewat
`AskUserQuestion`, dari beberapa kandidat kerjaan §44.24 yang
ditawarkan). Karena testing browser+Firebase live TIDAK BISA dilakukan
dari sandbox ini (tidak ada akses live), dibuatkan **checklist
interaktif** (Artifact HTML, checkbox tercentang tersimpan otomatis di
browser Guru lewat localStorage) merangkum SEMUA langkah dari riwayat
build Fase 1-4 (§44.13-§44.20) + `RENCANA-PERSIAPAN-PRODUKSI-V2.md` —
7 bagian: prasyarat (termasuk peringatan status Rules `spk_track` yang
TIDAK ada konfirmasi eksplisit se-jelas `spk_grouping`, beda dari yang
dikira sebelumnya — WAJIB Guru cek sendiri di Firebase Console dulu),
buat SPK Grouping test, jalur Bahan (5 tahap penuh), Acc Sewing/
Webbing/Finishing, jalur Vendor (perhatian khusus: driver sama buat
Scan Kirim & Scan Sampai), cek cetak label, dan daftar "bukan bug"
(Vendor tidak auto-detect BOM, Scan Masalah belum tersambung) supaya
tidak salah lapor.

URL: https://claude.ai/code/artifact/040df10f-df6d-4636-bec2-2acf582691b0

## §44.26 — Fix bug "minimize/maximize sidebar desktop tidak jalan" (30 Agt 2026)

**Laporan Guru (verbatim)**: "bantu perbaiki bug side bar desktop jadi
minizae dan maximazenya tidak jalan" — klik parent grup sidebar (Master
Absensi/Keuangan/Karyawan/Zevanic House/Persiapan Produksi/Integrasi)
buat buka/tutup sub-menunya (accordion) kelihatan tidak ngefek.

**Diagnosis — DIBUKTIKAN LANGSUNG di `gechoo.online` sungguhan lewat
browser automation (bukan tebak-tebak dari baca kode saja)**, karena
sesi ini kebetulan sudah dalam kondisi login (akun Owner). Ditemukan
lewat inspeksi DOM/CSS langsung (`getComputedStyle`), bukan cuma baca
`js/dashboard.js`:

Fungsi `toggleNavGroup()`/`setGrupSidebarTerbuka()` di `js/dashboard.js`
**SUDAH BENAR** — pasang/lepas class `hidden` ke elemen yang tepat,
tidak ada bug logic di situ sama sekali. Masalahnya di CSS: 6 pasang
elemen sidebar (tombol parent grup + `<div>` sub-menu di bawahnya, buat
KEENAM grup) di `index.html` punya `class="hidden ..."` **DAN**
`style="display:flex; ..."` inline SEKALIGUS. Inline `style=""` di HTML
SELALU menang mutlak dari aturan class manapun di CSS eksternal, tidak
peduli urutan/`!important` — jadi class `.hidden{display:none}` (di
`css/gechoo-design.css`) TIDAK PERNAH benar-benar bisa nyembunyiin
elemen-elemen itu, walau class-nya sendiri sudah benar terpasang/lepas
di DOM. Dikonfirmasi eksak lewat `getComputedStyle(el).display` yang
TETAP `"flex"` walau class `hidden` ADA di elemennya.

**Akibatnya DUA bug sekaligus** (satu akar penyebab, dua tempat kena):
1. **Yang dilaporkan Guru**: sub-menu SEMUA grup sidebar kelihatan
   selalu terbuka bersamaan terus-menerus — klik parent buat
   minimize/maximize kelihatan seperti tidak ngefek, padahal sebenarnya
   class-nya berubah, cuma visualnya tidak pernah ikut berubah.
2. **Temuan TAMBAHAN, TIDAK dilaporkan Guru tapi akar penyebabnya
   PERSIS SAMA** — `js/auth.js` `aturTampilanBerdasarkanRole()`
   nyembunyikan/nampilin TOMBOL PARENT grup sidebar (bukan cuma
   sub-menunya) berdasar role, juga lewat `classList.add/remove
   ('hidden')` ke elemen yang SAMA-SAMA kena inline `style="display:
   flex; ..."` — artinya role manapun yang login, KEENAM tombol parent
   grup sidebar (termasuk Master Karyawan/Superuser, WhatsApp/Mail
   Gateway yang harusnya cuma buat Owner/Superuser) selama ini SELALU
   kelihatan tampil ke SEMUA role, tidak peduli role-nya berhak atau
   tidak. Belum dicek apakah klik ke tab-nya juga ke-block di lapisan
   lain (kemungkinan besar iya, lewat Firestore Rules/`isAdminLevel()`)
   — TAPI dari sisi TAMPILAN sidebar, ini kebocoran visibilitas menu
   yang sebelumnya tidak disadari.

**Fix (diverifikasi lagi lewat `getComputedStyle` di browser SEBELUM
ditulis ke file, dengan cara suntik CSS+class persis sama pola yang
mau dipakai)**:
- `css/gechoo-design.css` — class baru `.gc-nav-subgroup` (styling
  sub-menu yang dulu inline, sekarang jadi class), ditaruh SEBELUM
  `.hidden{display:none;}` di source (ikut pola yang sudah ada &
  didokumentasikan di file yang sama soal urutan `.hidden` vs
  `.flex`/`.md:grid`).
- `index.html` — 6 `<div>` sub-menu: `style="display:flex; ..."`
  dihapus total, ganti `class="hidden gc-nav-subgroup"`. 6 `<button>`
  parent grup: inline `style` disederhanakan jadi cuma `style="font-
  weight:700;"` (properti `display:flex`/`align-items:center` yang tadi
  inline ternyata REDUNDAN — sudah didefinisikan class `.gc-nav-item`
  yang sudah dipakai di button yang sama, jadi aman dihapus, bukan
  kehilangan styling apapun). `?v=4`→`?v=5` di link `gechoo-design.css`
  (cache-bust, wajib biar browser Guru tidak pakai CSS lama dari cache).

**Cara Guru deploy & tes**: push kedua file (`index.html`,
`css/gechoo-design.css`) ke GitHub lewat github.dev (lihat obrolan chat
soal alur Drive→github.dev), tunggu GitHub Pages selesai rebuild, HARD
REFRESH (Ctrl/Cmd+Shift+R) di browser biar `?v=5` yang baru kepakai
(bukan `?v=4` lama dari cache) — baru coba klik parent grup sidebar,
harus beneran minimize/maximize sekarang.

File diupload ke folder Google Drive "Zevanic ERP - Kode" — **KONFIRMASI SELESAI**, kedua file sudah diverifikasi byte-exact (download-balik & diff terhadap file lokal, cuma beda newline-akhir yang tidak berpengaruh, pola yang sama seperti file lain sebelumnya):
- `css/gechoo-design.css` — https://drive.google.com/file/d/1nsW0FQgoNEv0dLM1auXapGMbF3Fainzt/view?usp=drivesdk
- `index.html` — https://drive.google.com/file/d/13lF5aNMXKPjhqUNrOwtR7Ppkj6XgrVox/view?usp=drivesdk

(Catatan proses: upload pertama `index.html` sempat salah ketik 2 karakter `&`→`&amp;` di teks tampilan — "ERP System & Mgmt" dan "presensi & operasional" — ketahuan lewat verifikasi download+diff, file salah langsung ditrash di Drive, diupload ulang dan diverifikasi ulang sampai byte-exact. File final di link atas sudah benar.)

## §45 — Fitur BARU: "Pesanan" (grup top-level, Kasir + pipeline SPK) (30 Agt 2026)

**Latar & permintaan Guru** — di luar Fase 5 audit (§44.21) yang sedang
jalan, Guru minta fitur baru sambil menunjukkan mockup pribadinya
(`zco.solusiumkm.digital/admin`, dikonfirmasi Guru sendiri "ini mockup
yg saya buat hehe" — BUKAN bagian proyek ini, cuma referensi visual).
Spesifikasi final (verbatim, dikirim ulang lengkap setelah 1 ronde
AskUserQuestion sempat garbled/interrupted):
...
+ "iyah kita implementasikan pada zevanic erp > yg saya suruh tadi
cuman desain mockup" (konfirmasi eksplisit: dibangun di Zevanic ERP
sungguhan, mockup cuma referensi tampilan Kasir).

**4 keputusan arsitektur** (AskUserQuestion, dijawab jelas Guru):
1. Fungsi lama Order SPK (list/edit/hapus/cetak label) → **"Jadi
   bagian menu 'Menunggu Proses'"**.
2. Data transaksi Kasir disimpan → **"Koleksi baru khusus transaksi
   Kasir (Direkomendasikan)"**.
3. Beda "Proses Persiapan" vs "Proses Produksi" → **"Persiapan = jalur
   Vendor+Bahan, Produksi = jalur Sewing+Webbing+Finishing"**.
4. Harga jual produk → **"Field 'Harga Jual' baru di Master Produk"**
   (master_produk sebelumnya nol field harga jual, 100% data BOM/ongkos
   produksi).

**3 hal yang BELUM PERNAH eksplisit dikonfirmasi Guru** (diputuskan
sendiri dengan asumsi masuk akal, BUKAN ditebak diam-diam — didokumentasikan
di sini & di komentar besar atas `js/vue-pesanan.js`, WAJIB dicek ulang
sama Guru):
- **A. Skema koleksi Kasir**: nama `transaksi_kasir` (+ counter harian
  `pengaturan_id_transaksi_kasir`), format `no_transaksi` = `TRX{yymmdd}
  {counter 3 digit}` — pola SAMA PERSIS seperti `generateKodeSpkGrouping()`
  (§44.13).
- **B. Lingkup "Proses Pengiriman"**: SEMUA jalur, `spk_track.status`
  IN `['perlu_dikirim','sedang_dikirim']` (dipilih karena "Pengiriman"
  cocok NAMA TAHAP pipeline, beda dari Persiapan/Produksi yang cocok
  NAMA JALUR).
- **C. Posisi grup sidebar "Pesanan"**: setelah "Zevanic House", SEBELUM
  "Persiapan Produksi" (`KATEGORI_URUTAN`, `vue-config-akses.js`) —
  ikuti alur kerja (Kasir jual → SPK → Persiapan Produksi kerjakan).

**Implementasi**:
- **`js/vue-pesanan.js`** (BARU, file baru) — 3 kelompok komponen:
  1. `PesananKasirManager` ("Penjualan Kasir") — POS: tab kategori
     (dari `jenis_produk` semua produk) + grid produk (`ambilSemuaProduk()`,
     harga dari `master_produk.harga_jual`) + panel keranjang (qty
     stepper, nama pelanggan opsional, metode pembayaran Tunai/Transfer/
     QRIS/Lainnya, total). Tombol "Buat Order" → 1 dokumen
     `transaksi_kasir` (no_transaksi via counter transaksi) + N dokumen
     `order_spk` OTOMATIS (1 per item keranjang, `no_spk =
     "{no_transaksi}-{urutan item}"`, field/skema SPK **TIDAK BERUBAH
     SAMA SEKALI** — mengalir tanpa hambatan ke pipeline Persiapan
     Produksi V2 yang sudah ada).
  2. `PesananMenungguManager` ("Menunggu Proses") — DISALIN (bukan
     diimpor, konvensi proyek) dari `OrderSpkManager` lama
     (`js/vue-order-spk.js`), logic/field CRUD 100% sama (form+cari+
     paginasi+cetak label QR+pilih-banyak), cuma `menuId` ganti jadi
     `pesanan_menunggu`. Koleksi `order_spk` yang dibaca SAMA — SPK
     dari Kasir DAN dari form manual di sini sama-sama tampil di sini.
  3. `RingkasanSpkTrackManager` (1 komponen dipakai 3× lewat props) —
     "Proses Persiapan"/"Proses Produksi"/"Proses Pengiriman", BACA
     SAJA (tidak ada tombol scan/aksi, itu tetap di menu "Persiapan
     Produksi" asli). Query `spk_track` SENGAJA cuma 1 filter `'in'`
     per query (jalur SAJA *atau* status SAJA, TIDAK PERNAH gabung
     dalam 1 query) — supaya **TIDAK butuh index komposit manual** di
     Firebase Console (grouping kedua dihitung client-side dari hasil
     query tunggal). Tiap baris ada tombol "Buka di Persiapan Produksi"
     (lompat `pindahTab`/`pindahSubTab` ke jalur+tahap aslinya).
- **`js/vue-master-produk.js`** (v17→v18) — field baru `harga_jual`
  (number, opsional, default 0) di `form` reactive, payload `simpan()`,
  input di template Entry Produk (setelah SKU), + baris tampilan di
  kartu List Produk.
- **`js/vue-config-akses.js`** (v6→v7) — `KATEGORI_URUTAN` tambah
  `'Pesanan'` (posisi lihat asumsi C di atas). 5 entry `DAFTAR_MENU`
  baru: `pesanan_kasir`/`pesanan_menunggu`/`pesanan_persiapan`/
  `pesanan_produksi`/`pesanan_pengiriman`. Id `order_spk`
  **DIPENSIUNKAN** (`deprecated: true`, label diganti "DIPENSIUNKAN,
  lihat Pesanan > Menunggu Proses" — pola SAMA seperti
  `persiapan_produksi_antrean` dkk §44.13, izin lama TIDAK yatim).
- **`index.html`** — grup sidebar baru `navgrp-pesanan` (5 tombol,
  ditaruh setelah grup Zevanic House, sebelum Persiapan Produksi) +
  tab-content baru `tab-pesanan` (5 div `sub-pesanan-content` + 5 mount
  point). Tombol & div konten Order SPK lama di Zevanic House **DIHAPUS
  TOTAL** (bukan cuma disembunyikan) — pola sama seperti pembersihan
  Persiapan Produksi lama §44.13. Script `js/vue-order-spk.js`
  **TIDAK LAGI DIMUAT** (file dibiarkan ada di disk, pola sama seperti
  `vue-persiapan-produksi.js`). Script baru `js/vue-pesanan.js?v=1`
  ditambahkan. `js/dashboard.js` v19→v20, `js/vue-config-akses.js`
  v6→v7, `js/vue-master-produk.js` v17→v18 (cache-bust).
- **`js/dashboard.js`** (v19→v20) — `petaGrupSidebarPerTab` tambah
  `'tab-pesanan': 'navgrp-pesanan'`. Array `tabs` tambah `'tab-pesanan'`.
  Landing default `tab-pesanan` → `sub-pesanan-kasir` (Penjualan Kasir,
  layar aksi harian utama — pola sama seperti landing default
  `tab-zevanic-house`/`tab-persiapan-produksi`). `petaTabIndukPerGrup`
  tambah `'sub-pesanan': 'tab-pesanan'`. `petaMount` tambah 5 entry
  (`pastikanMountPesananKasir`/`Menunggu`/`Persiapan`/`Produksi`/
  `Pengiriman`), entry lama `'sub-zevanic-house-orderspk':
  'pastikanMountOrderSpk'` **DIHAPUS**.
- **`firestore.rules`** — 2 match block BARU: `transaksi_kasir` (`allow
  read: if login(); allow write: if isAdminLevel();`) &
  `pengaturan_id_transaksi_kasir` (counter, pola sama persis
  `pengaturan_id_spk_grouping`). **⚠️ BELUM DIPUBLISH ke Firebase
  Console** — WAJIB Guru tempel manual sebelum "Penjualan Kasir" bisa
  benar-benar menulis transaksi (pola blocker berulang di proyek ini,
  lihat riwayat rules lain).

**Validasi yang SUDAH dilakukan** (otomatis, sebelum kode dikirim):
- `node --check` lolos di SEMUA file yang disentuh: `vue-pesanan.js`
  (baru), `vue-master-produk.js`, `vue-config-akses.js`, `dashboard.js`.
- `index.html`: `<div>` seimbang (229 buka = 229 tutup), 0 id duplikat.
- Cross-reference otomatis (skrip Python ad-hoc, bukan cuma baca mata):
  5 `petaMount` target `tab-pesanan` ↔ 5 `window.pastikanMountXxx` yang
  benar-benar ada di kode — cocok semua. 5 `getElementById(...)` di
  `vue-pesanan.js` ↔ 5 div `id="vue-pesanan-..."` di `index.html` —
  cocok semua. 5 tombol sidebar `data-target` ↔ 5 div konten
  `sub-pesanan-*` — cocok semua (tidak ada yang nyasar/hilang).
  Class `.no-scrollbar`/`.icon-btn`/`.btn-outline`/`.btn-primary`/
  `.gc-card`/`.gc-field`/`.gc-kosong`/`.lingkaran`/`.tag`/`.grid-cols-1`/
  `--aksen-ink` yang dipakai template baru — SEMUA sudah terdefinisi
  (dicek `grep` ke `css/gechoo-design.css`+`index.html`, 0 token tidak
  terdefinisi).

**⚠️ BELUM DIUJI di browser+Firebase sungguhan** (sama seperti pola
peringatan di fitur-fitur besar sebelumnya, §44.13 dst) — Guru WAJIB
coba manual sebelum dianggap beres:
1. Publish 2 match block `firestore.rules` baru di Firebase Console
   dulu (WAJIB, blocker keras — tanpa ini "Buat Order" di Kasir akan
   gagal permission-denied).
2. Isi `harga_jual` di beberapa produk lewat Master Produk > Entry
   Produk (produk lama defaultnya `0`/kosong, tidak akan tampil harga
   di grid Kasir sebelum diisi).
3. Buka Pesanan > Penjualan Kasir → pilih beberapa produk → cek
   keranjang (tambah/kurang qty, hapus item) → isi nama pelanggan +
   metode pembayaran → "Buat Order" → cek 1 dokumen `transaksi_kasir`
   + N dokumen `order_spk` baru muncul di Firestore Console.
4. Cek SPK yang baru otomatis dibuat itu MUNCUL di Pesanan > Menunggu
   Proses, DAN di Persiapan Produksi > Perlu Disiapkan (pipeline lama,
   harus tetap jalan tanpa perubahan).
5. Cek Menunggu Proses: tambah/edit/hapus SPK manual, cetak label QR
   (pola sama Order SPK lama) — pastikan fungsinya utuh 100% (cuma
   pindah lokasi menu).
6. Cek Pesanan > Proses Persiapan & Proses Produksi: angka ringkasan
   per tahap (5 kartu) sesuai jumlah `spk_track` beneran di jalur
   masing-masing, tombol "Buka di Persiapan Produksi" per baris lompat
   ke jalur+tahap yang benar.
7. Cek Pesanan > Proses Pengiriman: cuma tampilkan `spk_track` status
   `perlu_dikirim`/`sedang_dikirim` lintas SEMUA jalur, angka ringkasan
   per jalur sesuai.
8. Cek sidebar desktop: grup "Pesanan" muncul & minimize/maximize
   normal (setelah fix §44.26), posisinya antara Zevanic House &
   Persiapan Produksi.
9. Cek Config Akses > profil manapun: 5 menu-id baru muncul di kategori
   "Pesanan" buat diatur izinnya, `order_spk` (lama) sudah tidak nongol
   lagi (deprecated).

**BELUM dikirim ke Google Drive** — 6 file berubah (`js/vue-pesanan.js`
BARU, `js/vue-master-produk.js`, `js/vue-config-akses.js`,
`index.html`, `js/dashboard.js`, `firestore.rules`) masih di sesi kerja
Claude, menyusul diupload ke folder Drive "Zevanic ERP - Kode" +
verifikasi byte-exact (pola §44.26 — WAJIB download+diff, bukan cuma
cek ukuran byte).