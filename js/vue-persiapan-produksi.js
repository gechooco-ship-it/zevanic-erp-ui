...
// idAman — ganti karakter "/" (tidak valid buat doc id Firestore) kalau ternyata
// ada No. SPK yang memuatnya — jaga-jaga, bukan pola normal.
function idAman(teks) {
  return String(teks || '').replace(/\//g, '-');
}

// KODE_TAHAP_ACC — daftar 3 tahap Acc yang dikenali sistem (lihat "KEPUTUSAN
// SEPIHAK TAMBAHAN poin a" di atas file ini).
...
// PersiapanQueueManager — kartu 1 "Perlu Disiapkan" (antrean SPK baru + tombol
// Approve yang men-generate kartu 2-5 dari BOM).
...
const PersiapanQueueManager = {
  ...
  template: `
    ...
    <div v-else style="display:grid; gap:12px;" class="grid-cols-1 md:grid-cols-2">
      <!--
        Kartu-baris (README §4) — judul+subtitle, chip status kanan-atas, baris meta (ikon+teks),
        baris aksi dipisah border-top.
      -->
...
// Finishing). Komponen SATU dipakai 4x (prop `tipe`), "format sama, beda filter"
// persis. Checklist per baris READ-ONLY di sini — ditandai selesai lewat Scan
// Persiapan (lihat catatan header file).
