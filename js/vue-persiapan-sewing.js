...
const PersiapanSewingPerluDisiapkan = {
  ...
  setup() {
    ...
    function cetakLabelKartu(k) {
      ...
      // dari noSpk polos (TRX..., tidak enak dibaca & "menumpuk di layar") ke
      // `kode_kartu` (dulu disebut kode_anak_spk versi lanjutan 6, DINAMAI
      // ULANG 13 Sep krn sekarang ada level lebih detail di bawahnya — lihat
      // komentar besar tandaiKodeGrouping() vue-persiapan-produksi-v2.js;
      // utk jalur Acc, kartu = 1 order/no_spk, TIDAK berubah dari sebelumnya).
      // FALLBACK ke noSpk kalau kode_kartu belum ada (data lama, atau Config
      // > TLC & Prefix > jalur Acc Sewing belum diisi Guru) —
      // hasilScanTunjuk/hasilScanAksi di bawah dicocokkan ke NILAI YANG SAMA
      // PERSIS ini, tidak pernah beda dari yang dicetak. Kalau 1 order butuh
      // >1 aksesoris beda (Zipper+Kancing), 1 label/QR yang SAMA (kode_kartu)
      // tetap dipakai bareng, tapi tiap item tetap punya kode_komponen sendiri
      // (Zipper -01, Kancing -02 dst) supaya kebeda di info label & tampilan.
        ...
        <div v-for="k in kartuList" :key="k.trackId" class="gc-card gc-card-menonjol" style="padding:14px; border-radius:20px;">
          ...
          <div style="display:flex; flex-direction:column; gap:6px; margin-bottom:10px;">
            <label v-for="b in k.baris" :key="barisKey(b)" style="display:flex; align-items:center; gap:8px; font-size:11px; padding:6px 8px; border-radius:10px; flex-wrap:wrap;" :style="{ background: b.label_cetak_pada ? 'var(--ok-light)' : (b._bisa ? 'transparent' : 'var(--danger-light)') }">
              ...
              <span style="flex:1; min-width:120px;">{{ b.nama_aksesoris }} <span style="color:var(--text-faint);">{{ b.warna }}</span> <span class="gc-num" style="color:var(--text-faint); font-size:10px;">{{ b.kode_komponen || b.kode_kartu || b.no_spk }}</span></span>
...
const PersiapanSewingSedangDisiapkan = {
  ...
  setup() {
    ...
    async function hasilScanAksi(kodeMentah) {
      ...
      // FIX (12 Sep 2026 lanjutan 6; RENAME 13 Sep lanjutan 9) — cocokkan ke
      // kode_kartu (fallback no_spk utk data lama), SAMA kode yg dicetak
      // (lihat cetakLabelKartu) — 1 label fisik = 1 kode_kartu, walau order
      // ini butuh >1 komponen (tiap komponen tetap kebagian kode_kartu SAMA
      // krn berbagi 1 label yg sama).
