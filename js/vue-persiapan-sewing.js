...
// Persiapan Produksi > Acc Sewing. Pos komponen jahit — zipper, buckle,
// D-ring, cord lock, elastis. Satuan pcs. File ini pola dasar yang ditumpangi
// vue-persiapan-webbing.js & vue-persiapan-finishing.js.
//
// Koleksi & field:
// - Sumbernya spk_track jalur:'sewing', field sewing_rincian[] (diisi
// hitungSewingRincian di vue-persiapan-produksi-v2.js saat SPK Grouping
// diterbitkan). 1 baris per (komponen aksesoris x anak SPK), BOM dari
// master_produk.bom_aksesoris[] disaring tahap_proses berisi "sewing".
// - Baris: order_spk_id, no_spk, qty, bahan_aksesoris_id, nama_aksesoris,
// warna, produk_size, qty_per_pcs, satuan, butuh, status, masuk_tahap_pada,
// label_cetak_pada, operator_uid/nama, ditugaskan_pada, riwayat_operator[],
// entry_qty/oleh/pada, catatan_masalah, kode_bagging, kode_tugas,
// tlc_tujuan (ditulis saat Scan Kirim), sampai_pada (ditulis modul lain).
// - tlc_asal = 'TLC-SEW'. Koleksi bagging/tugas_kirim/master_tlc/
// cetak_ulang_log dipakai ulang apa adanya, generik lintas jalur.
//
// Jebakan:
// - Koleksi persiapan_komponen SUDAH DITINGGALKAN tanpa migrasi. Dokumen yang
// masih menyebutnya sebagai sumber data itu basi.
// - Satu kartu = satu SPK Grouping (1 dokumen spk_track, kode berakhiran
// -SEW), kebalikan dari Bahan yang mengumpulkan kartu lintas dokumen. Tidak
// ada kumulatif butuh/stok lintas grouping; cek stok per baris di
// kelompokKartuSpk.
// - "1 SPK = 1 label" dibaca sebagai 1 label per ANAK SPK, karena 1 grouping
// bisa berisi >1 anak SPK dan tiap anak butuh label fisiknya sendiri. Kode
// di QR cuma jejak cetak — yang discan balik selalu no_spk polos.
// - 1 scan pack/kirim bisa menandai banyak baris sekaligus, jadi ada
// updateBarisSewingMassal di samping updateBarisSewing versi 1-baris
// (dipakai penunjukan/entry/masalah/ganti-operator tunggal). Salah pilih
// fungsi = baris lain ikut/tidak ikut berubah diam-diam.
...
// updateBarisSewing — read-modify-write ATOMIK 1 baris (dipilih lewat index),
...
// updateBarisSewingMassal — (beda dari Bahan): patch SEMUA elemen array
...
// vue-persiapan-bahan.js (baca komentar besar di sana untuk detail lengkap,
// tidak diulang di sini) — disalin, TIDAK diimpor silang. Beda Sewing dari Bahan
// yang relevan di sini: grid Tab 1 SUDAH 1 kolom dari awal
// (flex-direction:column, dicek ke wireframe Acc Sewing — SESUAI, tidak perlu
// diubah), dan Tab 1 dapat TAMBAHAN 4 kotak KPI (SPK menunggu/baris
// komponen/stok kurang/siap dicetak — wireframe baris ~155-158) yang TIDAK ada
// di Bahan. Bar ringkasan sticky + cetak massal (temuan #5 Bahan) SENGAJA TIDAK
// disalin ke sini — dicek ke wireframe Acc Sewing, tidak ada bar/tombol
// cetak-massal lintas kartu di sana (checkbox di baris komponen cuma indikator
// "stoknya ada", bukan seleksi cetak; cetaknya "1 SPK = 1 label" per kartu,
// sudah ada).
...
const PersiapanSewingPerluDisiapkan = {
  ...
  setup() {
    ...
    // sama di vue-persiapan-bahan.js).
    ...
    // kotak KPI persis wireframe baris ~155-158 ("SPK menunggu / baris komponen
    // / stok kurang / siap dicetak").
    ...
    // fungsi GLOBAL yang sama dipakai Acc Webbing/ Finishing DAN kartu di layar
    // (template di bawah), sama pola bangunLabelBahan di vue-persiapan-bahan.js.
    // qrDataUrl ditambah di sini (bukan di dalam bangunLabelAksesoris) supaya QR
    // cuma digambar 1x per label yang BENAR-BENAR dicetak, bukan tiap kartu
    ...
    async function onCetakSelesai() {
      ...
      try {
        ...
        // dokumen) — cukup 1 updateBarisSewingMassal per kartu, bukan N
        ...
        // updateBarisSewingMassal cuma memanggil matchFn(arr[i]) TANPA index
        // kedua, jadi `i` selalu undefined dan baris TIDAK PERNAH tertandai .
        // Diganti matching by value (no_spk) — semua baris 1 anak SPK memang
    ...
    // vue-persiapan-bahan.js. Tombol per-kartu TETAP DIPERTAHANKAN.
    ...
    async function hasilScanTunjuk(kodeMentah) {
      ...
      // error, jadi harus dicek manual di sini.
      try {
    ...
    return { muat,
      ...
      // templat di bawah pakai `barisKey(b)` sebagai:key v-for (baris 653-an),
      // tapi fungsinya lupa diikutkan di return setup ini. Selama kartuList
      ...
      // undefined -> "TypeError: barisKey is not a function" -> render crash ->
      // Vue TAHAN vnode LAMA (yang masih nampilkan "Memuat..") di layar
      // selamanya, tanpa pesan error yang kelihatan user.
  ...
  template: `
    ...
    <div v-else class="gc-card gc-card-menonjol" style="padding:20px; border-radius:20px;">
      ...
      <!-- RETROFIT (temuan #6) — 4 kotak KPI, wireframe ~155-158. -->
      ...
      <div v-else style="display:flex; flex-direction:column; gap:10px;">
        <div v-for="k in kartuList" :key="k.trackId" class="gc-card gc-card-menonjol" style="padding:14px; border-radius:20px;">
          ...
          <!--
            Baris aksesoris TAMPIL PERSIS urutan bangunLabelAksesoris (kode/QR · nama produk+warna
            · nama aksesoris · warna+butuh+ satuan · pelanggan) — SENGAJA sama fungsi dgn yang
            dipakai cetak label fisik, supaya operator bisa cocokkan HP vs label yang sudah
            ditempel di gudang tanpa hafal urutan beda.
          -->
...
// alasan, SAMA persis pola vue-persiapan-bahan.js Tab 2 (lihat komentar besar di
// sana untuk detail) — tlc_asal='TLC-SEW', sumber_jalur='sewing'.
//
...
// DAN semua yang sedang_disiapkan sudah ber-entry_qty .
...
const PersiapanSewingSedangDisiapkan = {
  ...
  setup() {
    ...
    async function hasilScanAksi(kodeMentah) {
      ...
      // cocokkan ke kode_kartu (fallback no_spk utk data lama), SAMA kode yg
      // dicetak (lihat cetakLabelKartu) — 1 label fisik = 1 kode_kartu, walau
      // order ini butuh >1 komponen (tiap komponen tetap kebagian kode_kartu
      // SAMA krn berbagi 1 label yg sama).
...
const PersiapanSewingPerluDikirim = {
  ...
  setup() {
    ...
    async function hasilScanPack(kodeMentah) {
      ...
      // grouping, warna tetap boleh campur.
      ...
      try {
        // matchFn lama pakai idxSet.has(i) yang selalu undefined (lihat catatan
        // besar di onCetakSelesai di atas). `cocok` sudah difilter persis pakai
        // kode+!kode_bagging di atas — dipakai lagi di sini sebagai matchFn
        // (value-based, bukan index) supaya tidak perlu bikin key baru.
...
// window.pastikanMountPpSewingXxx dipanggil oleh pindahSubTab (js/ dashboard.js,
