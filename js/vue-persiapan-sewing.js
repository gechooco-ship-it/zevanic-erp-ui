    ...
    async function onCetakSelesai() {
      ...
      try {
        ...
        await Promise.all(Object.entries(byTrack).map(([trackId, barisGrup]) => {
          ...
          return updateBarisSewingMassal(trackId, (x, i) => idxSet.has(i) && !x.label_cetak_pada, () => ({ label_cetak_pada: now }));
        }));
...
const PersiapanSewingSedangDisiapkan = {
  ...
  template: `
    ...
    <popup-pin-generik v-if="pinCetakUlangAktif" judul="Verifikasi PIN — Cetak Ulang Label" konteks="Acc Sewing - Cetak Ulang Label" @sukses="pinCetakUlangSukses" @batal="pinCetakUlangAktif = false" />
    <popup-pratinjau-cetak-label :terbuka="popupCetakAktif" judul="Cetak Ulang Label Anak SPK" :daftar-label="daftarLabelPreview" jenis-cetak="label_spk_acc_sewing" @tutup="popupCetakAktif = false" />
  `
};
    ...
    async function hasilScanPack(kodeMentah) {
      ...
      try {
        ...
        const hasil = await Promise.all(Object.entries(byTrack).map(([trackId, barisGrup]) => {
          return updateBarisSewingMassal(trackId, (x) => kodeLabelAcc(x) === kode && !x.kode_bagging && x.status === 'perlu_dikirim', () => ({ kode_bagging: modalPack.bagging.kode }));
        }));
