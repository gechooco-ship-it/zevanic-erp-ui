...
const PersiapanSewingPerluDisiapkan = {
  ...
  setup() {
    ...
    // Isi label dari labelBarisAcc; qrDataUrl ditambah di titik cetak saja.
    ...
    const scanOperator = buatScanTerpadu({
      judul: 'Scan Operator — Acc Sewing', subjudul: 'Scan QR operator/tim, lalu label aksesoris satu per satu',
      twoStep: {
      ...
      padaUpload: async (rows, locked) => {
        ...
        try {
          for (const row of rows) {
            ...
            const kena = await updateBarisSewingMassal(targets[0]._trackId, (x) => cocokLabelAcc(x, row.kode) && x.status === 'perlu_disiapkan' && !!x.label_cetak_pada, (lama) => ({
    ...
    <div v-else class="gc-card gc-card-menonjol" style="padding:20px; border-radius:20px;">
      ...
      <div v-else style="display:flex; flex-direction:column; gap:10px;">
        <div v-for="k in kartuList" :key="k.trackId" class="gc-card gc-card-menonjol" style="padding:14px; border-radius:20px;">
          ...
          <!--
            Urutan baris aksesoris PERSIS labelBarisAcc, sengaja sama dengan label fisik
