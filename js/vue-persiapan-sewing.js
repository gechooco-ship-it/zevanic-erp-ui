...
const PersiapanSewingPerluDisiapkan = {
  ...
  setup() {
    ...
    function cetakLabelKartu(k) {
      ...
      // Kode label (teks besar + isi QR + kunci matching) = `kode_kartu`,
      // fallback ke noSpk kalau belum ada (data lama, atau Config > TLC &
      // Prefix > jalur Acc Sewing belum diisi Guru) — hasilScanTunjuk/
      // hasilScanAksi di bawah dicocokkan ke NILAI YANG SAMA PERSIS ini,
      // tidak pernah beda dari yang dicetak. 1 order bisa butuh >1 aksesoris
      // beda (Zipper+Kancing) — 1 label/QR yang SAMA (kode_kartu) dipakai
      // bareng, tiap item tetap dirinci baris sendiri + kode_komponen kalau
      // >1 item (supaya kebeda di label). Struktur baris info (item lalu
      // nama pelanggan) didelegasikan ke bangunInfoLabelAnakSpk() (js/vue-
      // components.js) — fungsi GLOBAL yang sama dipakai Bahan/Acc Webbing/
      // Finishing, no_spk TIDAK ditampilkan (id internal, bukan buat dibaca
      // orang — lihat tandaiKodeGrouping() vue-persiapan-produksi-v2.js).
