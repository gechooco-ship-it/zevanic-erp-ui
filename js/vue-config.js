...
// Zevanic House > Config — menu BARU . Pusat kelola data referensi yang
// SEBELUMNYA tersebar di gear (Pengaturan) 2 layar beda: - Gear "Entry Bahan &
// Aksesoris" (vue-bahan-aksesoris.js): Jenis Bahan, Jenis Aksesoris, Data
// Satuan, Data Warna, Data Ukuran. - Gear "Stock & Pembelian"
// (vue-stock-pembelian.js): Data Suplayer.
//
// BEDA dari tampilan lama (kumpulan tag/chip) yang masih dipertahankan APA
// ADANYA di semua pemakaian lama (Config Karyawan dst). Makanya
// MasterDataCategory & MasterDataTabelManager (vue-components.js) ditambah prop
// BARU `tampilTabel` (default false = tidak berubah) — logic tambah/hapus/cari
// TETAP 1 sumber yang sama, cuma template render beda.
//
// TIDAK dipindah ke sini: - Data Rak Penyimpanan (dulu di gear Entry Bahan &
// Aksesoris) — DIHAPUS TOTAL dari gear (bukan dipindah), karena sudah ada menu
// "Rak Penyimpanan" sendiri yang lebih lengkap (vue-rak-penyimpanan.js). -
// Prefix ID Bahan/Aksesoris & Prefix No. Pembelian — TETAP di gear
// masing-masing, karena sifatnya setting teknis (counter internal), bukan data
// referensi yang dicari-cari.
//
// Izin akses: SEMUA 6 tab pakai 1 menu-id yang sama, 'config_master_data'
// (didaftarkan di vue-config-akses.js DAFTAR_MENU) — sama seperti pola "Config
// Karyawan" (1 menu-id dipakai bareng banyak MasterDataCategory sekaligus). Menu
// BARU defaultnya HANYA Owner (kebijakan baku project ini) — kalau mau
// Admin/PIC/Superuser juga bisa akses, atur manual lewat Config Akses setelah
// kode ini live.
//
// Pola mount: SAMA seperti child-tab lain di app ini (lihat vue-rak-
// penyimpanan.js) — 6 mount point terpisah, masing-masing Vue app kecil (cuma
// bungkus 1 komponen reusable), di-mount LAZY lewat
// window.pastikanMountConfigXxx yang dipanggil dashboard.js (pindahSubTab) PAS
// tab-nya benar-benar dibuka — supaya tidak baca Firestore 6 koleksi sekaligus
// kalau orang belum pernah buka Config.
...
// AppConfigJenisProduk — BARU . Pola SAMA PERSIS seperti AppConfigUkuran di atas
// (koleksi 2-kolom nama+keterangan lewat MasterDataTabelManager) —. Sumber
...
// AppConfigKomponen — BARU .— pola SAMA PERSIS (koleksi 2-kolom nama+keterangan
// lewat MasterDataTabelManager). KOLEKSI (master_komponen), BELUM
// disambungkan ke field/dropdown manapun (mis. BOM Komponen di Master Produk
// masih pakai Bahan+Warna seperti biasa) — cuma minta tab-nya, kalau nanti mau
// disambungkan ke field tertentu, tinggal diminta terpisah. BARU
// :izinkan-import-excel="true" mengaktifkan fitur Import/Upload Massal Excel +
// Template (tombol "Import / Template Excel" di sebelah searchbox) yang baru
// ditambahkan ke MasterDataTabelManager (vue-components.js). Opt-in per tab —
// tab Config lain (Satuan/Ukuran/ Warna/dst) TIDAK ikut dapat tombol ini kecuali
// propnya juga dinyalakan di situ.
...
// AppConfigTahapPersiapan — Pola SAMA PERSIS seperti
// AppConfigKomponen/AppConfigJenisProduk (koleksi 2-kolom nama+keterangan lewat
// MasterDataTabelManager). Koleksi (master_tahap_persiapan) — sumber
// DropdownCari field "Tahap Proses" di BOM Aksesoris (Master Produk > Entry
// Produk, lihat js/vue-master- produk.js) DAN dipakai filter kartu Acc
// Sewing/Webbing/Finishing di menu Persiapan Produksi V2 . diminta isi PERSIS 3
// entry "Sewing"/"Webbing"/ "Finishing" (case-insensitive dicocokkan) supaya
// ke-3 kartu filter itu bisa mengelompokkan baris BOM Aksesoris dengan benar —
// lihat catatan panjang di vue-persiapan-produksi-v2.js soal pencocokan tahap.
...
// AppConfigTlc — (coretan di screenshot: hapus kartu "Prefix Kode SPK" + tabel
// CRUD master_tlc lama, "Kode per Divisi" ganti dari form(Group Menu/Sub
// Menu/dropdown Jalur) jadi SATU TABEL — "namun bukan dropdown tapi table").
// GANTI dari versi (form input bebas + dropdown Jalur 4 opsi).
//
// DIHAPUS TOTAL (sesuai coretan, bukan disembunyikan): - Kartu "Prefix Kode SPK"
// sudah tidak dipakai kode manapun sejak redesain kode_spk 12 Sep pagi (field
// lama di pengaturan_id_spk_ grouping/config DIBIARKAN apa adanya di Firestore,
// cuma UI-nya yang hilang — tidak ada migrasi/hapus data). - Tabel CRUD
// master_tlc terpisah (Kode TLC/Nama-Deskripsi/Tipe/Aksi + form tambahnya) —
// sekarang entry master_tlc dibuat OTOMATIS lewat kolom "Nama TLC" di tabel baru
// (jawaban: "dropdown + bisa buat baru sekalian", lihat simpanBaris). GAP
// DISENGAJA: karena tabel CRUD lama dihapus, TIDAK ADA LAGI cara hapus/ubah
// entry master_tlc yang salah ketik dari layar ini — kalau perlu, hapus manual
// lewat Firestore Console (koleksi master_tlc). Koleksi master_tlc sendiri TIDAK
// berubah skema ({kode, nama, tipe}) — tetap dipakai 6+ modul lain apa adanya
// (dropdown "TLC Tujuan" pas kirim tugas: Cutting, Persiapan
// Bahan/Sewing/Webbing/Finishing/Masalah).
//
// DAFTAR_MENU_DIVISI — jawaban poin "cek repo yg berkaitan dengan anak grouping,
// dan anak separating; semua titik tlc" — digrep LANGSUNG dari kode live 12 Sep
// (bukan ditebak): 4 jalur Persiapan Produksi yang SUDAH aktif dipakai
// kode_anak_spk (bahan/sewing/webbing/finishing, lihat
// tandaiKodeAnakSpk/ambilPetaKodeTujuanDivisi js/vue-persiapan- produksi-v2.js)
// + Persiapan Masalah (pakai dropdown master_tlc juga, vue-pp-masalah.js) + 5
// titik Proses Produksi yang masing-masing punya TLC_ASAL hardcode SENDIRI di
// kodenya (Cutting=TLC-PTG, Serie=TLC-SER, Sewing=TLC-JHT, Finishing=TLC-FIN,
// Gudang Barang Jadi=TLC-GBJ — lihat konstanta TLC_ASAL_*/TLC_TUJUAN_* di
// masing2 js/vue-pp-*.js).
//
// PENTING — baris Proses Produksi (jalur_key 'pp_*') MURNI DATA-ENTRY REFERENSI
// sesi ini: kode hardcode TLC_ASAL_* di vue-pp-cutting/serie/
// sewing/finishing/gudang.js TIDAK diubah . `ambilPetaKodeTujuanDivisi` cuma
// baca key `bahan/sewing/webbing/ finishing` dari koleksi ini — baris
// `pp_*`/`masalah` aman diisi kosong kapan saja, tidak dipakai generator kode
// manapun, nol resiko regresi.
//
// Baris CUSTOM di luar 10 titik tetap ini (mis. Vendor) tetap bisa ditambah
// manual lewat form kecil di bawah tabel — jalur_key null, murni referensi
// selamanya, TIDAK pernah dipakai generator.
...
const AppConfigTlc = {
  setup() {
    ...
    // Combobox "Nama TLC" per baris. DropdownCari (vue-components.js) TIDAK
    // dipakai — komponen itu strict-select 1 field string, sedangkan di sini
    // butuh 2 field sekaligus (nama+kode) DAN mode "buat baru kalau belum ada",
    // jadi ditulis sendiri, pola SAMA (kotak ketik+filter+panel saran) seperti
    // DropdownCari.
    ...
    function tutupCariTunda(row) {
      // @mousedown.prevent di opsi (pilihTlc) membuat cariTerbuka sudah false
      // SEBELUM timeout ini jalan — kalau begitu, jangan timpa lagi (kataCari
      // sudah dikosongkan pilihTlc). Kalau masih terbuka (user blur tanpa klik
      // saran), commit teks yang diketik jadi Nama TLC baru supaya tidak hilang
      // diam-diam (dukung alur "ketik nama baru").
      setTimeout(() => {
...
// AppConfigRiwayatPin — DIPINDAH ke Scan & Cetak > PIN, lihat
// AppScanCetakRiwayatPin di js/vue-scan-cetak.js (kode identik, cuma nama
// komponen/mount point berubah). Ini konsisten dengan wireframe "05 - Scan dan
// Cetak" §4.1 (Riwayat PIN memang bagian grup PIN, bukan Config) —. Blok
// komponen lama DIHAPUS dari sini, bukan cuma dikomentari, supaya tidak ada 2
// salinan kode yang bisa menyimpang.
