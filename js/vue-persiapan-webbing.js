    ...
    async function onCetakSelesai() {
      ...
      try {
        ...
        await Promise.all(Object.entries(byTrack).map(([trackId, barisGrup]) => {
          ...
          return updateBarisWebbingMassal(trackId, (x, i) => idxSet.has(i) && !x.label_cetak_pada, () => ({ label_cetak_pada: now }));
        }));
    ...
    <div v-else class="gc-card gc-card-menonjol" style="padding:20px; border-radius:20px;">
      <div class="gc-card-head">
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          ...
          <button v-if="bolehProses && aksiAktif(MY_TARGET,'operator_webbing')" @click="bukaPenunjukanGlobal" class="btn-primary hidden md:inline-block" style="padding:8px 14px;"><i class="fas fa-qrcode" style="margin-right:6px;"></i>Scan Operator</button>
        </div>
      </div>
...
const PersiapanWebbingSedangDisiapkan = {
  ...
  template: `
          ...
          <div v-for="g in op.kelompokSpk" :key="g.trackId" style="border:1px solid var(--line); border-radius:14px; padding:10px;" :style="{ background: g.siap ? 'var(--ok-light)' : 'transparent' }">
            ...
            <div style="display:flex; flex-direction:column; gap:8px; margin-bottom:8px;">
              <div v-for="b in g.baris" :key="barisKey(b)" style="border:1px solid var(--line); border-radius:12px; padding:8px;" :style="{ background: tertahan(b.masuk_tahap_pada) ? 'var(--warn-light)' : 'transparent' }">
                ...
                <div style="font-size:10.5px; color:var(--text-faint); margin-bottom:6px;">{{ formatQty(b.butuh) }} {{ b.satuan }} &middot; {{ formatRoll(b.roll) }}</div>
    ...
    <popup-pin-generik v-if="pinCetakUlangAktif" judul="Verifikasi PIN — Cetak Ulang Label" konteks="Acc Webbing - Cetak Ulang Label" @sukses="pinCetakUlangSukses" @batal="pinCetakUlangAktif = false" />
    <popup-pratinjau-cetak-label :terbuka="popupCetakAktif" judul="Cetak Ulang Label Anak SPK" :daftar-label="daftarLabelPreview" jenis-cetak="label_spk_acc_webbing" @tutup="popupCetakAktif = false" />
  `
};
