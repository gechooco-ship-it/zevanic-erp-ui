    ...
    async function onCetakSelesai() {
      ...
      try {
        ...
        await Promise.all(Object.entries(byTrack).map(([trackId, barisGrup]) => {
          ...
          return updateBarisWebbingMassal(trackId, (b, i) => idxSet.has(i), () => ({ label_cetak_pada: now }));
        }));
  ...
  setup() {
    ...
    async function hasilScanPack(kodeMentah) {
      ...
      try {
        await Promise.all(Object.entries(byTrack).map(([trackId, barisGrup]) => {
          const idxSet = new Set(barisGrup.map(b => b._lineIdx));
          return updateBarisWebbingMassal(trackId, (b, i) => idxSet.has(i), () => ({ kode_bagging: modalPack.bagging.kode }));
        }));
    ...
    async function hasilScanKirim(kodeMentah) {
      ...
      try {
        await Promise.all(Object.entries(byTrack).map(([trackId, barisGrup]) => {
          const idxSet = new Set(barisGrup.map(b => b._lineIdx));
          return updateBarisWebbingMassal(trackId, (b, i) => idxSet.has(i), () => ({
